...
FROM  sf_item_no b,po_rcv_transactions f,www_users w
  ...
  and f.stockid = b.item_no and f.created_by = w.userid
   ";
...
<p class="p2">入库单</p>
    ...
    $ListCount = DB_num_rows($result8);
     ...
     <?php
        ...
        (b.price/(1+a.tax_rate)) no_tax_price, f.transaction_quantity,f.lot_num,c.huohao,c.units,a.tax_rate,a.tax_flag,round((b.price/(1+a.tax_rate)*rl.DELIVERY_QUANTITY),2) no_tax_amount
        ...
        ifnull(rl.DELIVERY_QUANTITY,0) DELIVERY_QUANTITY,
        rl.subinventory_code,
        b.price no_tax_price, f.transaction_quantity,f.lot_num,c.huohao,c.units,a.tax_rate,a.tax_flag,round((b.price*rl.DELIVERY_QUANTITY),2) no_tax_amount
        FROM  po_headers_all a, po_lines_all b, po_rcv_receipt_header rh, po_rcv_receipt_line  rl, sf_item_no c,vendors d,po_rcv_transactions f
        WHERE a.vendor_code=d.vendor_code
        and a.po_num=b.po_num    
        and b.po_num=rl.po_num
...
?>
    ...
    <table width="100%"  cellpadding="0" cellspacing="0" class="content_tab" >
		...
		<tr>
            ...
            <td style="text-align:left;"><center><?php echo $myrow2['no_tax_amount'];?></center></td>
