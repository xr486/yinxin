...
if (isset($_GET['op'])) {
	...
	if ($op == 'versions') {
		...
		while ($row = DB_fetch_array($r)) {
			...
			echo '<td>' . ($row['file_patch'] && file_exists($row['file_patch']) ? '<a href="' . $RootPath . '/' . $row['file_patch'] . '" download="' . htmlspecialchars($row['file_name'] . ($row['file_ext'] !== '' ? '.' . $row['file_ext'] : '')) . '">下载</a>' : '') . '</td>';
			echo '</tr>';
		}
		exit;
	}
	...
	if ($op == 'item_cat_nodes') {
		...
		if ($cat != '') {
			$r = DB_query("SELECT i.item_no, i.item_name, i.item_type,
			    ...
			    FROM sf_item_no i WHERE i.item_category1='" . $cat . "' AND i.disable_flag<>'N' ORDER BY i.item_no", $db);
			while ($row = DB_fetch_array($r)) {
...
if (isset($_GET['ajax']) && $_GET['ajax'] == 1) {
	...
	if ($ajaxFolder == -1) {
		...
		$r = DB_query("SELECT i.item_category1, COUNT(*) AS item_cnt,
		    ...
		    FROM sf_item_no i WHERE i.item_category1<>'' AND i.disable_flag<>'N' GROUP BY i.item_category1 ORDER BY i.item_category1", $db);
		$cats = array();
...
<div class="plm-layout">
	...
	<div class="plm-left">
		...
		if (!$isTemplateMode && !$isAbolition && !$isRecycle) {
			$itemCats = array();
			$r = DB_query("SELECT DISTINCT item_category1 FROM sf_item_no WHERE item_category1<>'' AND disable_flag<>'N' ORDER BY item_category1", $db);
