...
";
    ...
    if (isset($_POST['SO_from']) and $_POST['SO_from'] != '') {
        $sql = $sql . " and a.stockid >=  '" . $_POST['SO_from'] . "'";
    }
    if (isset($_POST['SO_to']) and $_POST['SO_to'] != '') {
        $sql = $sql . " and a.stockid <=  '" . $_POST['SO_to'] . "' ";
    }
    ...
    if (empty($_POST['ToDate']) == 0) {
      ...
      if($_POST['checkresult']!=""){
        if($_POST['checkresult']=="APPROVED"){
            $sql .= " and a.approved_status = 'APPROVED'";
        }
        if($_POST['checkresult']=="INPROCESS"){
             $sql .=" and a.approved_status = 'INPROCESS'";
        }
        if($_POST['checkresult']=="CANCELED"){
             $sql .= " and a.approved_status = 'CANCELED'";
        }
        if($_POST['checkresult']=="REJECTED"){
             $sql .=" and a.approved_status = 'REJECTED'";
        }
    }
      $sql .=" order by a.stockid";
...
echo '<tr><td>' . _('签核状态') . ':</td><td><select name="checkresult">';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    echo '<tr>
                    ...
                    <th class="ascending"width = 90>' . _('单价') . '</th>
                    <th class="ascending"width = 200>' . _('签核状态') . '</th>
                    <th class="ascending"width = 120>' . _('生效日期') . '</th>                           
                  
                    <th class="ascending"width = 100>' . _('失效日期') . '</th>
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> 15 )) {
            ...
            if ($myrow['approved_status'] == 'INPROCESS') {
                $v_status = '待签核';
            } elseif ($myrow['approved_status'] == 'APPROVED') {
                $v_status = '已签核';
            } elseif ($myrow['approved_status'] == 'REJECTED') {
                $v_status = '已拒签';
            }   else {
                $v_status = '已取消';
            }
