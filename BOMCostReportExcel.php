...
$rows2 = array( 
  array('BOM子料明细'),
	...
	$rows = array( 
  array('阶层','序号','制程','料号','料号名称','规格型号','用量','单位','零件位置','生效时间','失效时间','备注','单价','金额'),
);
...
foreach($rows as $row)
	...
	while ($v = DB_fetch_array($result_num)) {
     ...
     $writer->writeSheetRow('Sheet1', array(' '.'1',$v['item_num'],$v['operation_seq_num'],$v['item_no'],$v['item_name'],$v['item_desc'],
        $v['component_quantity'],$v['units'],$v['weizhi'],date('Y-m-d H:i:s', $v['effectivity_date']),$disable_date,$v['component_remarks'],$v['price'],($v['price']*$v['component_quantity']),
        ));
     /*
	 //第二层
	   $sql2 = "SELECT a.component_item,a.item_num,
				...
				FROM bom_lines_all a, sf_item_no b
				where a.component_item=b.item_no   and a.disable_date=0 
				AND a.assembly_item_no = '".$v['item_no']."'";  	 
     ...
     $writer->writeSheetRow('Sheet1', array(' '.'-2',$v2['item_num'],$v2['operation_seq_num'],$v2['component_item'],$v2['item_name'],$v2['item_desc'],
        $v2['component_quantity'],$v2['units'],'',date('Y-m-d H:i:s', $v2['effectivity_date']),$disable_date,$v2['component_remarks'],
        ));
     ...
     $writer->writeSheetRow('Sheet1', array('----5',$v5['item_num'],$v5['operation_seq_num'],$v5['component_item'],$v5['item_name'],$v5['item_desc'],
        $v5['component_quantity'],$v5['units'],'',date('Y-m-d H:i:s', $v5['effectivity_date']),$disable_date,$v5['component_remarks'],
        ));
