...
AND a.customer_code = b.customer_code 
	and a.order_number IN (SELECT c.order_number FROM quote_line_details_all c
	WHERE c.buyer_help='Y' AND ( buyer_help_date=0 or buyer_help_date='' or buyer_help_date is null )  )
";
//echo $sql;
...
echo '<input type="text" name="customer_code" value="' . $_POST['customer_code'] . '" size="7" maxlength="25" /></td>';
...
if (isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> 10 )) {
            ...
            echo '  <td><a href="' . $RootPath . '/DOPurchaseQuote2.php?New=Yes&Updateorder_number=' . $myrow['order_number'] . '">' . $myrow['order_number'] . '</td>
				<td>' . $v_status . '</td>
