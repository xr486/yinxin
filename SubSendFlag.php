...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
  ...
  a.order_line_id,
	a.item,
	a.quantity,
	a.chuanghu_fucai_item,
	a.chuanghu_fucai_quantity,
	a.line_no,
	a.order_id,
	a.creation_date,
	a. STATUS,a.schedule_ship_date,
        b.realname
FROM send_quantity_v a,
     www_users b
WHERE
	1 = 1
...
";
    ...
    if (isset($_POST['vendor']) and $_POST['vendor'] != '') {
        $sql = $sql . " and a.vendor =  '" . $_POST['vendor'] . "'";
    }

	if (isset($_POST['stockid']) and $_POST['stockid'] != '') {
        $sql = $sql . " and a.item =  '" . $_POST['stockid'] . "'";
    }
    ...
    if (empty($_POST['ToDate']) == 0) {
      ...
      $sql .=" order by a.order_number,line_no";
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    echo '<tr>      <th class="ascending"width = 200>' . _('加盟商') . '</th>
                    <th class="ascending" width = 100>' . _('订单号') . '</th>
                    <th class="ascending"width = 70>' . _('订单行') . '</th>
                    <th class="ascending"width = 120>' . _('料') . '</th>                           
                    <th class="ascending"width = 100>' . _('数量') . '</th>
                    <th class="ascending"width = 100>' . _('出货日') . '</th>
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> 10 )) {
            ...
            . '                  <td><a href="' . $RootPath . '/SubSendFlag2.php?Updateso_num=' . $myrow['type'].$myrow['order_line_id'] . '">' . $myrow['order_number'] . '</td>
				<td>' . $myrow['line_no'] . '</td>
                                <td>' . $myrow['item'] . '</td> 
