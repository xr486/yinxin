...
$Title = _('其他原因出库单整批上传');
$ViewTopic = '其他原因出库单整批上传';
$BookMark = '其他原因出库单整批上传';
include('includes/header.inc');
//include('includes/SQL_CommonFunctions.inc');
include("excel/excel.php"); 

echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/supplier.png" title="' . _('其他原因出库单整批上传') .
 '" alt="" />' . ' ' . $Title . '</p>';
...
echo '<a href="' . $RootPath . '/OutHouseBatch2.php"><h3>' . _('进行上传资料确认界面') . '</h3>';
  
}  
 else  {	echo '<form action="OutHouseBatch.php" method="post" enctype="multipart/form-data">';
