...
if (isset($_POST['NextItem'])){
	$Result = DB_query("SELECT stockid FROM stockmaster WHERE stockid>'" . $StockID . "' and CategoryID='废品' ORDER BY stockid ASC LIMIT 1",$db);
...
if (isset($_POST['PreviousItem'])){
	$Result = DB_query("SELECT stockid FROM stockmaster WHERE stockid<'" . $StockID . "' and CategoryID='废品' ORDER BY stockid DESC LIMIT 1",$db);
...
echo '<div class="centre"><a href="' . $RootPath . '/SelectProduct.php">' . _('返回选择物料') . '</a></div>
	<br />
...
echo '<tr><td>111</td><td>111</td></tr>';
echo '<tr>
...
echo '<input type="hidden"  name="CategoryID" size="12" maxlength="10" value="废品" />';

echo '<input type="hidden"  name="ProductType" size="12" maxlength="10" value="0" />';

echo '<input type="hidden"  name="ItemType" size="12" maxlength="10" value="0" />';
