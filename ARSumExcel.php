...
// $sql = "SELECT ve.customer_code,ve.customer_name, pr.*,
...
// 	FROM customers ve, so_delivery_headers_all pr 
// 	where pr.customer_code=ve.customer_code 
// 	and  pr.delivery_amount > pr.already_receive_amount +pr.dis_receive_amount 
// 	AND pr.delivery_date > '". $sql_todate . "'
// 	group by ve.customer_code,ve.customer_name";
// 	echo $sql;
$sql = "SELECT customer_code,customer_name,jieqian_amount,kaipiaoweishou_amount FROM customers";
...
$objActSheet->setCellValue('B4', '客户名称');    
...
$objActSheet->setCellValue('D5', '开票未收金额');    
$objActSheet->setCellValue('E5', '本期出货');    
$objActSheet->setCellValue('F5', '本期退货');   
$objActSheet->setCellValue('G5', '本期已收');   
$objActSheet->setCellValue('H5', '本期结欠');   
$objActSheet->setCellValue('K5', '开票金额');   
$objActSheet->setCellValue('L5', '开票未收金额');   
...
if (empty($C4)==0) {
    ...
    $n=$i+5; 
    //前期未收款
	 $sql_no_receive_amount =   "SELECT sum(delivery_amount-already_receive_amount - dis_receive_amount) no_receive_amount 
	FROM so_delivery_headers_all 
	WHERE customer_code =  '".$mysql['customer_code']."' 
	AND delivery_amount <> (already_receive_amount + dis_receive_amount )
	AND delivery_date < '". $sql_todate . "'";
    //前期未开票
    $sql_no_invoice_amount = "SELECT sum(delivery_amount-already_invoice_amount -dis_invoice_amount) no_invoice_amount 
	FROM so_delivery_headers_all 
	WHERE customer_code =  '".$mysql['customer_code']."' 
	AND delivery_amount <> (already_invoice_amount + dis_invoice_amount)  
	AND delivery_date < '". $sql_todate . "'";
  ...
  $sql_leiji_no_invoice = "SELECT sum(delivery_amount-already_invoice_amount -dis_invoice_amount) to_invoice_amount 
  FROM so_delivery_headers_all
  WHERE customer_code =  '".$mysql['customer_code']."' 
  and delivery_date >= '". $SQL_FromDate . "'
	and delivery_date <= '". $SQL_ToDate . "'
  ...
  // $mysql_leiji_no_invoice=DB_fetch_array($result8);//to_invoice_amount
   ...
   $mysql_leiji_no_invoice=$mysql['kaipiaoweishou_amount'] + $mysql_leiji_amount['already_invoice_amount'] + $sql_benqi_receive_amount['already_receive_amount'];
    ...
    $objActSheet->setCellValue('B'.$n, $mysql['customer_name']); 
