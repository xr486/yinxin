...
";
    ...
    if (isset($_POST['SO_from']) and $_POST['SO_from'] != '') {
        $sql = $sql . " and a.so_order_number >=  '" . $_POST['SO_from'] . "'";
    }
    if (isset($_POST['SO_to']) and $_POST['SO_to'] != '') {
        $sql = $sql . " and a.so_order_number <=  '" . $_POST['SO_to'] . "' ";
    }
	...
	if($_POST['checkship']!=""){
      ...
      $sql .=" order by a.delivery_num,a.deliveryline"; 
    $result = DB_query($sql, $db);
    if (DB_num_rows($result) == 0) {
        unset($result);
        prnMsg(_('找不到该送货单，请重新输入条件查询！'), 'error');
    }
}
...
// <th  width = 70>' . _('打印') . '</th>
    echo '<tr>
                    ...
                    <th  width = 100>' . _('送货单号') . '</th>
                    <th  width = 100>' . _('送货日期') . '</th>
					 <th width =50 >' . '行' . '</th>
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> 15 )) {
            ...
            echo '  <td><a href="' . $RootPath . '/SearchSOList4.php?Updateorder_number=' . $myrow['so_order_number'] . '">' . $myrow['so_order_number'] . '</td>
		
				<td>' . $v_status . '</td>
                <td> <a href="' . $RootPath . '/AddCustomers4.php?UpdateCustomerCode=' . $myrow['customer_code'] . '">' . $myrow['customer_name'] . '</td>  
        ...
        echo  ' <tr>
            <td>'.合计总价：.'</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td>
