...
$Title = _('外协采购单未收货明细查询');
$ViewTopic = '外协采购单未收货明细查询';
$BookMark = '外协采购单未收货明细查询';
...
$sql = 'SELECT pha.po_num, pha.status, pha.note, pha.creation_date,   pla.line_amount,pla.quantity,pla.line,pla.price,v.vendor_code, v.vendor_name, pla.need_date, pha.created_by,pla.stockid,g.item_name,g.units,
ifnull(pla.quantity_received,0) this_received,pla.operation_code,pla.operation_seq_num,pla.wip_entity_name
FROM po_headers_all pha,  po_lines_all pla, vendors v,sf_item_no g
...
if (isset($_POST['wip_entity_name']) and $_POST['wip_entity_name'] != '') {
	...
	if (isset($_POST['operation_seq_num']) and $_POST['operation_seq_num'] != '') {
        $sql = $sql . " and pla.operation_seq_num " . LIKE . " '%" . $_POST['operation_seq_num'] . "%' ";
    }
...
echo '
        ...
        //   <a href="' . $RootPath . '/OspPOWaitdeliveryDetailExcel.php?vendor_name=' .$_POST['vendor_name'] .'&vendor_code=' .$_POST['vendor_code'] .'&FromDate=' .$_POST['FromDate'] . '&ToDate=' .$_POST['ToDate'] .'&wip_entity_name=' . $_POST['wip_entity_name'] .'&operation_seq_num=' . $_POST['operation_seq_num'] . '&operation_code=' . $_POST['operation_code'] .'&stockid=' .$_POST['stockid'] . '&item_name=' .$_POST['item_name'] .'&po_num=' .$_POST['po_num'] . '&status=' .$_POST['checkresult'] .' ">' .'导出' . '</a>
        //   </div>';
          echo ' </div>
';
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme .
    '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('外协采购单未收货明细查询') .
    '</p>';
...
if (isset($_POST['Search']) or isset($result) or isset($_POST['Go']) or isset($_POST['Next']) or
    isset($_POST['Previous'])) {
    ...
    echo '<tr>      
		...
		<th  >' . _('料号名称') . '</th>
				   ...
				   <th class="ascending"width = 30>' . _('收货量') . '</th>
				   <th class="ascending"width = 90>' . _('待收货量') . '</th>
	 ...
	 <td>' . $myrow['operation_code'] . '</td>
                <td>' . $myrow['stockid'] . '</td>
					...
					<td>' . $myrow['this_received'] . '</td>
					 <td>' . ($myrow['quantity']-$myrow['this_received']) . '</td>
