...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    //查找数据的SQL
    $sql = "select a.*,(select ifnull(sum(quantity),0) from inv_onhand_quantity_all b where b.stockid=a.item_no ) onhand_quantity from sf_item_no a where a.item_status='待签核' ";
...
<p class="page_title_text"><img src="<?php echo $RootPath; ?>/css/<?php echo $Theme; ?>//images/magnifier.png" title="料号审核" alt="料号审核">料号审核</p>
<form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'); ?>" method ="POST">
    <div>
		...
		<!-- <input type="submit" name="add_new" value="新增料号"> -->
    </div>
            ...
            <table cellpadding="2" class="selection">
                ...
                if (DB_num_rows($result) <> 0) {
                    ...
                    while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> $_SESSION['DisplayRecordsMax'])) {//$_SESSION['DisplayRecordsMax']
                        ...
                        ?>
                        <td>
                            <a href="<?= $RootPath ?>/segment1Approve2.php?ItemID=<?= $myrow['item_id']?>&ItemNo=<?= $myrow['item_no']?>"><?= $myrow['item_no'] ?>
                        </td>
