...
if (isset($_POST['Submit'])) {
	...
	if (isset($_POST['SelectedMeasureID']) AND $_POST['SelectedMeasureID']!='' AND $InputError !=1) {
		...
		$sql = "SELECT count(*) FROM tax_set
				...
				AND tax_name ".LIKE." '" . $_POST['MeasureName'] . "'";
		...
		$sql = "SELECT count(*) FROM tax_set
				WHERE tax_name " .LIKE. " '".$_POST['MeasureName'] ."'";
		$result = DB_query($sql,$db);
		$myrow = DB_fetch_row($result);
		if ( $myrow[0] > 0 ) {
			$InputError = 1;
...
/* An unit of measure could be posted when one has been edited and is being updated
  ...
  or deletion of the records*/
	...
	echo '<table class="selection">
			<tr>
				<th class="ascending">' . _('税率名称') . '</th>
                
                <th class="ascending">' . _('税率') . '</th>
                 <th>' . _('编辑') . '</th>
                        <th>' . _('删除') . '</th>
			</tr>';
...
if (! isset($_GET['delete'])) {
	...
	echo '<tr>
     ...
     echo   '<tr>
		<td>' . _('税率') . ':' . '</td>
		<td><input " pattern="(?!^ *$)[^+<>-]{1,}" type="text" name="MeasureName2" title="'._('Cannot be blank or contains illegal characters').'" placeholder="'._('请输入税率').'" size="30" maxlength="30" value="' . $_POST['MeasureName2'] . '" /></td>
		</tr>';
