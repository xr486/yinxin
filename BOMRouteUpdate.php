...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
	...
	FROM  sf_item_no b
WHERE	item_type in ('F','B') and b.item_no  not in (select assembly_item_no from bom_routings_all) 
";
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('产品工艺建立') . '</p>';
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> 15 )) {
            ...
            echo '  <td> ' . $myrow['item_no'] . ' </td>
		      ...
		      echo '<td><a href="' . $RootPath . '/BOMRouteUpdate2.php?New=Yes&UpdateBOMItem=' . $myrow['item_id'] . '" target="_blank" >新增</td> ';
