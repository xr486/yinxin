...
if(isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
	...
	c.customer_name,
	b.price,
	-(sum(e.quantity)) quantity_so,
	b.stockid,
	d.item_name ,
	d.item_desc ,
    e.cost_price,
  -(b.price-e.cost_price)*sum(e.quantity) total_quantity
FROM
	inv_transaction_so e,
	so_lines_all b,
	customers c,
	sf_item_no d,
    so_headers_all a
WHERE
	e.so_order_number = b.order_number
...
if (isset($_POST['Stockid_to']) and $_POST['Stockid_to'] != '') {
 ...
 if (isset($_POST['customer_code']) and $_POST['customer_code']!= '') {
    ...
    $sql = $sql." group by c.customer_code,c.customer_name,	b.price,b.stockid,
	d.item_name ,d.item_desc,e.cost_price
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('出货总明细利润查询') . '</p>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ($RowIndex <> $_SESSION['DisplayRecordsMax'])) {
            ...
            <td>' . $myrow['quantity_so']  . '</td>  
            <td>' . $myrow['cost_price']  . '</td> 
            <td>' . $myrow['price']  . '</td>
			<td>' . $myrow['total_quantity']  . '</td>  
			
			</tr>';
             $total_amount = $total_amount+ $myrow['total_quantity'];
            $RowIndex++;
			$i++;
			$RowIndex++;
