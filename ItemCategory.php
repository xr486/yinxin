...
if (isset($_POST['Submit'])) {
	...
	if (isset($_POST['SelectedMeasureID']) AND $_POST['SelectedMeasureID']!='' AND $InputError !=1) {
		...
		$sql = "SELECT count(*) FROM sf_item_category
		...
		if ( $myrow[0] > 0 ) {
			...
			$sql = "INSERT INTO sf_item_category (unitname )
...
// PREVENT DELETES IF DEPENDENT RECORDS IN 'sf_item_no'
	...
	if ( DB_num_rows($result) == 0 ) {
		// This is probably the safest way there is
		prnMsg( _('不能删除该料号类型，因为料号类型已经不存在'),'warn');
	} else {
		$myrow = DB_fetch_row($result);
		$OldMeasureName = $myrow[0];
		$sql= "SELECT COUNT(*) FROM sf_item_no WHERE item_category ".LIKE." '" . $OldMeasureName . "'";
		$result = DB_query($sql,$db);
		$myrow = DB_fetch_row($result);
		if ($myrow[0]>0) {
			prnMsg( _('不能删除这个料号类型，因为有料号属于该料号类型'),'warn');
			echo '<br />' . _('There are') . ' ' . $myrow[0] . ' ' . _('个材料属于这个材料类型') . '</font>';
		} else {
			$sql="DELETE FROM sf_item_category WHERE unitname ".LIKE."'" . $OldMeasureName . "'";
			$result = DB_query($sql,$db);
			prnMsg( $OldMeasureName . ' ' . _('材料类型删除成功') . '!','success');
		}
...
/* An unit of measure could be posted when one has been edited and is being updated
  ...
  or deletion of the records*/
	...
	while ($myrow = DB_fetch_row($result)) {
		...
		echo '<td><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?SelectedMeasureID=' . $myrow[0] . '&amp;delete=1" onclick="return confirm(\'' . _('是否要删除这个材料类型?') . '\');">' . _('Delete')  . '</a></td>';
		echo '</tr>';
...
if (isset($SelectedMeasureID)) {
	echo '<div class="centre">
			<a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '">' . _('查看材料类型') . '</a>
		</div>';
}
