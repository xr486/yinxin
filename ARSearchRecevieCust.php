...
if(isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
    ...
    $sql =" SELECT  realname,ch.ReceiveNum,
											ch.ReceiveAmount,
											ch.taxAmount,
											ch.customer_code, 
											ch.narrative,
                                            ch.ReceiveDate, cl.receiveline, 										
													cl.invoicenum, 
													cl.amount,
													cl.linenarrative,cl.so_order_number,
													cl.so_line_no,cl.delivery_num,ch.creationdate	
                    FROM  ar_receive_headers_all ch ,
					ar_receive_lines_all cl,
					 www_users a
                   where  a.userid=ch.customer_code
				   and cl.customer_code=ch.customer_code
				   and ch.ReceiveNum=cl.ReceiveNum     " ;
  
if (empty($_POST['FromDate'])==0) {
    $SQL_FromDate = strtotime( $_POST['FromDate']);
    //echo $SQL_FromDate;
    $sql .= " and  ReceiveDate >= '" . $SQL_FromDate . "' ";
}
if (empty($_POST['ToDate'])==0) {
    $SQL_ToDate = strtotime( $_POST['ToDate']) + 86400;
     //echo $SQL_ToDate;
    $sql .= " and  ReceiveDate <='" . $SQL_ToDate . "' ";
}
if(isset($_POST['customer_code']) and $_POST['customer_code'] != ''){
    ...
    if(isset($_POST['customer_name']) and $_POST['customer_name'] != ''){
        $sql = $sql." and realname ".LIKE." '%".$_POST['customer_name']."%' ";
    }

$sql = $sql." and a.userid= '".$_SESSION['UserID']." ' ";
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('查找已付款记录查询') . '</p>';
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ($RowIndex <> 10)) {//$_SESSION['DisplayRecordsMax'])
			if ($k == 1) {
			 ...
			 $creationdate= date('Y-m-d H:i:s',$myrow['creationdate']);  
        	echo ' <td>' . $myrow['ReceiveNum'] . '</td> 					
				<td class="number">' . $myrow['receiveline'] . '</td>
				<td class="number">' .$myrow['ReceiveAmount'] . '</td> 
				<td class="number">' .$myrow['taxAmount'] . '</td> 
				<td class="number">' . $ReceiveDate . '</td>
				<td class="number">' . $myrow['narrative'] . '</td>	
				<td  >' . $myrow['invoicenum'] . '</td>	
                <td class="number">' . $myrow['amount'] . '</td>
				<td class="number">' .$myrow['linenarrative'] . '</td>
				<td class="number">' .$myrow['so_order_number'] . '</td>
				<td class="number">' .$myrow['so_line_no'] . '</td>
				<td class="number">' .$myrow['delivery_num'] . '</td>
