...
unset($result);
 ...
 $sql = 'select a.customer_code,a.customer_name,b.customer_item,b.remark,c.item_no,c.item_name,c.item_desc,b.enable_flag,b.item_relation_id 
    ...
    if(isset($_GET['CustomerCode']) and $_GET['CustomerCode'] != ''){
        $sql = $sql." and a.customer_code ".LIKE." '%".$_GET['CustomerCode']."%' ";
    }
    if(isset($_GET['CustomerName']) and $_GET['CustomerName'] != ''){
        $sql = $sql." and a.customer_name ".LIKE." '%".$_GET['CustomerName']."%' ";
    }
...
$rows = array( 
  array('客户简称','客户全称','料号','料号名称','规格型号','是否生效'),
...
foreach($rows as $row)
	...
	while ($v = DB_fetch_array($result_num)) {
	 ...
	 $writer->writeSheetRow('Sheet1', array($v['customer_code'],$v['customer_name'],$v['item_no'],$v['item_name'],$v['item_desc'],$v['enable_flag']
        ));
	 }
