<?php 
	...
	if (!isset($ItemID)) {
		header('Location: Searchitem_no.php');
	}
	...
	if (isset($_POST['Save'])) {
		if (!empty($_FILES["Pic"]["tmp_name"])) {
			...
			$_POST['PicPath'] = "itempic/" . $_POST['item_no'] .".jpg";
		}else{
		...
		if ($uploadflag == 1) {
			move_uploaded_file($_FILES["Pic"]["tmp_name"],"itempic/" . $_POST['item_no'] .".jpg");
	...
	$sql = "select * from sf_item_no where item_id ='".$ItemID."' ";
	...
	while ($v = DB_fetch_array($result)) {
		...
		$_POST['vendor_code'] = $v['vendor_code'];
		$_POST['PicPath'] = $v['pic_path'];
...
?>
<div class="centre"><a href="<?=$RootPath?>/MaterialNumberMaintain.php">返回查找料号</a></div>
...
<table class="selection">
	...
	<tr>
        <td>供应商：</td>
        <td><input type="text" name="vendor_code" value="<?= $_POST['vendor_code'] ?>" required="required"></td>
        <td>采购价：</td>
