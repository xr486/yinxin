...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    $sql = "select pha.transaction_type,
                ...
                pha.creation_date,pha.created_by,pha.vendor_code,c.vendor_name,pha.status,pha.execute_flag
	from fin_bank_transaction_headers_all pha, vendors c
            where pha.status='核准'
			and pha.transaction_type IN ('AP付款')
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('供应商付款执行') . '</p>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    echo '<tr>
                    ...
                    <th class="ascending" width = 100>' . _('已执行') . '</th>
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> 10 )) {
            ...
            unset($v_status);
           if($myrow['execute_flag'] == 'Y'){
echo '  <td>' . $myrow['transaction_num'] . '</td>'; 
           }else{

             echo '  <td><a href="' . $RootPath . '/APPaymentExecute2.php?New=Yes&Updatepo_num=' . $myrow['transaction_num'] . '">' . $myrow['transaction_num'] . '</td>'; 
           }
             if($myrow['execute_flag'] == 'Y'){
echo '  <td>是</td>'; 
           }else{
