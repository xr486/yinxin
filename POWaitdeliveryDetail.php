...
if (isset($_POST['Search']) and isset($result) or isset($_POST['Go']) or isset($_POST['Next']) or
    ...
    echo '<tr>      
	               <th class="ascending" width = 90>' . _('供应商编号') . '</th>
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        echo '<input type="hidden" name="JustSelectedAvendor" value="Yes" />';
		 echo '<div>
                 <a href="' . $RootPath . '/POWaitdeliveryDetailExcel.php?vendor_name=' .$_POST['vendor_name'] .'&vendor_code=' .$_POST['vendor_code'] .'&FromDate=' .$_POST['FromDate'] . '&ToDate=' .$_POST['ToDate'] .'&stockid=' .$_POST['stockid'] . '&item_name=' .$_POST['item_name'] .'&po_num=' .$_POST['po_num'] . '&status=' .$_POST['checkresult'] .' ">' .'资料导出成Excel表' . '</a>
                </div>';
    }
