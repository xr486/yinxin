...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('客户出货汇总查询') . '</p>';
...
echo '<input type="text" onfocus="WdatePicker()" alt="' . $_POST['FromDate'] . '" name="FromDate" maxlength="10" size="15" value="' . $_POST['FromDate'] . '" />';
echo '</div>';

echo '<div class="text-nav-1"><div>' . _('出货日期止') . ':</div>';
echo '<input type="text" onfocus="WdatePicker()" alt="' . $_POST['ToDate'] . '" name="ToDate" maxlength="10" size="15" value="' . $_POST['ToDate'] . '" />';
echo '</div>';

echo '<div class="text-nav-1"><div>' . _('料号起') . ':</div>';
echo '<input type="text"  value="' . $_POST['Stockid_from'] . '" name="Stockid_from" maxlength="10" size="15"  />';
echo '</div>';

echo '<div class="text-nav-1"><div>' . _('料号止') . ':</div>';
echo '<input type="text"  value="' . $_POST['Stockid_to'] . '" name="Stockid_to" maxlength="10" size="15"  />';
echo '</div>';
echo '</div>';
//echo '</tr>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
    ...
    echo '<tr>    <th bgcolor="#87CEFA" class="ascending">' . _('客户代码') . '</th>
                        ...
                        <th bgcolor="#87CEFA"  >' . _('出货总数量') . '</th>   
						<th bgcolor="#87CEFA"  >' . _('出货总价') . '</th>   
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ($RowIndex <> $_SESSION['DisplayRecordsMax'])) {
			...
			<td>' . $myrow['delivery_amount']  . '</td>  
             ...
             $total_amount = $total_amount+ $myrow['delivery_amount'];
            $RowIndex++;
			$i++;
			$RowIndex++;
...
<td><?php echo $total_amount ?></td>
		...
		echo '</table>';
                ...
                echo '<div>
                        <a href="' . $RootPath . '/SoshipsumReportExcel.php?customer_name=' .$_POST['customer_name'] .
                        '&customer_code=' .$_POST['customer_code'] .'&Stockid_from='.$_POST['Stockid_from'] .
