     ...
     foreach ($_POST as $key => $value) {
            if (mb_substr($key, 0, 6) == 'status') {  
                ...
                if(($_POST['accept'.$i]!=''||$_POST['reject'.$i]!='') && $checkQty!=1){
                    ...
                    $sql1="UPDATE po_lines_all set 
                               quantity_received=ifnull(quantity_received,0)-".$_POST['accept'.$i]." , 
			 ...
			 $sql2="delete from po_rcv_transactions 				 
			...
			and receipt_line='".$_POST['receipt_line'.$i]."' ";
                $result = DB_query($sql2, $db);
					...
					$sqlUpdatercvline="delete from po_rcv_receipt_line 
				 where receipt_num='".$_POST['receipt_num'.$i]."'   
			and  po_num='".$_POST['po_num'.$i]."'  
			and  po_line='".$_POST['po_line'.$i]."' 
			and receipt_line='".$_POST['receipt_line'.$i]."' ";
       ...
       unset($result_trancsation);
        ...
        if ($InputError == 1) {
            ...
            $msg = '检测成功！';
            prnMsg($msg, 'success');
             echo '<br /><div class="centre"><a href="' . $RootPath . '/POReceiptDelete.php">' . _('采购来料报检删除') . '</a></div>';
...
echo '<p class="page_title_text">
		<img src="' . $RootPath . '/css/' . $Theme . '/images/supplier.png" title="' . '采购来料报检删除' .
 '" alt="" />' . ' ' . $Title . '
	</p>';
if (isset($NUM) and $NUM != '') {
    ...
    $sql = "select 
rh.receipt_num,rl.receipt_line,rl.receipt_line_id,
a.po_num,
b.line,
b.stockid,
c.item_desc,
...
//echo '<div style="width:1300px;height:400px;overflow-x: hidden; overflow-y: scroll;">';
        ...
        while ($myrow = DB_fetch_array($TransResult)) {
            ...
            echo '<td>' . $myrow['this_received'] . '  '; 
            echo'  <input type="hidden"   name="accept'.$myrow['receipt_line_id'].'" size="4" maxlength="25"  value="' . $myrow['this_accept'] . '" /> ';
...
echo '<input type="hidden" name="NUM" value="' . $myrow['receipt_num'] . '"/></tr>';
        ...
        echo '</div>';
        echo '<div class="centre">
			<input type="submit" name="Submit" value="' . _('确认删除') . '" />
...
if (isset($_POST['return'])) {
    ...
    header('Location: POReceiptDelete.php');
}
