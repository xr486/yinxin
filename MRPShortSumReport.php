...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    if (empty($_POST['ToDate']) == 0) {
	 ...
	 $sql .= " group by b.segment1, c.item_desc,c.item_name,c.units"; 
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('缺料汇总报表') . '</p>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    if (isset($ListPageMax) AND $ListPageMax > 1) {
	    ...
	    echo '<br>
          <div class="centre">
                <a href="' . $RootPath . '/MRPShortSumReportExcel.php?wip_entity_name=' .$_POST['wip_entity_name'] . '&item_no=' .$_POST['item_no'].'&item_name=' .$_POST['item_name'] .'&item_desc=' .$_POST['item_desc'] . '">
                ' .'资料导出Excel表' . '
                </a>
          </div>';
