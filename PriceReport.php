...
if(isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
        $sql = "SELECT a.vendor_code,a.last_update_date,a.last_updated_by, a.item_no,b.item_name,a.price,a.last_price,b.item_desc,b.units
        FROM po_vendor_item_price a, sf_item_no b 
   ...
   if(isset($_POST['item_no']) and $_POST['item_no'] != ''){ 
    ...
    if (DB_num_rows($result)==0) {
        unset($result);
        prnMsg(_('找不到该材料价格，请重新输入条件查询！') ,'error');
    }
}
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('查找材料价格') . '</p>';
echo '<table cellpadding="3" class="selection">';
echo '<div class="text-nav">';
 
echo '<div class="text-nav-1"><div>' . _('供应商') . ':</div>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
    ...
    echo '<tr>  
	 <th  width = 80>' . _('供应商') . '</th>
                    ...
                    <th  width = 90>' . _('规格型号') . '</th>
                    <th class="ascending"width = 150>' . _('单位') . '</th>
                    <th  width = 90>' . _('上次单价') . '</th>
                    <th  width = 80>' . _('单价') . '</th>
                    <th  width = 50>' . _('建立人员') . '</th>
    ...
    if (DB_num_rows($result) <> 0) {
		...
		echo '</table></div>';
                ...
                echo '<div>
                 <a href="' . $RootPath . '/PriceReportExcel.php?status=' .$_POST['checkresult'] .'&ToDate=' .$_POST['ToDate'] .'&item_no=' .$_POST['item_no'] .'&item_name=' .$_POST['item_name'] .'&po_num=' .$_POST['po_num'] . '&ToDate=' .$_POST['ToDate']  .' ">' .'资料导出Excel表' . '</a>
                </div>';
        }
