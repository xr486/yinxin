...
$msg = '料号'.$_SESSION['OrderNum' . $identifier].'建立成功！';
                prnMsg($msg, success);
                 echo '<br /><div class="centre"><a href="' . $RootPath . '/AddItemNo.php?New=Y">' . _('继续新建产品') . '</a></div>';
                 echo '<br /><div class="centre"><a href="' . $RootPath . '/segment1set.php">' . _('查询产品资料') . '</a></div>';
...
$sql = "insert into sf_item_no_file (file_name,item_no,file_patch,creation_date,created_by) values ('".$_POST['file_name']."','".$_POST['ItemNo']."','".$dest."','".$time."','".$_SESSION['UserID']."')";
...
<table class="selection">
  ...
  <td ><input type="text"  required="required"  maxlength="200" size="20" name="file_name"  value="<?=$_POST['file_name']?>" /> </td>
		...
		<td><input  type="hidden" name="ItemNo"   value="<?=$_SESSION['OrderNum' . $identifier]?>" size="8" maxlength="25"/> 
