...
unset($result);
 ...
 $sql = "SELECT
    ...
    a.need_date, 
    a.creation_date
FROM quote_headers_all a,
    customers b 
WHERE
    1 = 1
AND a.customer_code = b.customer_code
AND a.is_send_signed='N' 
 and a.order_number not in (SELECT d.order_number FROM quote_lines_all d
	WHERE d.rd_help='Y' AND ( rd_help_date=0 or rd_help_date='' or rd_help_date is null)  )
	and a.order_number not in (SELECT d.order_number FROM quote_line_details_all d
	WHERE d.buyer_help='Y' AND ( buyer_help_date=0 or buyer_help_date='' or buyer_help_date is null)  )
";
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('查询报价单') . '</p>';
...
if (isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous']) ) {
    ...
    echo '<tr>
                    ...
                    <th class="ascending"width = 180>' . _('报价单日期') . '</th>
            </tr>';
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> 10 )) {
            ...
            echo '  <td><a href="' . $RootPath . '/SendSigned2.php?New=Yes&Updateorder_number=' . $myrow['order_number'] . '">' . $myrow['order_number'] . '</td>
				<td>' . $v_status . '</td>
