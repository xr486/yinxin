...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
  ...
  $sql="SELECT 
    ...
    p.delivery_date,    
    p.creation_date,
    p.created_by,p.narrative,(SELECT realname FROM www_users w WHERE w.userid=p.created_by) realname,
    p.last_update_date,
    p.last_updated_by  
 ...
 and delivery_type='出货' " ;
    ...
    if (DB_num_rows($result) == 0) {
        unset($result);
        prnMsg(_('找不到需要处理的出货单，请重新输入条件查询！'), 'error');
    }
}
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('出货单签核处理') . '</p>';

echo '<div class="text-nav">';
echo '<div class="text-nav-1"><div>' . _('出货申请单起') . ':</div>';
echo '<input type="text" name="SO_from" value="' . $_POST['SO_from'] . '" size="20" maxlength="25" /></div>';
echo '<div class="text-nav-1"><div>' . _('出货申请单止') . ':</div>';
...
echo '<div class="text-nav-1"><div> 出货日期从:</div>
		...
		<div class="text-nav-1"><div>' . _('到出货日期') . ':</div>
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    echo '<tr>
                     <th class="ascending"width = 70>' . _('出货单号') . '</th>
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> 10 )) {
            ...
            echo '   <td><a target="_blank" href="' . $RootPath . '/ShipOrderApprove2.php?Updatedelivery_num=' . $myrow['delivery_num'] . '">' . $myrow['delivery_num'] . '</td> 
