...
if(isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
    $sql = 'SELECT userid,a.realname ,
					salesman ,phone,email,address,lastvisitdate
				FROM www_users a where 1=1 ';
    if(isset($_POST['userid']) and $_POST['userid'] != ''){
        $sql = $sql." and userid ".LIKE." '%".$_POST['userid']."%' ";
    }
    if(isset($_POST['realname']) and $_POST['realname'] != ''){
        $sql = $sql." and realname ".LIKE." '%".$_POST['realname']."%' ";
    }
...
echo '<tr><td>' . _('登陆账号') . ':</td><td>';
echo '<input type="text" name="userid" value="' . $_POST['userid'] . '" size="20" maxlength="25" /></td>';
echo '<td>' . _('使用者名称') . ':</td>
	<td>';
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ($RowIndex <> 20)) {
			...
			echo '  <td>' .$myrow['userid'] . '</td>
				...
				<td>' . $myrow['email'] . '</td> 
				<td>' . $myrow['address'] . '</td>  
				<td>' . $myrow['lastvisitdate'] . '</td> 
				<td><a target="_blank" href="'.$RootPath.'/UserFunctionAll.php?Updateuser_num=' . $myrow['userid'] . '">' . _('权限管理') . '</a></td> 
				 <td><a target="_blank" href="'.$RootPath.'/UserAndFunction.php?Updateuser_num=' . $myrow['userid'] . '">' . _('增加权限') . '</a></td> 
				';
