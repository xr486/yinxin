...
if(isset($_POST['customer_code']) and $_POST['customer_code'] != ''){
        $sql = $sql." and pr.customer_code ".LIKE." '%".$_POST['customer_code']."%' ";
    }
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('销售出货明细报表') . '</p>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
    ...
    echo '<tr>
				 ...
				 <th width="150">销售出货单号</th>
					<th width="200">出货内容</th>   
					<th width="80" >出货日</th>
					<th width="80" >预计付款日</th> 
					<th width="80">出货金额</th>
					<th width="80">已开票金额</th>  
					<th width="80">已优惠金额</th>  
					<th width="80">未开票金额</th>
					<th width="80">已收款金额</th>
					<th width="80">已优惠金额</th>
					<th width="80">未收款金额</th>
            </tr>';
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ($RowIndex <> 10)) {//$_SESSION['DisplayRecordsMax'])
			if ($k == 1) {
			 ...
			 $v_wait_amount=$myrow['need_payment_amount']-$myrow['already_invoice_amount'] - $myrow['dis_invoice_amount'];
			 $v_wait_receive_amount=$myrow['need_payment_amount']-$myrow['already_receive_amount'] - $myrow['dis_receive_amount'];
			 $delivery_date= date('Y-m-d',$myrow['delivery_date']); 
			  $need_receive_date= date('Y-m-d',$myrow['need_payment_date']); 
			  ...
			  $already_receive_amount=$already_receive_amount + $myrow['already_receive_amount'];
			  $dis_receive_amount=$dis_receive_amount + $myrow['dis_receive_amount'];
              $v_all_wait_receive_amount= $v_all_wait_receive_amount + $v_wait_receive_amount;


        	echo ' <td>' . $myrow['customer_code'] . '</td> 
				<td>' . $myrow['customer_name'] . '</td>
				<td>' . $myrow['shipment_num'] . '</td> 
				<td>' . $myrow['shipment_remark'] . '</td> 		
				<td >' . $delivery_date . '</td>
				<td >' . $need_receive_date . '</td>
                <td class="number">' . $myrow['need_payment_amount'] . '</td>
				<td class="number">' .$myrow['already_invoice_amount'] . '</td>
				<td class="number">' .$myrow['dis_invoice_amount'] . '</td> 
				<td class="number">' .$v_wait_amount. '</td>   
				<td class="number">' .$myrow['already_receive_amount'] . '</td>
				<td class="number">' .$myrow['dis_receive_amount'] . '</td> 
				<td class="number">' .$v_wait_receive_amount. '</td>    
			</tr>';
			$i++;
			$RowIndex++;
		...
		echo '  <td> '.'合计'.'</td> <td></td>  <td></td>  <td></td> <td></td> <td></td> <td> '.$need_payment_amount.'</td>  <td> '.$already_invoice_amount.'</td>  <td>'.$dis_invoice_amount.' </td> <td>' .$v_all_wait_amount  . '</td>
		<td>' .$already_receive_amount  . '</td>
		<td>' .$dis_receive_amount  . '</td>
		<td>' .$v_all_wait_receive_amount  . '</td>';
		echo '</table>';
