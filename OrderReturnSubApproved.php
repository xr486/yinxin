...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
 ...
 and status='待仓库审核'
 and delivery_type='退货' and sub_approved_by is NULL and shouhuo_approved_by is not NULL and u.userid = p.created_by " ;
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('销售退货入库审核') . '</p>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> 10 )) {
            ...
            echo '   <td><a target="_blank" href="' . $RootPath . '/OrderReturnSubApproved2.php?Updatedelivery_num=' . $myrow['delivery_num'] . '&subinventory='.$_POST['insubinventory'].'">' . $myrow['delivery_num'] . '</td> 
