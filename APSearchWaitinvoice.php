...
$sql ="SELECT d.vendor_code,a.stockid,b.*,sf.item_no,sf.item_name
FROM po_headers_all c,po_lines_all a, po_rcv_receipt_line b,vendors d,sf_item_no sf
WHERE  a.po_num=c.po_num  and a.po_num=b.po_num and a.line=b.po_line and  b.check_amount > b.invoice_amount
and c.vendor_code=d.vendor_code  and sf.item_no = a.stockid
              and 1=1 " ;
...
if(isset($_POST['receipt_num']) and $_POST['receipt_num'] != ''){
  ...
  $sql = $sql." order by b.creation_date ";
    $result = DB_query($sql,$db);
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ($RowIndex <> $_SESSION['DisplayRecordsMax'])) {//$_SESSION['DisplayRecordsMax'])
			if ($k == 1) {
             ...
             $waitquantity=$myrow['check_amount']-$myrow['invoice_amount'] ;		
			 $v_all_waitamount= $v_all_waitamount + $waitquantity;  
			 $v_all_invoice_amount=$v_all_invoice_amount +   $myrow['invoice_amount'] ;  
			 $v_all_po_all_amount=$v_all_po_all_amount +   $myrow['check_amount'] ; 
		...
		echo '  <td> '.'合计'.'</td>   <td></td> <td></td> <td></td><td></td><td></td><td></td> <td> '.$v_all_po_all_amount.'</td>     <td>  '.$v_all_invoice_amount.'</td> <td>' .$v_all_waitamount  . '</td> ';
		echo '</table>';
