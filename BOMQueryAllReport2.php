...
<div class="text-nav">
	...
	bom_routings_all c 
		...
		from bom_substitutes_all bsa where b.component_sequence_id=bsa.component_sequence_id ) sub_count,b.creation_date,b.created_by,b.last_update_date,b.last_updated_by
        ...
        if (DB_num_rows($result2) == 0) {
            ...
            $tableheader = '<tr><th width =50 >' . '阶层' . '</th>
	                             ...
	                             <th width =50 >' . '制程' . '</th> 
									   ...
									   <th width =120 >' . '备注' . '</th> 
									   <th width =120 >' . '建立者' . '</th> 
									   <th width =120 >' . '建立时间' . '</th> 
									   <th width =120 >' . '最近修改者' . '</th> 
...
echo '<div>
        <a href="' . $RootPath . '/BOMQueryAllReportExcel.php?item_no=' .$searchitem_no .' ">' .'资料导出Excel表' . '</a>
    </div>';
...
function DisplayBOMItems( $Component,$Level, $db ) {
		...
		$sql4 = "SELECT a.component_item,a.item_num,
						...
						(select sum(quantity) from inv_onhand_quantity_all d where d.stockid=a.component_item) as qoh,a.creation_date,a.created_by,a.last_update_date,a.last_updated_by
