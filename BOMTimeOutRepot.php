...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    $sql = "SELECT b.item_no,b.item_name,b.item_desc,b.creation_date,b.created_by,b.item_category1,b.youxiao_date
	FROM  sf_item_no b
WHERE	item_no in (select component_item from bom_lines_all) 
...
";
    ...
    if (isset($_POST['item_category']) and $_POST['item_category'] != '') {
        $sql = $sql . " and b.item_category1 " . LIKE . " '%" . $_POST['item_category'] . "%' ";
    }
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('料号过期查询') . '</p>';
echo '<table cellpadding="3" class="selection">';
echo '<div class="text-nav">';
echo '<div class="text-nav-1"><div>' . _('料号起') . ':</div>';
echo '<input type="text" name="item_no_from" value="' . $_POST['item_no_from'] . '" size="20" maxlength="25" /></div>';
echo '<div class="text-nav-1"><div>' . _('料号止') . ':</div>
';
echo '<input type="text" name="item_no_to" value="' . $_POST['item_no_to'] . '" size="20" maxlength="25" /></div>';

echo '<div class="text-nav-1"><div>' . _('料号分类') . ':</div>';
echo '<input type="text" name="item_category" value="' . $_POST['item_category'] . '" size="20" maxlength="25" /></div>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    echo '<tr>
                    ...
                    <th class="ascending"width = 170>' . _('有效期') . '</th>
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> $_SESSION['DisplayRecordsMax'] )) {
            ...
            echo '  <td><a href="' . $RootPath . '/SearchWhereUseDetail.php?searchitem_no=' . $myrow['item_no'] . '" target="_blank">' . $myrow['item_no'] . '</td>
			
                                <td>' . $myrow['item_name'] . '</td> 
								<td>' . $myrow['item_desc'] . '</td> 
                                <td>' . $myrow['youxiao_date'] . '</td>			
                            
				<td>' . date('Y-m-d H:i:s', $myrow['']) . '</td>
