...
if (isset($_POST['Submit'])) {
	...
	if (isset($_POST['SelectedMeasureID']) AND $_POST['SelectedMeasureID']!='' AND $InputError !=1) {
		...
		$sql = "SELECT count(*) FROM sf_item_wendu
				WHERE wenduid <> '" . $SelectedMeasureID ."'
				AND wendu ".LIKE." '" . $_POST['MeasureName'] . "'";
		$result = DB_query($sql,$db);
		$myrow = DB_fetch_row($result);
		if ( $myrow[0] > 0 ) {
			$InputError = 1;
			prnMsg( _('温度有重复，请重新输入'),'error');
		} else {
			...
			// relavant
			$sql = "SELECT wendu FROM sf_item_wendu WHERE wenduid = '" . $SelectedMeasureID . "'";
			$result = DB_query($sql,$db);
			if ( DB_num_rows($result) != 0 ) {
				...
				$sql = array();
				$sql[] = "UPDATE sf_item_wendu SET wendu='" . $_POST['MeasureName'] . "' WHERE wendu ".LIKE." '".$OldMeasureName."'";
				 
			} else {
				$InputError = 1;
				prnMsg( _('温度不存在.'),'error');
			}
		}
		$msg = _('温度已经更新');
	} elseif ($InputError !=1) {
		...
		$sql = "SELECT count(*) FROM sf_item_wendu
				WHERE wendu " .LIKE. " '".$_POST['MeasureName'] ."'";
		$result = DB_query($sql,$db);
		$myrow = DB_fetch_row($result);
		if ( $myrow[0] > 0 ) {
			$InputError = 1;
			...
			$sql = "INSERT INTO sf_item_wendu (wendu )
...
// PREVENT DELETES IF DEPENDENT RECORDS IN 'sf_item_no'
	...
	if ( DB_num_rows($result) == 0 ) {
		// This is probably the safest way there is
		prnMsg( _('不能删除该温度，因为温度已经不存在'),'warn');
	} else {
		$myrow = DB_fetch_row($result);
		$OldMeasureName = $myrow[0];
		 
			$sql="DELETE FROM sf_item_wendu WHERE wendu ".LIKE."'" . $OldMeasureName . "'";
			$result = DB_query($sql,$db);
...
/* An unit of measure could be posted when one has been edited and is being updated
  ...
  or deletion of the records*/
	...
	echo '<table class="selection">
			<tr>
				<th bgcolor="#87CEFA" class="ascending">' . _('温度') . '</th>
			</tr>';
...
if (isset($SelectedMeasureID)) {
	echo '<div class="centre">
			<a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '">' . _('查看温度') . '</a>
		</div>';
}
...
if (! isset($_GET['delete'])) {
	...
	if (isset($SelectedMeasureID)) {
		...
		$result = DB_query($sql, $db);
		if ( DB_num_rows($result) == 0 ) {
			prnMsg( _('找不到需要的温度，请重试.'),'warn');
			unset($SelectedMeasureID);
		} else {
			$myrow = DB_fetch_array($result);

			$_POST['MeasureID'] = $myrow['wenduid'];
			$_POST['MeasureName']  = $myrow['wendu'];
