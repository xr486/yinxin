...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    $sql = "select invoice_name,ar_invoice_type,
			   ...
			   pha.invoice_amount,payment_amount,
			       pha.tax_amount,
                  c.customer_code,
                    pha.remark,
				pha.currency_code,
	...
	from ar_invoice_headers_all pha, customers c
            where  pha.customer_code=c.customer_code and pha.status='核准'  ";
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('未收款明细查询') . '</p>';
...
echo '</table><div class="centre"><input type="submit" name="Search" value="查找">&nbsp;&nbsp;
  <a href="' . $RootPath . '/ARSearchInvoiceExcel.php?FromDate=' .$_POST['FromDate'] .
                    ...
                    '&customer_name=' .$_POST['customer_name'] .'&status=' .$_POST['status'] . '&customer_code=' .$_POST['customer_code'] .' ">' .'导出' . '</a></div>'
 . '</br>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    echo '<tr> <th class="ascending" width = 100>' . _('发票单号') . '</th>
                    ...
                    <th class="ascending"width = 90>' . _('已收款金额') . '</th>  
