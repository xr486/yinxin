...
$Title = _('预付款匹配采购入库明细');
$ViewTopic= '预付款匹配采购入库明细';
$BookMark = '预付款匹配采购入库明细';
...
if(isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
    $sql = 'select ah.payment_num,ah.payment_amount,ah.tax_amount,ah.payment_date,ah.narrative
            ,al.payment_line,al.amount,ah.vendor_code,ve.vendor_name,al.receipt_num,al.receipt_line,
			 al.amount,al.dis_amount ,ah.bankchangenum,ah.currency_code,ah.bankaccountname,ah.payment_type,ah.prepayment_used 
            from  ap_payment_headers_all ah,
            ap_payment_lines_all al,
			 po_rcv_receipt_header pr,
			vendors ve
            where ah.payment_num=al.payment_num
			and al.vendor_code=pr.vendor_code
			and al.receipt_num=pr.receipt_num
			and ve.vendor_code=ah.vendor_code
			and ah.payment_type="预付货款" ';
