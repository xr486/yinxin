<?php
/* $Id: UnitsOfMeasure.php 6495 2013-12-11 23:05:30Z rchacon $*/
...
// PREVENT DELETES IF DEPENDENT RECORDS IN 'sf_item_no'
	...
	if ( DB_num_rows($result) == 0 ) {
		...
		$myrow = DB_fetch_row($result);
		$OldMeasureName = $myrow[0];
		$sql= "SELECT COUNT(*) FROM sf_item_no WHERE units ".LIKE." '" . $OldMeasureName . "'";
		$result = DB_query($sql,$db);
		$myrow = DB_fetch_row($result);
		if ($myrow[0]>0) {
...
/* An unit of measure could be posted when one has been edited and is being updated
  ...
  or deletion of the records*/
	...
	echo '<table class="selection">
			<tr>
				<th bgcolor="#87CEFA" class="ascending">' . _('Units of Measure') . '</th>
			</tr>';
...
if (! isset($_GET['delete'])) {
	...
	echo '<tr>
		<td bgcolor="#87CEFA">' . _('Unit of Measure') . ':' . '</td>
