...
$rows = array( 
  array('领用日期','料号' ,'料号名称', '规格型号','单位','领用数量' ,'金额','领用部门','领用项目','领用人', '批号/SN号','仓库', '备注'  ,'建立日期', '交易单号', '交易类型' ),
...
foreach($rows as $row)
	...
	while ($v = DB_fetch_array($result_num)) {
        ...
        //                      }
        //   }
        if($v['transaction_type'] == '工单领料' || $v['transaction_type'] == '工单退料' || $v['transaction_type'] == '工单入库' ){
            $writer->writeSheetRow('Sheet1', array(date('Y-m-d H:i:s',$v['transaction_date']),
            $v['item_no'],$v['item_name'],$v['item_desc'],$v['uom'],$v['quantity'],round($v['price'] * ABS($v['quantity']),2 ),$v['depart_code'],$v['wip_entity_name'],$v['realname'],$v['lot_num'],$v['loccode'],$v['remark'],date('Y-m-d',$v['creation_date']),$v['trans_num'],$v['transaction_type'] ));
        }else{
            $writer->writeSheetRow('Sheet1', array(date('Y-m-d H:i:s',$v['transaction_date']),
            $v['item_no'],$v['item_name'],$v['item_desc'],$v['uom'],$v['quantity'],round($v['price'] * ABS($v['quantity']),2 ),$v['depart_code'],$v['project_name'],$v['realname'],$v['lot_num'],$v['loccode'],$v['remark'],date('Y-m-d',$v['creation_date']),$v['trans_num'],$v['transaction_type'] ));
        }
	 }
