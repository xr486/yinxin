...
$rows = array( 
  array('年月','仓库','料号','料号名称','期初数量','期初金额','入库数量','入库金额','出库数量','出库金额','期未数量','期未金额'),
...
foreach($rows as $row)
	...
	while ($v = DB_fetch_array($result_num)) {
	 ...
	 $writer->writeSheetRow('Sheet1', array($v['ym'],$v['sub_code'],$v['item_no'],$v['item_name'],$v['start_quantity'],($v['start_quantity'] * $v['cost_price']),$v['in_quantity'],($v['in_quantity'] * $v['cost_price']),$v['out_quantity'],($v['out_quantity'] * $v['cost_price']),$v['end_quantity'],($v['end_quantity'] * $v['cost_price']),
        ));
	 }
