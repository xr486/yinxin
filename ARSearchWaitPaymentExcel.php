...
$sql ="SELECT ch.need_date,ch.approve_date,ch.order_number,ch.customer_code,ve.customer_name,cl.line,cl.stockid,d.item_desc,d.item_name,cl.price,
	cl.quantity_shiped,cl.quantity,cl.check_amount,cl.payment_amount,cl.dis_payment_amount
...
$rows = array( 
  array('客户代号','订单','需求日期','订单行','料号','料号名称','规格型号','订单数量','单价','对账金额','已收款金额',
       '优惠金额','未收款金额'),
); 
...
foreach($rows as $row)
    ...
    $v_waitquantity = $v['check_amount']-$v['payment_amount']-$v['dis_payment_amount'];
     ...
     $v['check_amount'],$v['payment_amount'],$v['dis_payment_amount'],$v_waitquantity));
	 }
