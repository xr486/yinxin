...
if(isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
    ...
    $sql ="SELECT cl.tax_name,cl.yewu,cl.currency_code,cl.creation_date,cl.order_number,cl.customer_order_number,cl.customer_code,ve.customer_name,cl.order_all_amount,cl.payment_amount,cl.invoice_amount  
                    FROM  so_headers_all cl,
						   customers ve
                   where  cl.customer_code=ve.customer_code and cl.status='已签核'
				 
                   and  cl.payment_amount-cl.invoice_amount>0  
              and 1=1 " ;
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('查询已收款未开票明细') . '</p>';
...
echo '</table><div class="centre"><input type="submit" name="Search" value="查找"> &nbsp;&nbsp;
 <a href="' . $RootPath . '/ARSearchWaitinvoiceExcel.php?FromDate=' .$_POST['FromDate'] .
                        '&ToDate=' .$_POST['ToDate'] .'&customer_order_number=' .$_POST['customer_order_number'] .'&order_number=' .$_POST['order_number'] .'&stockid=' .$_POST['stockid'] .'&item_name=' .$_POST['item_name'] . '&customer_name=' .$_POST['customer_name'] .
                        '&customer_code=' .$_POST['customer_code'] .' ">' .'导出' . '</a> </div>';
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ($RowIndex <> $_SESSION['DisplayRecordsMax'])) {//$_SESSION['DisplayRecordsMax'])
			if ($k == 1) {
             ...
             $waitquantity=$myrow['payment_amount']-$myrow['invoice_amount']  ;		
		...
		echo '  <td> '.'合计'.'</td>  <td></td> <td></td><td></td><td></td><td></td><td></td>   <td> '.$v_all_check_amount.'</td>  <td>  '.$v_all_invoice_amount.'</td>    <td>' .$v_all_waitamount  . '</td> ';
		echo '</table>';
