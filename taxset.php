...
if (isset($_POST['Submit'])) {
	...
	if (ContainsIllegalCharacters($_POST['MeasureName'])) {
		$InputError = 1;
		prnMsg( _('请输入正确的税率名称') ,'error');
	}
	if (trim($_POST['MeasureName']) == '') {
		$InputError = 1;
		prnMsg( _('税率名称不能为空'), 'error');
	}
	...
	if (isset($_POST['SelectedMeasureID']) AND $_POST['SelectedMeasureID']!='' AND $InputError !=1) {
		...
		$sql = "SELECT count(*) FROM tax_set
				WHERE tax_id <> '" . $SelectedMeasureID ."'
				AND tax_name = '" . $_POST['MeasureName'] . "'";
		$result = DB_query($sql,$db);
		 
		$myrow = DB_fetch_row($result);
		if ( $myrow[0] > 0 ) {
			$InputError = 1;
			prnMsg( _('税率名称有重复，请重新输入'),'error');
		} else {
			...
			// relavant
			$sql = "SELECT tax_name,tax_mount FROM tax_set
				WHERE tax_id = '" . $SelectedMeasureID . "'";
			$result = DB_query($sql,$db);
			if ( DB_num_rows($result) != 0 ) {
				...
				$sql = array();
				$sql[] = "UPDATE tax_set
					SET tax_name='" . $_POST['MeasureName'] . "',
                    
                    tax_mount='" . $_POST['MeasureName2'] . "'
					WHERE tax_name ".LIKE." '".$OldMeasureName."'";
				$sql[] = "UPDATE tax_set
					SET tax_name='" . $_POST['MeasureName'] . "',
		...
		$sql = "SELECT count(*) FROM tax_set
				WHERE tax_name = '".$_POST['MeasureName'] ."'";
		$result = DB_query($sql,$db);
		...
		if ( $myrow[0] > 0 ) {
			...
			$sql = "INSERT INTO tax_set (tax_name,tax_mount)
...
// PREVENT DELETES IF DEPENDENT RECORDS IN 'sf_item_no'
	...
	if ( DB_num_rows($result) == 0 ) {
		// This is probably the safest way there is
		prnMsg( _('不能删除该税率名称，因为税率名称已经不存在'),'warn');
	} else {
		$myrow = DB_fetch_row($result);
		$OldMeasureName = $myrow[0];
		$sql= "SELECT COUNT(*) FROM tax_set WHERE tax_mount ".LIKE." '" . $OldMeasureName . "'";
		$result = DB_query($sql,$db);
		$myrow = DB_fetch_row($result);
		if ($myrow[0]>0) {
			prnMsg( _('不能删除这个税率，因为有属于该税率'),'warn');
			echo '<br />' . _('There are') . ' ' . $myrow[0] . ' ' . _('个属于这个税率') . '</font>';
		} else {
			$sql="DELETE FROM tax_set WHERE tax_name ".LIKE."'" . $OldMeasureName . "'";
			$result = DB_query($sql,$db);
			prnMsg( $OldMeasureName . ' ' . _('税率删除成功') . '!','success');
		}
...
/* An unit of measure could be posted when one has been edited and is being updated
  ...
  or deletion of the records*/
	...
	echo '<table class="selection">
			<tr>
				<th bgcolor="#87CEFA" class="ascending">' . _('税率名称') . '</th>
                
                <th bgcolor="#87CEFA" class="ascending">' . _('税率') . '</th>
...
if (isset($SelectedMeasureID)) {
	echo '<div class="centre">
			<a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '">' . _('查看税率') . '</a>
		</div>';
}
...
if (! isset($_GET['delete'])) {
	...
	if (isset($SelectedMeasureID)) {
		...
		$result = DB_query($sql, $db);
		if ( DB_num_rows($result) == 0 ) {
			prnMsg( _('找不到需要的税率，请重试.'),'warn');
			unset($SelectedMeasureID);
		} else {
			$myrow = DB_fetch_array($result);

			$_POST['MeasureID'] = $myrow['tax_id'];
			$_POST['MeasureName']  = $myrow['tax_name'];
            
            $_POST['MeasureName2']  = $myrow['tax_mount'];
	...
	echo '<tr>
     ...
     echo   '<tr>
		...
		<td><input required="required" class="number" pattern="(?!^ *$)[^+<>-]{1,}" type="text" name="MeasureName2" title="'._('Cannot be blank or contains illegal characters').'" placeholder="'._('请输入税率').'" size="10" maxlength="30" value="' . $_POST['MeasureName2'] . '" /></td>
		</tr>';
