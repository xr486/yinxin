...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
	...
	from fin_bank_transaction_headers_all pha, vendors c
            where  pha.transaction_type in ('AP付款','AP退款','AP预付款')
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('供应商付款管理') . '</p>';
...
<input type="submit" name="add_new" value="新增付款">&nbsp;&nbsp;
<input type="submit" name="add_fee" value="新增扣款">&nbsp;&nbsp;
<input type="submit" name="yu_fee" value="新增预付款"></div>'
 . '</br>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> 10 )) {
            ...
            unset($v_status);
            if ($myrow['transaction_type']=='AP付款') {
            echo '  <td><a href="' . $RootPath . '/APPaymentModify2.php?New=Yes&Updatepo_num=' . $myrow['transaction_num'] . '">' . $myrow['transaction_num'] . '</td> ';
			} else if ($myrow['transaction_type']=='AP退款') {
			echo '  <td><a href="' . $RootPath . '/APPaymentModify3.php?New=Yes&Updatepo_num=' . $myrow['transaction_num'] . '">' . $myrow['transaction_num'] . '</td> ';
			} else if ($myrow['transaction_type']=='AP预付款') {
			echo '  <td><a href="' . $RootPath . '/APPaymentModify4.php?New=Yes&Updatepo_num=' . $myrow['transaction_num'] . '">' . $myrow['transaction_num'] . '</td> ';
			}
...
if (isset($_POST['add_new'])) {
        header('Location: APPayment.php?New=Y');
}
if (isset($_POST['add_fee'])) {
        header('Location: APPaymentBack.php?New=Y');
}
if (isset($_POST['yu_fee'])) {
        header('Location: APPrePayment.php?New=Y');
}
