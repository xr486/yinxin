...
if(isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
    $sql = "SELECT s.transaction_num,s.transaction_amount,s.dis_amount,s.tax_amount,s.transaction_date,s.bankchangenum,s.currency_code,s.transaction_type,s.other_person,s.other_person,a.bankaccountname,a.bankaccount
    FROM
   ...
   ";
      if (isset($_POST['bankaccountname']) and $_POST['bankaccountname'] != '') { 
		$sql = $sql . " and  s.bankaccountname " . LIKE . " '%" . $_POST['bankaccountname'] . "%' ";
    }
    if (isset($_POST['transaction_date']) and $_POST['transaction_date'] != '') { 
		$sql = $sql . " and  s.transaction_date " . LIKE . " '%" . $_POST['transaction_date'] . "%' ";
    }
	if (isset($_POST['transaction_type']) and $_POST['transaction_type'] != '') { 
		$sql = $sql . " and s.transaction_type " . LIKE . " '%" . $_POST['transaction_type'] . "%' ";
    }
	if (isset($_POST['transaction_num']) and $_POST['transaction_num'] != '') { 
		$sql = $sql . " and s.transaction_num " . LIKE . " '%" . $_POST['transaction_num'] . "%' ";
    }
    if (isset($_POST['other_person']) and $_POST['other_person'] != '') {
        $sql = $sql . " and s.other_person " . LIKE . " '%" . $_POST['other_person'] . "%' ";
    }
	if (isset($_POST['bankchangenum']) and $_POST['bankchangenum'] != '') {
        $sql = $sql . " and s.bankchangenum " . LIKE . " '%" . $_POST['bankchangenum'] . "%' ";
    }
 
  
    if (empty($_POST['FromDate']) == 0) {
        $SQL_FromDate = strtotime($_POST['FromDate']);
        $sql .= " and  s.transaction_date >= '" . $SQL_FromDate . "' ";
    }
    if (empty($_POST['ToDate']) == 0) {
        $SQL_ToDate = strtotime($_POST['ToDate']) + 86400;
        //echo $SQL_ToDate;
        $sql .= " and  s.transaction_date <='" . $SQL_ToDate . "' ";
    }
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('账户流水查询报表') . '</p>';
...
echo '<td >' . _('单位/个人名称') . ':</td><td>';
echo '<input type="text" name="other_person" value="' . $_POST['other_person'] . '" size="10" maxlength="25" /></td>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
   ...
   echo '<tr>
                    ...
                    <th class="ascending"width = 120>' . _('单位/个人名称') . '</th>    
                    <th class="ascending"width = 120>' . _('银行/单据单号') . '</th>   
                    <th class="ascending"width = 50>' . _('币别') . '</th> 
