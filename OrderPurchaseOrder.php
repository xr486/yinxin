...
WHERE
	1 = 1
AND a.vendor_code = b.vendor_code AND  so_quote = 'SO'
 ";
// echo $sql;
    if (isset($_POST['SO_from']) and $_POST['SO_from'] != '') {
        $sql = $sql . " and a.po_num >=  '" . $_POST['SO_from'] . "'";
    }
    if (isset($_POST['SO_to']) and $_POST['SO_to'] != '') {
        $sql = $sql . " and a.po_num <=  '" . $_POST['SO_to'] . "' ";
    }
    ...
    // echo $sql;
    $sql .=" order by a.po_num";
    $result = DB_query($sql, $db);
    if (DB_num_rows($result) == 0) {
        unset($result);
        prnMsg(_('找不到采购单，请重新输入条件查询！'), 'error');
    }
}
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('查询订单转采购单') . '</p>';
echo '<table cellpadding="3" class="selection">';

echo '<tr><td >' . _('订单转采购单起') . ':</td><td>';
echo '<input type="text" name="SO_from" value="' . $_POST['SO_from'] . '" size="20" maxlength="25" /></td>';
echo '<td>' . _('订单转采购单止') . ':</td>
	<td>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    echo '<tr>
                    <th class="ascending" width = 100>' . _('订单转采购单号') . '</th>
                    ...
                    <th class="ascending"width = 180>' . _('订单转采购单日期') . '</th>
            </tr>';
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> 10 )) {
            ...
            echo '  <td><a href="' . $RootPath . '/OrderPurchaseOrder2.php?Updatepo_num=' . $myrow['po_num'] . '">' . $myrow['po_num'] . '</td>
				<td>' . $v_status . '</td>
...
$totalamount = $myrow['amount']+$totalamount;
            echo '
