...
and a.bom_header_id in (select max(c.bom_header_id) from  bom_headers_all c where a.assembly_item_no=c.assembly_item_no)
";
/*
  $sql4 = "select  * from so_line_temp ";
 $result4 = DB_query($sql4, $db);
 while  ($myrow4 = DB_fetch_array($result4)) { 
  $sql5 = "update so_headers_all 
      set qianding_date= '" . strtotime($myrow4['creation_date']) . "' 
	  ,delivery_date= '" . strtotime($myrow4['need_date']) . "' 
		  where order_number= '" . $myrow4['so_num'] . "' ";
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('查询当前BOM') . '</p>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> $_SESSION['DisplayRecordsMax'] )) {
            ...
            echo '  <td><a href="' . $RootPath . '/SearchBOMDetail.php?searchitem_no=' . $myrow['bom_header_id'] . '&item_name=' . $myrow['assembly_item_no'] . '"  target="_blank">' . $myrow['assembly_item_no'] . '</td>			                
                                <td>' . $myrow['status'] . '</td> 
