...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
	...
	FROM  sf_item_no b
WHERE	item_type in ('F','B')
and EXISTS (select 1 from bom_routings_all a  where a.assembly_item_no =b.item_no)
";
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('产品工艺查询') . '</p>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    echo '<div class="hier-toolbar"><span class="version-tag">产品工艺查询结果</span></div>';
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> 15 )) {
            ...
            echo ' <td><a href="' . $RootPath . '/BOMRoutesReport2.php?New=Yes&UpdateBOMItem=' . $myrow['item_id'] . '" target="_blank" >  ' . $myrow['item_no'] . ' </td>
