<?php 
	...
	if (isset($_GET['OrderNum']) or isset($_POST['OrderNum'])) {
		if (!isset($_POST['CustomerNum']) or $_POST['CustomerNum']=='') {
			...
			$sql = "select order_number,customer_number,customer_name,b.realname,order_quantity,delivery_qty,schedule_ship_date,operating_man 
			from sf_orders_all  a,www_users b where b.userid=a.sales_man_id and order_number = '".$OrderNum."'";
			$result=DB_query($sql, $db);
			while ($v=DB_Fetch_Array($result)) {
				$_POST['OrderNum'] = $v['order_number'];
				$_POST['CustomerNum'] = $v['customer_number'];
				$_POST['SalesName'] = $v['realname'];
