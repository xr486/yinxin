...
$sql = 'SELECT  pha.po_num, pha.status, pha.note, pha.creation_date,   pla.line_amount,pla.quantity,pla.line,pla.price,v.vendor_code, v.vendor_name, pha.need_date, pha.created_by,c.stockid, g.item_name,c.uom,e.realname,
ifnull(pla.quantity_received,0) this_received,pla.operation_code,pla.operation_seq_num,pla.wip_entity_name
FROM waixie_headers_all pha, waixie_lines_all pla, vendors v,wip_jobs_all b,so_lines_all c,www_users e,sf_item_no g
WHERE pla.wip_entity_name=b.wip_entity_name and  b.so_header_number=c.order_number  and c.stockid=g.item_no and  	b.so_line_number=c.line and pla.po_num = pha.po_num
...
AND v.vendor_code = pha.vendor_code ';
 if (isset($_GET['wip_entity_name']) and $_GET['wip_entity_name'] != '') {
    ...
    if (isset($_GET['item_name']) and $_GET['item_name'] != '') {
        $sql = $sql . " and g.item_name " . LIKE . " '%" . $_GET['item_name'] . "%' ";
    }
...
if ($_SESSION['price_flag'] == 'N') {
    $rows = array(
        array('供应商代码', '采购单号', '签核状态', '备注', '需求日期', '行',  '工单', '工序号', '工序名称','零件图号','零件名称', '单位', '采购单价', '采购数量', '采购金额', '收货数量',  '建立日期', '建立人员'),

    );
} else {
    $rows = array(
        array('供应商代码', '采购单号', '签核状态', '备注', '需求日期', '行', '工单', '工序号', '工序名称','零件图号','零件名称', '单位', '采购数量', '收货数量', '建立日期', '建立人员'),
...
foreach ($rows as $row)
	...
	while ($v = DB_fetch_array($result_num)) {
        ...
        date('Y-m-d ', $v['need_date']), $v['line'], 
        $v['wip_entity_name'],$v['operation_seq_num'],$v['operation_code'],$v['stockid'],$v['item_name'],
        $v['uom'],$v['quantity'],$v['this_received'],
