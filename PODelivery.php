...
if (isset($_POST['Submit'])) {
    ...
    if ($_SESSION['num' . $identifier] == 400 and $errorflag == 0 ) {
		...
		END
        ) order_number from po_rcv_transactions where substr(delivery_num,-10,8) = '" . $date . "'";
        $result_num = DB_query($sql_num, $db); 
        while ($v = DB_fetch_array($result_num)) {
            if ($v['order_number'] == null) {
                $OrderNum = 'RK'.$date . '01';
            } else {
                $OrderNum =  'RK'. $date . $v['order_number'];
            }
        }
...
//    echo $sqlinsertrcvline;
                        ...
                        $sqltrancsation = "insert into po_rcv_transactions(delivery_num,receipt_num,receipt_line,po_num,po_line,stockid,transaction_type,transaction_date,transaction_quantity,lot_num,huohao,shengchan_date,youxiaoqi,creation_date,created_by,last_update_date,last_updated_by) ";
                        $sqltrancsation.="values( '".$OrderNum."','" . $_POST['receipt_num' . $i] . "','" . $_POST['receipt_line' . $i] . "','" . $_POST['po_num' . $i]. "','" .  $_POST['po_line' . $i] . "','" . $_POST['stockid' . $i] . "','POIN','" . $time . "','" . $_POST['delivery_quantity' . $i] . "','" . $_POST['lot_num' . $i] . "','" . $_POST['huohao' . $i] . "','" . strtotime($_POST['shengchan_date' . $i]) . "','".$_POST['youxiaoqi' . $i]."','" . $time . "','" . $_SESSION['UserID'] . "','" . $time . "','" . $_SESSION['UserID'] . "')";
                        ...
                        $sqlinvtrancsation="insert into inv_transactions_all_temp(status,subinventory_from,transaction_type,
    ...
    $sqlinvtrancsation.="values('开始','".$_POST['subinventory_code' . $i]."','POIN','".$OrderNum."',
        ...
        if ($count != 0) {
            if ($InputError == 1) {
                ...
                $NUM=$_POST['NUM'];
                 ...
                 echo '<br /><div class="centre"><a href="' . $RootPath . '/PrintInPODelivery.php?Updatedelivery_num='.$OrderNum .'" target="_blank"  >' . _('打印') . '</a></div>';
...
rl.reject_area_quantity,b.price ,rl.lot_num,c.project_name,c.sub_code,a.tax_flag,a.tax_rate,b.line_amount
...
//echo '<div style="width:1300px;height:400px;overflow-x: hidden; overflow-y: scroll;">';
        ...
        while ($myrow = DB_fetch_array($TransResult)) {
            ...
            if($myrow['tax_flag'] == 'Y'){
                ...
                echo '<td>' . round($myrow['price']*(1+$myrow['tax_rate']),9) . ' </td>';
            }
            //未税单价
            if($myrow['tax_flag'] == 'Y'){
                echo '<td>' . round($myrow['price']/(1+$myrow['tax_rate']),9) . ' </td>';
            ...
            if($myrow['tax_flag'] == 'Y'){
                ...
                echo '<td>' . round($myrow['line_amount']*(1+$myrow['tax_rate']),2) . ' </td>';
            }
             //未税金额
             if($myrow['tax_flag'] == 'Y'){
                echo '<td>' . round($myrow['line_amount']/(1+$myrow['tax_rate']),2) . ' </td>';
            ...
            echo'<td><input type="text"  id="shengchan_date' . $myrow['receipt_line_id'] . '"  name="shengchan_date' . $myrow['receipt_line_id'] . '" size="10" maxlength="100"  value="' . $myrow['shengchan_date'] . '"   onfocus="WdatePicker() " /></td>';
            echo '<td>' . $myrow['youxiaoqi'] . ' </td>';
            echo '<td>' . $myrow['huohao'] . ' </td>';

            echo '<td><select name="subinventory_code' . $myrow['receipt_line_id'] . '" id="text_slect_sub' . $myrow['receipt_line_id'] . '">';
         
                $sql = "select loccode from locations order by loccode";
                $result = DB_query($sql,$db);
                while ($v = DB_fetch_array($result)) {
                    if ($v['loccode']==$myrow['sub_code']) {
            
                        echo ' <option value="' . $v['loccode'] . '" selected="selected">' . $v['loccode'] . '</option>';
            }else{
                echo '<option value="' . $v['loccode'] . '">' . $v['loccode'] . '</option>';
