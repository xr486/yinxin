...
if(isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
    ...
    $sql =" SELECT  vendor_name,ch.paymentNum,
											...
											,c2.creation_date,c2.paymentNum chongxiao_paymentnum,
											c2.paymentAmount chongxiao_amount,
											c2.taxAmount chongxiao_taxamount
                    FROM  ap_payment_headers_all ch , 
					ap_payment_headers_all c2 , 
					  vendors a
                   where  a.vendor_code=ch.vendor_code
				   and c2.chong_prepaymentnum=ch.paymentNum
				   and ch.vendor_code=c2.vendor_code
				   and ch.payment_type='预付款' " ;
  
if (empty($_POST['FromDate'])==0) {
    $SQL_FromDate = strtotime( $_POST['FromDate']);
    //echo $SQL_FromDate;
    $sql .= " and  ch.paymentDate >= '" . $SQL_FromDate . "' ";
}
if (empty($_POST['ToDate'])==0) {
    $SQL_ToDate = strtotime( $_POST['ToDate']) + 86400;
     //echo $SQL_ToDate;
    $sql .= " and  ch.paymentDate <='" . $SQL_ToDate . "' ";
}
if(isset($_POST['vendor_code']) and $_POST['vendor_code'] != ''){
  ...
  $sql = $sql." order by ch.paymentDate ";
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ($RowIndex <> 10)) {//$_SESSION['DisplayRecordsMax'])
        	...
        	echo ' <td>' . $myrow['paymentNum'] . '</td> 				 
				...
				<td  >' . $myrow['chongxiao_paymentnum'] . '</td>
