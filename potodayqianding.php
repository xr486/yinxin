...
if (isset($_POST['Search']) or isset($_POST['Go']) or isset($_POST['Next']) or
    ...
    $sql = 'SELECT DISTINCT pha.po_num, 
            ...
            and pha.approve_date >=' . $SQL_From1Date . '
		   and   pha.approve_date <=' . $SQL_To1Date . ' ';
    ...
    if (isset($_POST['stockid']) and $_POST['stockid'] != '') {
        $sql = $sql . " and pla.stockid =  '" . $_POST['stockid'] . "'";
    }
...
if (isset($_POST['Search']) and isset($result) or isset($_POST['Go']) or isset($_POST['Next']) or
    ...
    echo '<tr>
                    ...
                    <th class="ascending"width = 120>' . _('总额') .
        '</th>                           
                    <th class="ascending"width = 250>' . _('备注') . '</th>
                    <th class="ascending"width = 90>' . _('需求日') . '</th>
                    <th class="ascending"width = 160>' . _('下单日期') . '</th>
                     <th class="ascending"width = 70>' . _('下单人员') . '</th>
            </tr>';
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) and ($RowIndex <> 10)) {
            ...
            echo '  <td><a href="' . $RootPath . '/SearchPO2.php?Updatepo_num=' . $myrow['po_num'] .'" target="_blank" >' . $myrow['po_num'] . '</td>
				...
				<td><a target="_blank" href="' . $RootPath . '/PrintPOQuote.php?Updatedelivery_num=' . $myrow['po_num'] .
                '">打印</td> ';
