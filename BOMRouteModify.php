...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    $sql = "SELECT   b.item_id,b.item_no,b.item_name,b.item_desc,b.creation_date,b.created_by,(select count(*)
	 from bom_routings_all a where  a.assembly_item_no =b.item_no) route_count ,(SELECT distinct route_status from bom_routings_all where assembly_item_no =b.item_no ) route_status
	FROM  sf_item_no b
WHERE	item_type in ('F','B') and b.item_no in (select assembly_item_no from bom_routings_all) 
";
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('产品工艺修改') . '</p>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> 15 )) {
           ...
           if ( $myrow['route_count']==0 ) {
		      echo '<td><a href="' . $RootPath . '/BOMRouteModify2.php?New=Yes&UpdateBOMItem=' . $myrow['item_id'] . '" target="_blank" >新增</td> ';
		   } else {
		      echo '<td><a href="' . $RootPath . '/BOMRouteModify2.php?New=Yes&UpdateBOMItem=' . $myrow['item_id'] . '" target="_blank" >修改</td> ';
		   }
