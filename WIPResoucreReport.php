...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    $sql = " select  a.so_header_number,a.so_line_number,c.item_no,c.item_desc,c.item_name,a.wip_entity_name, b.transaction_date,b.op_code,b.line_code,b.transaction_quantity,b.transaction_uom
						from wip_jobs_all a,wip_resoure_transactions b,sf_item_no c 
                        where b.wip_entity_name =a.wip_entity_name and a.primary_item=c.item_no ";
	...
	if (isset($_POST['op_code']) and $_POST['op_code'] != '') {
        $sql = $sql . " and b.op_code " . LIKE . " '%" . $_POST['op_code'] . "%' ";
    }
	if (isset($_POST['line_code']) and $_POST['line_code'] != '') {
        $sql = $sql . " and b.line_code " . LIKE . " '%" . $_POST['line_code'] . "%' ";
    }
   
    if (empty($_POST['FromDate']) == 0) {
        $SQL_FromDate = strtotime($_POST['FromDate']);
        $sql .= " and b.transaction_date >= '" . $SQL_FromDate . "' ";
    }
    if (empty($_POST['ToDate']) == 0) {
        $SQL_ToDate = strtotime($_POST['ToDate']) + 86400;
        //echo $SQL_ToDate;
        $sql .= " and b.transaction_date <='" . $SQL_ToDate . "' ";
    }
...
echo '<div class="text-nav-1"><div>' . _('工单') . ':</div>';
echo '<input type="text" name="wip_entity_name" value="'. $_POST['wip_entity_name'] . '" size="25" maxlength="250" /></div>';
...
echo '<div class="text-nav-1"><div>' . _('工序') . ':</div>';
echo '<input type="text" name="op_code" value="' . $_POST['op_code'] . '" size="25" maxlength="250" /></div>';
echo '<div class="text-nav-1"><div>'. _('线别') . ':</div>';
echo '<input type="text" name="line_code" value="' . $_POST['line_code'] . '" size="25" maxlength="250" /></div>';
...
echo '<input type="text" onfocus="WdatePicker()" alt="' . $_SESSION['DefaultDateFormat'] . '" name="FromDate" maxlength="10" size="20" /></div>';

echo '<div class="text-nav-1"><div>' . _('生产日期止') . ':</div>';
echo '<input type="text" onfocus="WdatePicker()" alt="' . $_SESSION['DefaultDateFormat'] . '" name="ToDate" maxlength="10" size="20" /> </div>';;
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    echo '<tr>       <th bgcolor="#87CEFA"    >' . _('工单号') . '</th> 
                    <th bgcolor="#87CEFA"  width = 100>' . _('产品料号') . '</th>
					 <th bgcolor="#87CEFA"   >' . _('料号名称') . '</th>
					 <th bgcolor="#87CEFA"  >' . _('规格型号') . '</th>
                    <th bgcolor="#87CEFA"   width = 90>' . _('生产日期') . '</th>
                    <th bgcolor="#87CEFA"  width = 70>' . _('工序') . '</th> 
                    <th bgcolor="#87CEFA" width = 90>' . _('线别') . '</th>
					<th bgcolor="#87CEFA"  width = 70>' . _('工时') . '</th> 
					<th bgcolor="#87CEFA"  width = 70>' . _('单位') . '</th> 
            </tr>';
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> $_SESSION['DisplayRecordsMax'] )) {
			...
			$wait=$myrow['required_quantity']-$myrow['quantity_issued'] ;
           echo '
		   ...
		   <td>'. $myrow['wip_entity_name'] . '</td> 
                   ...
                   <td>' . date('Y-m-d', $myrow['transaction_date']) . '</td>
					<td>' . $myrow['op_code'] . '</td> 
					<td>' . $myrow['line_code'] . '</td> 
					<td>' . $myrow['transaction_quantity'] . '</td> 
					<td>' . $myrow['transaction_uom'] . '</td>   
                                 ';
