...
$Title = _('MRP需求及供给查询报表');
$ViewTopic = 'MRP需求及供给查询报表';
$BookMark = 'MRP需求及供给查询报表';
...
if(isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
    ...
    $sql ="SELECT a.item_no,a.item_desc,a.item_name,a.units,b.mrpdemandtype,b.quantity,b.demanddate,b.demandtype,b.demandorder,b.demandorder_line
	FROM mrpdemands b,sf_item_no a 
	   WHERE a.item_no = b.stockid
        AND  demandtype<>'PLANMO'  " ;
...
if (isset($_POST['Stockid_to']) and $_POST['Stockid_to'] != '') {
	 ...
	 if (isset($_POST['demandorder']) and $_POST['demandorder'] != '') {
        $sql = $sql . " and b.demandorder " . LIKE . " '%" . $_POST['demandorder'] . "%' ";
    }


  $sql = $sql." order by demanddate ";
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('MRP需求及供给查询报表') . '</p>';
...
echo ' <td>' . _('需求单号') . ':</td>
		<td>';
echo '<input type="text" name="demandorder"   value="' . $_POST['demandorder'] . '" size="20" maxlength="25" /></td>';
echo '</tr>';
 /*
echo '<td>' . _('From') . ':</td>
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ($RowIndex <> 10)) {//$_SESSION['DisplayRecordsMax'])
        	...
        	echo ' <td>' . $myrow['demandtype'] . '</td>
			   <td>' . $myrow['demandorder'] . '</td>
			   <td>' . $myrow['demandorder_line'] . '</td>
			   <td>' . $myrow['item_no'] . '</td>
			   ...
			   <td>' . $demanddate . '</td>
				...
				<td><a href="' . $RootPath . '/MRPDemanddetail.php?demandorder=' . $myrow['demandorder'] .'&demandorder_line=' . $myrow['demandorder_line'] . '" target="_blank">查询</td>
