...
if(isset($_POST['wip_entity_name']) and $_POST['wip_entity_name'] != ''){ 
   ...
   if(isset($_POST['pr_num_from']) and $_POST['pr_num_from'] != ''){ 
	...
	if(isset($_POST['item_desc']) and $_POST['item_desc'] != ''){ 
        $sql = $sql." and s.item_desc ".LIKE." '%".$_POST['item_desc']."%' "; 
    }  
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('查找未转采购明细') . '</p>';
echo '<table cellpadding="3" class="selection">';
echo '<div class="text-nav">';
echo '<div class="text-nav-1"><div>' . _('请购单') . ':</div>';
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ($RowIndex <> $_SESSION['DisplayRecordsMax'])) {
			...
			//end of page full new headings if
                ...
                echo '</table>
                <a href="' . $RootPath . '/PrWaitToPoDetailReportExcel.php?item_desc=' . $_POST['item_desc'] .'&wip_entity_name=' . $_POST['wip_entity_name'] .  '&pr_num_from=' . $_POST['pr_num_from'] . '&depart_name=' . $_POST['depart_name'] . '&item_no=' . $_POST['item_no'] . '&FromDate=' . $_POST['FromDate'] . '&item_name=' . $_POST['item_name'] . '&ToDate=' . $_POST['ToDate'] . '">' . '资料导出Excel表' . '</a>
