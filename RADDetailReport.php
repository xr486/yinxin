...
AND s.rd_help='Y'
AND ( rd_help_date=0 or rd_help_date='' or rd_help_date is null )
";
 $sql .=" order by s.order_number DESC";
...
echo '<td>' . _('订单状态') . ':</td><td><select name="checkresult">';
...
if (  isset($result) AND ( isset($result) OR isset($_POST['Search']) OR  isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous']) ) ) {
    ...
    if (DB_num_rows($result) <> 0) {
		...
		//	<td><a href="' . $RootPath . '/PrintSo.php?OrderNum=' .$myrow['order_number'] . '">' . '打印' .'<img src="%s/css/' . $Theme . '/images/pdf.png" title="' . _('Click for PDF') . '" alt="" /></a></td> 
            echo '  
                <td><a href="' . $RootPath . '/SearchQuote2.php?Updateorder_number=' . $myrow['order_number'] . '" target="view_window">' . $myrow['order_number'] . '</a></td> 
                <td>' . $myrow['line'] . '</td> 
