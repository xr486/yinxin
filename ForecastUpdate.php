...
WHERE a.customer_code = b.customer_code ";
    ...
    if (DB_num_rows($result) == 0) {
        unset($result);
        prnMsg(_('找不到该预测计划单，请重新输入条件查询！'), 'error');
    }
}
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('查询预测计划单') . '</p>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> 10 )) {
            ...
            echo '  <td><a href="' . $RootPath . '/ForecastUpdate2.php?New=Yes&Updateorder_number=' . $myrow['order_number'] . '">' . $myrow['order_number'] . '</td>
				<td>' . $myrow['status'] . '</td>
