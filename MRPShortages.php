...
$_SESSION['DisplayRecordsMax']=100;
unset($result);
...
if(isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
    ...
    $sql ="SELECT b.already_order_flag,a.item_no,a.item_desc,a.item_name,a.units,b.orderno,b.ordernoline,b.supplydate,b.book_order_date,b.demanddate,b.supplyquantity
...
if (empty($_POST['FromDate'])==0) {
    ...
    //echo $SQL_FromDate;
    $sql .= " and  demanddate >= '" . $SQL_FromDate . "' ";
}
if (empty($_POST['ToDate'])==0) {
    $SQL_ToDate = strtotime( $_POST['ToDate']) + 86400;
     //echo $SQL_ToDate;
    $sql .= " and  demanddate <='" . $SQL_ToDate . "' ";
}
...
if (isset($_POST['orderno']) and $_POST['orderno'] != '') {
        $sql = $sql . " and b.orderno =  '" . $_POST['orderno'] . "' ";
    }
	if (isset($_POST['already_order_flag']) and $_POST['already_order_flag'] != '') {
        $sql = $sql . " and b.already_order_flag =  '" . $_POST['already_order_flag'] . "' ";
    }
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('MRP缺料报表') . '</p>';
...
echo ' <td>' . _('是否下单(Y/N)') . ':</td> <td>';
echo '<input type="text" name="already_order_flag"     value="' . $_POST['already_order_flag'] . '" size="5" maxlength="35" /></td>';
echo '</tr>';
echo '</tr>';
echo '  <td>' . _('建议下单日从') . ':</td>
		<td>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
    ...
    $ListPageMax = ceil($ListCount / 100);//$_SESSION['DisplayRecordsMax']
	...
	$v_waitamount = 0; 
    if (DB_num_rows($result) <> 0) {
        DB_data_seek($result, ($_POST['PageOffset'] - 1) * 100);//$_SESSION['DisplayRecordsMax']
        $i = 0; //counter for input controls
        while (($myrow = DB_fetch_array($result)) AND ($RowIndex <> 100)) {//$_SESSION['DisplayRecordsMax'])
			if ($k == 1) {
        	...
        	echo '
			   <td>' . $myrow['orderno'] . '</td>
			   <td>' . $myrow['ordernoline'] . '</td>
			   <td>' . $myrow['item_no'] . '</td>
			   <td>' . $myrow['item_name'] . '</td>
			   <td>' . $myrow['item_desc'] . '</td>
			   <td>' . $myrow['units'] . '</td>
			   <td>' . date('Y-m-d',$myrow['demanddate']) . '</td>
			   <td>' . date('Y-m-d',$myrow['book_order_date']) . '</td>
			   <td>' . date('Y-m-d',$myrow['supplydate']) . '</td>
				<td>' . $myrow['supplyquantity'] . '</td>	
				<td>' . $myrow['already_order_flag'] . '</td>	
