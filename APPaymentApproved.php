...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
	...
	from fin_bank_transaction_headers_all pha, vendors c
            where pha.status='建立'
			and pha.transaction_type IN ('AP付款','AP预付款','AP退款')
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('供应商付款签核') . '</p>';
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> 10 )) {
           ...
           if ($myrow['transaction_type']=='AP付款') {
            echo '  <td><a href="' . $RootPath . '/APPaymentApproved2.php?New=Yes&Updatepo_num=' . $myrow['transaction_num'] . '">' . $myrow['transaction_num'] . '</td>'; }
