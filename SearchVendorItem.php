...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('查找供应商料号') . '</p>';
echo '<table cellpadding="3" class="selection">';
echo '<div class="text-nav">';
echo '<div class="text-nav-1 "><div>' . _('供应商全称') . ':</div>';
...
echo '</table><div class="centre"><input type="submit" name="Search" value="查找"> &nbsp;&nbsp;<input type="submit" name="add_new" value="新增"> &nbsp;&nbsp;<a href="' . $RootPath . '/SearchVendorItemUpload.php">批量上传</a> &nbsp;&nbsp;<a href="' . $RootPath . '/SearchVendorItemExcel.php?VendorCode=' .$_POST['VendorCode'] .'&VendorName=' .$_POST['VendorName'] .'&item_no=' .$_POST['item_no'] .'&item_name=' .$_POST['item_name'] .'&item_desc=' .$_POST['item_desc'] .'&enable_flag=' .$_POST['enable_flag'] .'">导出Excel</a></div>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
    ...
    echo '<tr>
                    <th bgcolor="#87CEFA" class="ascending" width = 150>' . _('供应商简称') . '</th>
                    <th bgcolor="#87CEFA" class="ascending"width = 250>' . _('供应商全称') . '</th>
        ...
        while (($myrow = DB_fetch_array($result)) AND ($RowIndex <> $_SESSION['DisplayRecordsMax'])) {
			...
			echo '  <td> ' . $myrow['vendor_code'] . '</td>
				...
				<td>' . $myrow['item_desc'] . '</td>
					...
					<td><a href="' . $RootPath . '/SearchVendorItem2.php?Updateitem_relation_id=' . $myrow['item_relation_id'] . '">修改</td>
				';
