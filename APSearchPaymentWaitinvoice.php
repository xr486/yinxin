...
WHERE    c.payment_amount > c.invoice_amount
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('查找已付款未开票明细') . '</p>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
    ...
    if (DB_num_rows($result) <> 0) {
		...
		echo '  <td> '.'合计'.'</td>  <td></td>  <td></td> <td></td><td></td>     <td>  '.$v_all_invoice_amount.'</td> <td>' .$v_all_waitamount  . '</td> ';
		echo '</table>';
