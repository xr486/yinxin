...
vendors d,waixie_lines_all e,so_lines_all f,sf_item_no g
where a.receipt_num = b.receipt_num  and f.stockid=g.item_no
and b.vendor_code = d.vendor_code 	and a.po_line=e.line and a.po_num=e.po_num and e.wip_entity_name=f.wip_entity_name 
and a.check_flag ='N'  	    
";
...
$rows = array( 
  array('入库日期', '类型', '采购入库单', '项', '供应商', '外协采购单号' , '行' ,'零件图号','零件名称', '单位' ,'入库数量', '采购单价', '入库金额',),
...
foreach($rows as $row)
	...
	while ($v = DB_fetch_array($result_num)) {
	 ...
	 $v['vendor_code'],$v['po_num'],$v['po_line'],$v['stockid'],$v['item_name'],
     $v['uom'],$v['transaction_quantity'],$v['unit_price'],$check_amount, ));
	 }
