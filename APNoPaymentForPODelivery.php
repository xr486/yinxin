...
$sql ="SELECT a.vendor_code,b.vendor_name,a.*,c.delivery_date
       ...
       WHERE  a.vendor_code=b.vendor_code
         ...
         AND  a.check_amount > a.payment_amount + a.payment_dis_amount  " ;
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('已采购入库未付款明细') . '</p>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
    ...
    echo '<tr>      <th width="100">供应商代码</th>
	                <th width="250">供应商名称</th>
                    <th width="150">采购入库单号</th>
                    <th width="40">入库项目</th>
					<th width="150">入库备注</th>   
					<th width="80" >收货日</th>
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ($RowIndex <> 10)) {//$_SESSION['DisplayRecordsMax'])
			if ($k == 1) {
			  ...
			  $v_wait_payment_amount=$myrow['check_amount']-$myrow['payment_amount'] - $myrow['payment_dis_amount'];
         ...
         echo ' <td>' . $myrow['vendor_code'] . '</td> 
			    ...
			    <td class="number">'  .$myrow['payment_amount'] . '</td>
				<td class="number">'  .$myrow['payment_dis_amount'] . '</td> 
				<td class="number">'  .$v_wait_payment_amount. '</td>   
