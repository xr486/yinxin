...
if (isset($_POST['Submit'])) {
	...
	if (isset($_POST['SelectedMeasureID']) AND $_POST['SelectedMeasureID']!='' AND $InputError !=1) {
		...
		$sql = "SELECT count(*) FROM sf_item_shidu
				WHERE shiduid <> '" . $SelectedMeasureID ."'
				AND shidu ".LIKE." '" . $_POST['MeasureName'] . "'";
		$result = DB_query($sql,$db);
		$myrow = DB_fetch_row($result);
		if ( $myrow[0] > 0 ) {
			$InputError = 1;
			prnMsg( _('湿度有重复，请重新输入'),'error');
		} else {
			...
			// relavant
			$sql = "SELECT shidu FROM sf_item_shidu WHERE shiduid = '" . $SelectedMeasureID . "'";
			$result = DB_query($sql,$db);
			if ( DB_num_rows($result) != 0 ) {
				...
				$sql = array();
				$sql[] = "UPDATE sf_item_shidu SET shidu='" . $_POST['MeasureName'] . "' WHERE shidu ".LIKE." '".$OldMeasureName."'";
				 
			} else {
				$InputError = 1;
				prnMsg( _('湿度不存在.'),'error');
			}
		}
		$msg = _('湿度已经更新');
	} elseif ($InputError !=1) {
		...
		$sql = "SELECT count(*) FROM sf_item_shidu
				WHERE shidu " .LIKE. " '".$_POST['MeasureName'] ."'";
		$result = DB_query($sql,$db);
		$myrow = DB_fetch_row($result);
		if ( $myrow[0] > 0 ) {
			$InputError = 1;
			...
			$sql = "INSERT INTO sf_item_shidu (shidu )
...
// PREVENT DELETES IF DEPENDENT RECORDS IN 'sf_item_no'
	...
	if ( DB_num_rows($result) == 0 ) {
		// This is probably the safest way there is
		prnMsg( _('不能删除该湿度，因为湿度已经不存在'),'warn');
	} else {
		$myrow = DB_fetch_row($result);
		$OldMeasureName = $myrow[0];
		 
			$sql="DELETE FROM sf_item_shidu WHERE shidu ".LIKE."'" . $OldMeasureName . "'";
			$result = DB_query($sql,$db);
...
/* An unit of measure could be posted when one has been edited and is being updated
  ...
  or deletion of the records*/
	...
	echo '<table class="selection">
			<tr>
				<th bgcolor="#87CEFA" class="ascending">' . _('湿度') . '</th>
			</tr>';
...
if (isset($SelectedMeasureID)) {
	echo '<div class="centre">
			<a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '">' . _('查看湿度') . '</a>
		</div>';
}
...
if (! isset($_GET['delete'])) {
	...
	if (isset($SelectedMeasureID)) {
		...
		$result = DB_query($sql, $db);
		if ( DB_num_rows($result) == 0 ) {
			prnMsg( _('找不到需要的湿度，请重试.'),'warn');
			unset($SelectedMeasureID);
		} else {
			$myrow = DB_fetch_array($result);

			$_POST['MeasureID'] = $myrow['shiduid'];
			$_POST['MeasureName']  = $myrow['shidu'];
