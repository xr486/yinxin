...
$Title = _('其它收入/支出待主管签核');
$ViewTopic = '其它收入/支出待主管签核';
$BookMark = '其它收入/支出待主管签核';
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('其它收入/支出待主管签核') . '</p>';
...
echo '<td>' . _('单位/个人名称 ') . ':</td>
	<td>';
echo '<input type="text" name="other_person" value="' . $_POST['other_person'] . '" size="20" maxlength="25" /></td>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> 10 )) {
            ...
            echo '  <td><a href="' . $RootPath . '/ExpInWaitApproved2.php?New=Yes&Updatepo_num=' . $myrow['transaction_num'] . '"target="_blank" >' . $myrow['transaction_num'] . '</td>
