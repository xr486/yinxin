...
$sql = "SELECT a.creation_date,a.subinventory_from,
...
if (empty($_GET['FromDate']) == 0) 
{
  ...
  $sql .= " and aa.creation_date < '" . $SQL_FromDate . "' ";
}
 $sql = $sql." order by aa.creation_date desc limit 1 ) qichu_after_onhand, 
...
$sql = $sql." order by aa.creation_date desc limit 1 ) qichu_after_amount, 
(SELECT ifnull(after_onhand,0) from inv_transactions_all aa where a.item_no = aa.item_no and a.subinventory_from = aa.subinventory_from";
if (empty($_GET['ToDate']) == 0) 
{
  $SQL_ToDate = strtotime($_GET['ToDate']) + 86400;
  $sql .= " and aa.creation_date < '" . $SQL_ToDate . "' ";
}
 $sql = $sql." order by aa.creation_date desc limit 1 ) qimo_after_onhand, 
(SELECT ifnull(after_amount,0) from inv_transactions_all aa where a.item_no = aa.item_no and a.subinventory_from = aa.subinventory_from";
if (empty($_GET['ToDate']) == 0) 
{
  $SQL_ToDate = strtotime($_GET['ToDate']) + 86400;
...
$sql = $sql." order by aa.creation_date desc limit 1 ) qimo_after_amount, 
...
$sql = $sql . " order by a.creation_date,c.item_no, c.item_name";
...
foreach($rows as $row)
	...
	while ($v = DB_fetch_array($result_num)) {
	 ...
	 $writer->writeSheetRow('Sheet1', array(date('Y-m-d',$v['creation_date']),$v['subinventory_from'],$v['item_no'],$v['item_name'],$v['qichu_after_onhand'],round($v['qichu_after_amount'],2),$v['in_quantity'],round($v['in_amount'],2),ABS($v['out_quantity']),ABS(round($v['out_amount'],2)),$v['qimo_after_onhand'],round($v['qimo_after_amount'],2)
        ));
	 }
