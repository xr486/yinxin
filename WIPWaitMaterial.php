...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    $sql = " select d.item_no mitem_no,d.item_name, mitem_name,a.plan_start_date,a.so_header_number,a.so_line_number,b.segment1,c.item_desc,c.units,c.item_name,b.comments,quantity_issued,b.required_quantity,
	...
	and j.status_type='开始' ) chaofa_qty,(select sum(cc.quantity)  from inv_onhand_quantity_all cc where cc.stockid=c.item_no  ) onhand_quantity
    ...
    //and b.required_quantity > quantity_issued
    ...
    if (empty($_POST['ToDate']) == 0) {
      ...
      $sql .= " order by a.plan_start_date,a.wip_entity_name,a.plan_start_date,b.operation_seq_num ,b.segment1"; 
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
	 ...
	 <th bgcolor="#87CEFA"    >' . _('产品料号') . '</th> 
	  <th bgcolor="#87CEFA"    >' . _('产品名称') . '</th> 
                    ...
                    <th bgcolor="#87CEFA"  width = 100>' . _('料号') . '</th>
					 ...
					 <th bgcolor="#87CEFA"  >' . _('数量') . '</th>
					 <th bgcolor="#87CEFA"  >' . _('阶层') . '</th>
					 <th bgcolor="#87CEFA"  >' . _('单位') . '</th>
                    <th bgcolor="#87CEFA"  width = 70>' . _('需求量') . '</th> 
                    <th bgcolor="#87CEFA"  width = 70>' . _('库存量') . '</th> 
                    <th bgcolor="#87CEFA"  width = 70>' . _('超发量') . '</th> 
                    <th bgcolor="#87CEFA" width = 90>' . _('已发量') . '</th>
					<th bgcolor="#87CEFA"  width = 70>' . _('欠料量') . '</th>
					<th bgcolor="#87CEFA"  width = 70>' . _('备注') . '</th>  
            </tr>';
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> $_SESSION['DisplayRecordsMax'] )) {
			...
			if ($myrow['required_quantity'] > $myrow['quantity_issued']) {
    ...
    if (isset($ListPageMax) AND $ListPageMax > 1) {
	 ...
	 echo '<br>
          <div class="centre">
                <a href="' . $RootPath . '/WIPWaitMaterialExcel.php?wip_entity_name=' .$_POST['wip_entity_name'] . '&item_no=' .$_POST['item_no'].'&item_name=' .$_POST['item_name'] .'&item_desc=' .$_POST['item_desc'] .'&FromDate=' .$_POST['FromDate'] .'&ToDate=' .$_POST['ToDate'] . '">
                ' .'资料导出Excel表' . '
                </a>
          </div>';
}
