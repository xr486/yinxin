...
WHERE    c.po_all_amount > c.payment_amount 
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('查找未付款明细') . '</p>';
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ($RowIndex <> $_SESSION['DisplayRecordsMax'])) {//$_SESSION['DisplayRecordsMax'])
			if ($k == 1) {
             ...
             $waitquantity=$myrow['po_all_amount']-$myrow['payment_amount'] ;		
		...
		echo '  <td> '.'合计'.'</td>    <td></td> <td></td>     <td>  '.$v_all_invoice_amount.'</td> <td>' .$v_all_waitamount  . '</td> ';
		echo '</table>';
