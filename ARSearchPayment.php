...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
	...
	from fin_bank_transaction_headers_all pha, customers c
            where pha.status='核准'
			and pha.transaction_type in ('AR收款','AR退款','AR预收款')
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        echo '<tr><td>总计</td><td>笔数</td><td>' . $all_line . '</td><td></td><td></td><td>' . $all_transaction_amount . '</td><td>' . $all_dis_amount . '</td></tr>';
		echo '</table></div>';
        echo '<input type="hidden" name="JustSelectedAvendor" value="Yes" />';
        echo '<div>
                    <a href="' . $RootPath . '/ARSearchPaymentExcel.php?
