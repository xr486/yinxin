...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
	...
	from fin_bank_transaction_headers_all pha, customers c
            ...
            and  pha.transaction_date >='" . $SQL_From1Date . "'
            and  pha.transaction_date <='" . $SQL_To1Date . "'"
            ;
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('客户收款明细查询') . '</p>';
...
echo '<td>' . _('客户代码') . ':</td>
	<td>';
echo '<input type="text" name="customer_code" value="' . $_POST['customer_code'] . '" size="20" maxlength="25" /></td></tr>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    echo '<tr>
                    ...
                    <th width = 80>' . _('客户') . '</th>
                    <th class="ascending"width = 200>' . _('客户') . '</th>
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> 10 )) {
            ...
            echo '  <td><a target="_blank"  href="' . $RootPath . '/ARSearchPayment2.php?New=Yes&Updatepo_num=' . $myrow['transaction_num'] . '">' . $myrow['transaction_num'] . '  </td>
