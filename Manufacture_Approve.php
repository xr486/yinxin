...
and EXISTS(select 1 from sf_order_lines_all c
                   where  a.order_number = c.order_number 
                     and (ifnull(bumanufacture_flag,'N') = 'A' or ifnull(shamanufacture_flag,'N') = 'A'))
";
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> 10 )) {
            ...
            echo '  <td><a href="' . $RootPath . '/Manufacture_Approve2.php?Updateso_num=' . $myrow['order_number'] . '">' . $myrow['order_number'] . '</td>
				<td>' . $v_status . '</td>
