...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    $sql = "select pha.invoice_name,ap_invoice_type,invoice_type,
			   ...
			   pha.invoice_amount,
				...
				pha.currency_code,
                 pha.invoice_date,
                pha.creation_date,pha.created_by,pha.vendor_code,c.vendor_name,pha.status,pha.payment_amount
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('供应商发票审管理') . '</p>';
...
<input type="submit" name="add_new" value="新增采购发票">&nbsp;&nbsp;
<input type="submit" name="add_fee" value="新增费用发票">&nbsp;&nbsp;
<input type="submit" name="redfee" value="新增红字发票"></div>'
 . '</br>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    echo '<tr>
                    <th class="ascending" width = 100>' . _('发票单号') . '</th>     
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> 10 )) {
		    ...
		    echo '  <td><a href="' . $RootPath . '/APInvoiceModify3.php?Updatepo_num=' . $myrow['invoice_name'] . '">' . $myrow['invoice_name'] . '</td>';
		   } else if ($myrow['invoice_type']=='费用发票'  ) {
		    echo '  <td><a href="' . $RootPath . '/APInvoiceModify4.php?Updatepo_num=' . $myrow['invoice_name'] . '">' . $myrow['invoice_name'] . '</td>';
		   } else  {
            echo '  <td><a href="' . $RootPath . '/APInvoiceModify2.php?Updatepo_num=' . $myrow['invoice_num'] . '&vendor_code=' . $myrow['vendor_code'] . '">' . $myrow['invoice_name'] . '</td>';
		   }
...
if (isset($_POST['add_new'])) {
        header('Location: APInvoice1.php?New=Y');
}
if (isset($_POST['add_fee'])) {
        header('Location: APInvoice.php?New=Y');
}
if (isset($_POST['redfee'])) {
        header('Location: APInvoiceBack.php?New=Y');
} 
