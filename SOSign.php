...
//        }
            ...
            $msg = '审核成功！';
            prnMsg($msg, 'success');
             echo '<br /><div class="centre"><a href="' . $RootPath . '/InSOSign.php">' . _('生产主管订单审核 （生管）') . '</a></div>';
...
echo '<p class="page_title_text">
		<img src="' . $RootPath . '/css/' . $Theme . '/images/supplier.png" title="' . '生产主管订单审核 （生管）' .
 '" alt="" />' . ' ' . $Title . '
	</p>';
//
...
//    }
    ...
    $ErrMsg = _('订单单查询错误，请查看所选采购单') . ' - ' . DB_error_msg($db);
    $DbgMsg = _('The SQL that failed was');
    if (DB_num_rows($TransResult) == 0) {
        unset($TransResult);
        prnMsg(_('没有找到需要签核的订单，请重新输入条件查询！'), 'info');
    } else {
