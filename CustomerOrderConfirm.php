...
// echo '<meta http-equiv="refresh" content="0.3" url=RequestReceive.php"/>';
        ...
        unset($_POST['purchase_flag']);
        unset($_POST['invoice_flag']);
        unset($_POST['express_flag']);
        unset($_POST['urgent_flag']);
    }
} else {
...
if (isset($_POST['ShowResults'])) {
    ...
    $sql = "select order_id,
            ...
            where (order_flag is null
              or order_flag <> 'Y' or (order_flag = 'Y' and  (
...
//echo '<div style="width:1300px;height:400px;overflow-x: hidden; overflow-y: scroll;">';
        ...
        $tableheader = '<tr>
                                     ...
                                     <th  width = 100>' . '数量' . '</th>
                                     <th width = 100 >' . '金额' . '</th>
                                     <th width = 100 >' . '钢网采购' . '</th>
                                     <th width = 100 >' . '开发票' . '</th>
                                     <th width = 100 >' . '快递' . '</th>
                                     <th width = 100 >' . '是否加急' . '</th>
				</tr>';
        echo $tableheader;
...
//$ModifyPage = $RootPath . '/CustomerOrderConfirmDetail.php.php?OrderNum=' . $myrow['order_number'];
      /*      echo '<td><input type="checkbox" name="status' . $myrow['order_id'] . '" /></td>
	 	<td><font color="red">' . $myrow['order_number'] . '</font></td>';*/
?>
           <td><a href="<?=$RootPath?>/CustomerOrderConfirmDetail.php?OrderNum=<?=$myrow['order_number']?> "><?=$myrow['order_number']?></td>
	<?php	//echo '<td>' . $ModifyPage. ' </td>';
	    echo '<td>' . $myrow['customer_number'] . ' </td>';
            echo '<td>' . $myrow['customer_name'] . '</td>';            
            echo '<td>' . $myrow['realname'] . '</td>';
            echo '<td>' . $myrow['order_quantity'] . ' </td>';
            echo '<td>' . $myrow['unit_price'] * $myrow['order_quantity'] . '</td>';
