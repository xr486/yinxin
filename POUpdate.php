...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('采购单修改') . '</p>';
...
if (isset($_POST['Search']) and isset($result) or isset($_POST['Go']) or isset($_POST['Next']) or isset($_POST['Previous'])) {
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) and ($RowIndex <> $_SESSION['DisplayRecordsMax'])) {
            ...
            echo '  <td><a href="' . $RootPath . '/POUpdate2.php?New=Yes&Updatepo_num=' . $myrow['po_num'] . '">' . $myrow['po_num'] . '</td>
				<td>' .   $myrow['status']. '</td>
