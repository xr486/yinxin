...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    $sql = "SELECT distinct b.item_no,b.item_name,b.item_desc,b.creation_date,b.created_by,b.item_category1
	FROM  sf_item_no b 
WHERE	1=1
";
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('料号用途查询') . '</p>';
echo '<table cellpadding="3" class="selection">';
echo '<div class="text-nav">';
echo '<div class="text-nav-1"><div>' . _('料号编码') . ':</div>';
...
echo '<div class="text-nav-1"><div>' . '建立日' . _('起') . ':</div>
	...
	';
echo '<div class="text-nav-1"><div>' . _('生效BOM') . ':</div><select name="dangqian">';

echo '<option  selected="selected" value="' . $_POST['dangqian'] . '">' . $_POST['dangqian'] . '</option>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    echo '<tr>
                    ...
                    <th class="ascending"width = 170>' . _('建立日期') . '</th>
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> $_SESSION['DisplayRecordsMax'] )) {
			...
			if   ($_POST['dangqian'] =='否' ) {			
                 echo '  <td><a href="' . $RootPath . '/SearchWhereUseDetail.php?searchitem_no=' . $myrow['item_no'] . '" target="_blank"
			>' . $myrow['item_no'] . '</td>';
			} else {			
                 echo '  <td><a href="' . $RootPath . '/SearchWhereUseDetail2.php?searchitem_no=' . $myrow['item_no'] . '" target="_blank"
			>' . $myrow['item_no'] . '</td>';
			}
