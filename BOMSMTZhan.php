...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    $sql = "SELECT distinct a.assembly_item_no,b.item_name,b.item_desc 
	FROM bom_smt_liaozhan a,
	 sf_item_no b
WHERE	1 = 1
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('BOMSMT料站别维护') . '</p>';
...
';
echo '</div>';
echo '</table><div class="centre"><input type="submit" name="Search" value="查找"> &nbsp;&nbsp;<input type="submit" name="add_new" value="上传"></div>'
 . '</br>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> $_SESSION['DisplayRecordsMax'] )) {
            ...
            echo ' <td><a href="' . $RootPath . '/BOMSMTZhan2.php?New=Yes&UpdateBOMItem=' . $myrow['assembly_item_no'] . '" target="_blank" >修改</td> 
				<td>'   . $myrow['assembly_item_no'] . '</td> 
			
                                <td>' . $myrow['item_name'] . '</td> 
                                <td>' . $myrow['item_desc'] . '</td>  	
                               
                <td><a href="' . $RootPath . '/BOMSMTZhan3.php?New=Yes&UpdateBOMItem=' . $myrow['assembly_item_no'] . '" target="_blank">查询</td>      
				<td><a href="' . $RootPath . '/BOMSMTZhanExcel.php?item_no=' . $myrow['assembly_item_no'] . '" target="_blank">导出</td>    ';
...
if (isset($_POST['add_new'])) {
        header('Location: BOMSMTUpload.php');
}
