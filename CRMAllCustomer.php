...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('全部客户查询') . '</p>';
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ($RowIndex <> 10)) {
			...
			echo '  <td>' . $myrow['customer_code'] . '</td>
				...
				<td><a href="' . $RootPath . '/CrmAllCustomers.php?UpdateCustomerCode=' . $myrow['customer_code'] . '">' . _(查看) .'
                </td>
