...
if(isset($_GET['FromDate']) and $_GET['FromDate'] != ''){
    $sql = $sql." and a.last_update_date >=".strtotime($_GET['FromDate'])." ";
}
...
$rows = array( 
  array('交易单号', '交易类型' ,'料号' ,'料号名称', '规格型号','批号','生产日期', '交易日期', '交易数量' ,'余量' , '交易金额' ,'剩余金额' , '单位',
   '仓库', '申请人工号' ,'申请人名称' ,  '备注'  ,'建立日期','建立人'),

); 
}else{
    $rows = array( 
        array('交易单号', '交易类型' ,'料号' ,'料号名称', '规格型号','批号','生产日期', '交易日期', '交易数量' ,'余量' ,  '单位',
         '仓库', '申请人工号' ,'申请人名称' ,  '备注'  ,'建立日期','建立人'),
...
if($_SESSION['price_flag'] == 'N'){
    ...
    $v['item_no'],
    $v['item_name'],$v['item_desc'],$v['lot_num'],$v['shengchan_date'],date('Y-m-d',$v['transaction_date']),$v['quantity'],$v['after_onhand'],ABS($v['quantity']*$v['price']),round($v['after_amount'],2),$v['uom'],$v['loccode'],$v['request_person'],
    $v['employee_name'], $v['remark'],date('Y-m-d H:i:s',$v['last_update_date']), $v['realname'] ));

}else{
    $writer->writeSheetRow('Sheet1', array($v['trans_num'],$v['transaction_type'],
    $v['item_no'],
    $v['item_name'],$v['item_desc'],$v['lot_num'],$v['shengchan_date'],date('Y-m-d',$v['transaction_date']),$v['quantity'],$v['after_onhand'],$v['uom'],$v['loccode'],$v['request_person'],
    $v['employee_name'], $v['remark'],date('Y-m-d H:i:s',$v['last_update_date']), $v['realname'] ));
}
