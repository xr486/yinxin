...
if(isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
    ...
    $sql =" SELECT realname,ch.customer_code,invoicenum,invoiceamount,taxamount,invoicedate,AlreadyReceiveamount,order_number,line_no 
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('查找已立账未付款查询明细查询') . '</p>';
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ($RowIndex <> 10)) {//$_SESSION['DisplayRecordsMax'])
        	...
        	echo ' <td>' . $myrow['invoicenum'] . '</td> 					
				<td class="number">' . $myrow['order_number'] . '</td>
				<td class="number">' .$myrow['line_no'] . '</td>  
