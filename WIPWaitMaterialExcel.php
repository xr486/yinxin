...
unset($result);
 $sql = " select d.item_no mitem_no,d.item_name mitem_name,a.plan_start_date,a.so_header_number,a.so_line_number,b.segment1,c.item_desc,c.units,c.item_name,b.comments,quantity_issued,
 b.required_quantity,b.quantity_per_assembly,a.wip_entity_name,b.operation_seq_num,(select sum(quantity_issued- required_quantity) 
 ...
 and j.status_type='开始' ) chaofa_qty,(select sum(cc.quantity)  from inv_onhand_quantity_all cc where cc.stockid=c.item_no  ) onhand_quantity
                        ...
                        where b.wip_entity_name =a.wip_entity_name and 
						b.segment1=c.item_no and a.primary_item = d.item_no and a.status_type='开始'
    ...
    if (empty($_GET['ToDate']) == 0) {
	 ...
	 $sql .= " order by a.plan_start_date,a.wip_entity_name,b.operation_seq_num,b.segment1 "; 
...
$rows = array( 
  array('工单号', '产品料号','产品名称','开工日期' ,'料号' ,'料号名称', '规格型号','数量','阶层','单位', '需求量', '库存量' , '超发量','已发量','欠料量','备注' ),
...
foreach($rows as $row)
	...
	while ($v = DB_fetch_array($result_num)) {
     ...
     $v['segment1'],
     $v['item_name'],$v['item_desc'],$v['quantity_per_assembly'].' ',$v['operation_seq_num'],$v['units'],$v['required_quantity'],$v['onhand_quantity'],$v['chaofa_qty'],$v['quantity_issued'],$wait,$v['comments']  ));
	 }
