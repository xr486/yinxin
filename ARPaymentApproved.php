...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('客户收款签核') . '</p>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> 10 )) {
            ...
            echo '  <td><a href="' . $RootPath . '/ARPaymentApproved3.php?New=Yes&Updateorder_number=' . $myrow['transaction_num'] . '">' . $myrow['transaction_num'] . '</td>'; }
			else if ($myrow['transaction_type']=='AR预收款') {
			 echo '  <td><a href="' . $RootPath . '/ARPaymentApproved4.php?New=Yes&Updateorder_number=' . $myrow['transaction_num'] . '">' . $myrow['transaction_num'] . '</td>';
			} else   {
			 echo '  <td><a href="' . $RootPath . '/ARPaymentApproved2.php?New=Yes&Updateorder_number=' . $myrow['transaction_num'] . '">' . $myrow['transaction_num'] . '</td>';
			}
