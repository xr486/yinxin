...
$i = 1;
	...
	while ($myrow = DB_fetch_array($result2)) {
		...
		echo '  
					  ...
					  <td><img src=' . $myrow['file_patch'] . ' "  id="myImg' .$i.'" onclick=check(' .$i.') width="100%" height="100%"></td>
					   <td><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?file_11patch=' .$myrow['file_patch'] .'&OrderNum=' .$myrow['item_id'] .'&route_id=' .$myrow['route_id'] . '&itemid=' .$myrow['itemid'] . '&amp;delete=1" onclick="return confirm(\'' . _('是否要删除这个文件?') . '\');">' . _('删除')  . '</a></td>
  ...
  echo '<p class="page_title_text">
		<img src="' . $RootPath . '/css/' . $Theme . '/images/supplier.png" title="' . '工序指导书' .
 '" alt="" />' .'料号'. $_SESSION['item_no' . $identifier] .'工序指导书' . '
	</p>';
                    ...
                    for ($i = 0; $i < $length; $i++) {
                        if ($decoded_dest[$i]['name'] == $key) {
                            if (!$decoded_dest[$i]['dest']) {
                                ...
                                echo '<meta http-equiv="refresh" content="0; url=' . $RootPath .'/SussCreateBOM.php?assembly_item_no=' . $_POST['OrderNum'] . '&route_id=' .$_POST['route_id'] . '" />';
                            } else {
                                // echo $key . ': ' . $decoded_dest[$i]['dest'] . '<br>';
                                $sql = "insert into bom_routing_all_file (route_id,file_name,item_no,file_patch,creation_date,created_by) values ('".$_POST['route_id']."','".$value."','".$_POST['item_no']."','".$decoded_dest[$i]['dest']."','".$time."','".$_SESSION['UserID']."')";
...
<div>
	...
	<input type="hidden" name="route_id" value = "<?php echo $_SESSION['route_id' . $identifier]; ?>">
<br>
