...
$msg = '采购单编号'.$OrderNum.'建立成功！';
                prnMsg($msg, success);
                 echo '<br /><div class="centre"><a href="' . $RootPath . '/AddPurchaseOrder.php">' . _('继续创建采购单') . '</a></div>';
		...
		<img src="' . $RootPath . '/css/' . $Theme . '/images/supplier.png" title="' . '为采购单上传附件' .
 '" alt="" />' . ' ' .'为订单上传附件' . '
	</p>';
...
$_POST['ItemNo'];
	if (isset($_POST['Save'])) {
     $time = time();
		$upload=new upload('Pic','PO');
$dest=$upload->uploadFile();

$sql = "insert into po_headers_all_file (file_name,order_number,file_patch,creation_date,created_by) values ('".$_POST['file_name']."','".$_POST['OrderNum1']."','".$dest."','".$time."','".$_SESSION['UserID']."')";
...
<table class="selection">
	...
	<tr>
	<td>附件名称：</td>
  <td ><input type="text"  maxlength="200" size="20" name="file_name"  value="<?=$_POST['file_name']?>" /> </td>
		<td>上传附件：</td>
