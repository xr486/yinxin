...
$sql ="SELECT wip_entity_id,
              ...
              model_output_quantity,
              model_station,
              stockid
     from wip_jobs_all
    where  model_input_quantity > 0 and model_output_quantity + 
    ...
    if ($errorflag == 0) {
        foreach ($_POST as $key => $value){
            ...
            if (mb_substr($key,0,10)=='UpdateLine') {
                ...
                $outputCount =$_POST['model_output_quantity'.$wip_entity_id] + $_POST['goodProductCount'.$wip_entity_id];

                $inputcount = $_POST['model_input_quantity'.$wip_entity_id] - $_POST['scrapCount'.$wip_entity_id];
                ...
                $update_wip_jobs_all="UPDATE wip_jobs_all SET                        
                        model_output_quantity=".$outputCount.",
                        casting_input_quantity=".$outputCount."
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('工单造型完工处理') . '</p>';
echo '<table cellpadding="3" class="selection">';
echo '</tr>';

echo '<tr><td >' . _('预计开工时间') . ':</td><td>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
    ...
    if (DB_num_rows($result) <> 0 ) {
        ...
        while (($myrow = DB_fetch_array($result))
            ...
            echo '  <td>' . $myrow['wip_entity_name'] . '
                ...
                <!--总入站量-->
                <td>' . $myrow['model_input_quantity'] . '
                       <input type="hidden"
                              id="all" 
                              name="model_input_quantity'.$myrow['wip_entity_id'].'" 
                              value="'.$myrow['model_input_quantity'].'"></td>
                              
                <!--出站量-->          
                <td>' . $myrow['model_output_quantity'] . '
                        <input type="hidden" 
                              name="model_output_quantity'.$myrow['wip_entity_id'].'" 
                              value="'.$myrow['model_output_quantity'].'"></td>
                              
                <!--待出站量-->              
                <td id="pre_outputCount'.$myrow['wip_entity_id'].'">' .
                ($myrow['model_input_quantity'] - $myrow['model_output_quantity'] - $rejectNum).'</td>';
            ...
            echo "  <option selected='selected'>".$myrow['model_station']."</option>
                    </select>
                    </td>";
            ...
            //多种不良
            echo '<td>
                      <a href="addMoreBadReason.php?id='.$myrow['wip_entity_id'].'&reason_type=model">多选</a>
                   </td>';
            //显示报废数量
            echo '<td><input 
            ...
            //多种报废
            echo '<td>
                      <a href="addMoreScrapReason.php?id='.$myrow['wip_entity_id'].'&reason_type=model">多选</a>
                   </td>';
            ...
            echo '<input type="hidden" 
                   name="model_input_quantity'.$myrow['wip_entity_id'].'" 
                   value="'.$myrow['model_input_quantity'].'"/>
           ';
            echo '<input type="hidden" 
                   name="model_output_quantity'.$myrow['wip_entity_id'].'" 
                   value="'.$myrow['model_output_quantity'].'"/>
           ';
            echo  '
            </tr>';
            $i++;
 ...
 $(document).ready(function(){
       ...
       $('#goodProductCount".$myrow['wip_entity_id']."').blur(function(){          
             check();       
        });
       ...
       $('#badCount".$myrow['wip_entity_id']."').keyup(function(){
        ...
        $('#scrapCount".$myrow['wip_entity_id']."').blur(function(){
           check();
        });
        ...
        function checkEmployee(){
            ...
            )
            {
                ...
                // $('#text_slect_employename1".$myrow['wip_entity_id']."').attr(\"required\",\"required\");
            } else {
               $('#text_slect_employename".$myrow['wip_entity_id']."').removeAttr(\"required\");
               $('#text_slect_employename1".$myrow['wip_entity_id']."').removeAttr(\"required\");
            }
        ...
        function check(){
            ...
            var pre_outputCount=$('#pre_outputCount".$myrow['wip_entity_id']."').text();                       
        ...
        $('#selectscrapReason".$myrow['wip_entity_id']."').dialog({
            ...
            width: '950px',
            height: '470px',
            content:'url:btnSearchScapReasons.php?fwValue=" .$myrow['wip_entity_id']."&type=model',
            init:function(){
                 this.content.document.getElementById('cat').value = 'buliao';
                this.content.document.getElementById('fwValue').value = '".$myrow['wip_entity_id']."';
            }
        ...
        $('#selectBadReason".$myrow['wip_entity_id']."').dialog({
            ...
            width: '950px',
            height: '470px',
            content:'url:btnSearchBadReasons.php?fwValue=" .$myrow['wip_entity_id']."&type=model',
            init:function(){
                 this.content.document.getElementById('cat').value = 'buliao';
                this.content.document.getElementById('fwValue').value = '".$myrow['wip_entity_id']."';
            }
