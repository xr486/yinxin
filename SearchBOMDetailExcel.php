...
FROM bom_lines_all b,sf_item_no a
	...
	function DisplayBOMItems( $Component,$Level, $db ) {
		...
		$sql4 = "SELECT a.component_item,a.item_num,
				...
				where a.component_item=b.item_no   and a.disable_date=0 
				and bom_header_id in ( select max(bom_header_id) from bom_headers_all where assembly_item_no= '".$Component."' )
...
$filename = $item_name."当前BOM子料号明细".$date.".xlsx";
...
foreach($rows as $row)
	...
	while ($v = DB_fetch_array($result_num)) {
     ...
     //第二层
	   $sql2 = "SELECT a.component_item,a.item_num,
				...
				FROM bom_lines_all a, sf_item_no b
				where a.component_item=b.item_no   and a.disable_date=0 
				and bom_header_id in ( select max(bom_header_id) from bom_headers_all where assembly_item_no= '".$v['item_no']."' )
				";  	 
		$result2 = DB_query($sql2,$db);	
		while ($v2=DB_fetch_array($result2)) {				 
     $writer->writeSheetRow('Sheet1', array(' '.'-2',$v2['item_num'],$v2['operation_seq_num'],$v2['component_item'],$v2['item_name'],$v2['item_desc'],
        $v2['component_quantity'],$v2['units'],$v2['weizhi'],date('Y-m-d H:i:s', $v2['effectivity_date']),$disable_date,$v2['component_remarks'],
        ));
     ...
     //第三层
	   $sql3 = "SELECT a.component_item,a.item_num,
				...
				FROM bom_lines_all a, sf_item_no b
				where a.component_item=b.item_no   and a.disable_date=0 
				and bom_header_id in ( select max(bom_header_id) from bom_headers_all where assembly_item_no= '".$v2['component_item']."' )
				";  	 
     ...
     $writer->writeSheetRow('Sheet1', array('--3',$v3['item_num'],$v3['operation_seq_num'],$v3['component_item'],$v3['item_name'],$v3['item_desc'],
        $v3['component_quantity'],$v3['units'],$v3['weizhi'],date('Y-m-d H:i:s', $v3['effectivity_date']),$disable_date,$v3['component_remarks'],
        ));
	 ...
	 //第四层
	   $sql4 = "SELECT a.component_item,a.item_num,
				...
				FROM bom_lines_all a, sf_item_no b
				where a.component_item=b.item_no   and a.disable_date=0 
				and bom_header_id in ( select max(bom_header_id) from bom_headers_all where assembly_item_no= '".$v3['component_item']."' )
				";  	 
		$result4 = DB_query($sql4,$db);	
		while ($v4=DB_fetch_array($result4)) {				 
     $writer->writeSheetRow('Sheet1', array('---4',$v4['item_num'],$v4['operation_seq_num'],$v4['component_item'],$v4['item_name'],$v4['item_desc'],
        $v4['component_quantity'],$v4['units'],$v4['weizhi'],date('Y-m-d H:i:s', $v4['effectivity_date']),$disable_date,$v4['component_remarks'],
        ));
	 ...
	 //第五层
	   $sql5 = "SELECT a.component_item,a.item_num,
				...
				FROM bom_lines_all a, sf_item_no b
				where a.component_item=b.item_no   and a.disable_date=0 
				and bom_header_id in ( select max(bom_header_id) from bom_headers_all where assembly_item_no= '".$v4['component_item']."' )
				";  	 
		$result5 = DB_query($sql5,$db);	
		while ($v5=DB_fetch_array($result5)) {				 
     $writer->writeSheetRow('Sheet1', array('----5',$v5['item_num'],$v5['operation_seq_num'],$v5['component_item'],$v5['item_name'],$v5['item_desc'],
