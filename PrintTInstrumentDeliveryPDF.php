...
$sql8="SELECT a.customer_po,a.customer_code,b.customer_name,a.delivery_num,a.creation_date,ifnull(a.ship_address,b.customer_address) ship_address,b.bank_name,b.customer_contacts,b.bank_account,b.contacts_phone,b.contacts_fax,a.trackingcompany,a.narrative,(SELECT realname FROM www_users where userid = a.created_by) realname,(SELECT realname FROM www_users where userid = a.sub_approved_by) sub_approved_realname,(SELECT realname FROM www_users where userid = a.depart_approved_by) depart_approved_realname,(SELECT realname FROM www_users where userid = a.shouhuo_approved_by) shouhuo_approved_realname,a.original_delivery_num,b.customer_address
FROM so_delivery_headers_all a, customers b where a.customer_code=b.customer_code 
 ...
 $sql = "SELECT distinct so_order_number,subinventory_code
...
<p class="p1"><?php echo $myrow5['coyname'];?></p>
    ...
    <tr>
            <td width="80" >记录编号：</td>
            <td>YXZD-CC-ZD-002-R04</td>
            <td width="120">原销售出库单：</td>
            <td><?php echo $myrow6['original_delivery_num'];?></td>
            <td width="70">版本号：</td>
            <td>A/1</td>
        </tr>
        <tr>
        ...
        <tr>
            <td>地址：</td>
            <td><?php echo $myrow6['customer_address'];?></td>
            <td>联系人：</td>
    ...
    <table width="100%"  cellpadding="0" cellspacing="0" class="content_tab" >
        ...
        <tr>
            ...
            <td style="text-align:left;"> <?php echo '';?> </td>
			 ...
			 <span>备注：收货部门及时核对退货产品信息并退回仓库。</span><br/>
			 
    <table width="100%" border="0" cellpadding="0" cellspacing="0">
    <tr>
        <td width="30">制单人：</td>
        <td width="100"><?php echo $myrow6['realname'];?></td>
        <td width="50">收货人审核：</td>
        <td width="72"><?php echo $myrow6['shouhuo_approved_realname'];?></td>
        <td width="60">仓储物流部审核：</td>
        <td width="72"><?php echo $myrow6['sub_approved_realname'];?></td>
        
    </tr>
    </table>
    <span>第一联（仓库）                第二联（财务）                    第三联（收货部门）</span>
