...
a.order_amount,b.status
FROM sf_orders_all a,sf_order_lines_all b
WHERE a.order_number=b.order_number and (b.status ='OK' or b.status='NO') and b.start_work_flag='Y'";
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
    ...
    echo '<tr>
                   ...
                   <th class="ascending"width = 150>' . _('预计出货日期') . '</th>
                    <th class="ascending"width = 150>' . _('订单金额') . '</th>
					<th class="ascending"width = 150>' . _('检验状态') . '</th>
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ($RowIndex <> 10)) {
			...
			if ( $myrow['status']=='OK') {
			 $v_check_status='合格';}
			 elseif ( $myrow['status']=='NO') {
			 $v_check_status='不合格';}

			echo '  <td><a href="' . $RootPath . '/SOCheck2.php?NUM=' . $myrow['order_number'] . '">' . $myrow['order_number'] . '</td>
