...
$Title = _('查询待高阶签核销售订单');
$ViewTopic = '查询待高阶签核销售订单';
$BookMark = '查询待高阶签核销售订单';
...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
 ...
 WHERE p.customer_code = b.customer_code AND p.status   in ( '高阶签核') " ;
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('查询待高阶签核销售订单') . '</p>';
echo '<div class="text-nav">';
echo '<div class="text-nav-1"><div>' . _('订单起') . ':</div>';
echo '<input type="text" name="SO_from" value="' . $_POST['SO_from'] . '" size="20" maxlength="25" /></div>';

echo '<div class="text-nav-1"><div>' . _('订单止') . ':</div>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    echo '<tr>
                    ...
                    <th bgcolor="#87CEFA" class="ascending"width = 120>' . _('总额') . '</th>                 <th bgcolor="#87CEFA" class="ascending"width = 200>' . _('备注') . '</th>
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> $_SESSION['DisplayRecordsMax'] )) {
            ...
            echo '  <td><a target="_blank" href="' . $RootPath . '/ARFinApprove2.php?Updateorder_number=' . $myrow['order_number'] . '">' . $myrow['order_number'] . '</td>
				<td>' . $myrow['status'] . '</td>
