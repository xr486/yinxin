<?php
//  首先引入XLSXWriter包

putenv("NLS_LANG=AMERICAN_AMERICA.AL32UTF8");
ob_start();
include('includes/session2.inc');
include('includes/SQL_CommonFunctions.inc');
unset($result);
$sql ="SELECT  b.customer_code,
	b.customer_name, a.so_header_number,a.so_line_number,
	a.wip_entity_name,c.stockid,e.item_name,e.item_desc, a.plan_start_date,e.gongyi,
	e.units,a.start_quantity,a.quantity_completed,d.need_date,a.make_factory
	 from wip_jobs_all a, 
	 customers b, 
	 so_lines_all c  , 	
	   so_headers_all d,
       sf_item_no e
	where  d.customer_code=b.customer_code 
	and c.order_number=a.so_header_number
	and d.order_number=a.so_header_number
    and c.line=a.so_line_number 
    and e.item_no=c.stockid 
	and a.start_quantity=a.quantity_completed
	and a.status_type<>'结束'
	";
 //
	// echo $sql;


              
    if (isset($_GET['order_number']) and $_GET['order_number'] != '') { 
        $sql = $sql . " and a.so_header_number " . LIKE . " '%" . $_GET['order_number'] . "%' ";
    }
	 
if (isset($_GET['wip_entity_name']) and $_GET['wip_entity_name'] != '') { 
        $sql = $sql . " and a.wip_entity_name " . LIKE . " '%" . $_GET['wip_entity_name'] . "%' ";
    }
    if (isset($_GET['item_no']) and $_GET['item_no'] != '') { 
        $sql = $sql . " and e.item_no " . LIKE . " '%" . $_GET['item_no'] . "%' ";
    } 
	if (isset($_GET['item_name']) and $_GET['item_name'] != '') { 
        $sql = $sql . " and e.item_name " . LIKE . " '%" . $_GET['item_name'] . "%' ";
    } 
	 
    if (isset($_GET['customer_code']) and $_GET['customer_code'] != '') { 
        $sql = $sql . " and b.customer_code " . LIKE . " '%" . $_GET['customer_code'] . "%' ";
    }
  
     
      if(isset($_GET['FromDate']) and $_GET['FromDate'] != ''){
          $sql = $sql." and a.plan_start_date >=".strtotime($_GET['FromDate'])." ";
      }
       if(isset($_GET['ToDate']) and $_GET['ToDate'] != ''){
          $sql = $sql." and a.plan_start_date <=".strtotime($_GET['ToDate'])." ";
      }
   


 $result_num = DB_query($sql, $db); 
//oci_execute($par);
include_once("xlsxwriter.class.php");
$date=date('YmdHis');
//ini_set('display_errors', 0);
//ini_set('log_errors', 1);
error_reporting(E_ALL & ~E_NOTICE);

$filename = "工单已入库未关闭明细".$date.".xlsx";
header('Content-disposition: attachment; filename="'.XLSXWriter::sanitize_filename($filename).'"');
header("Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
header('Content-Transfer-Encoding: binary');
header('Cache-Control: must-revalidate');
header('Pragma: public');
$rows2 = array( 
  array('工单已入库未关闭明细'),

);
$rows = array( 
  array('工单名', '订单号','行','产品料号','料号名称','规格型号','单位' ,'开工量','完工量' , '生产日期' ),

); 
     	

$writer = new XLSXWriter();
$writer->setAuthor('Shunfansoft'); 

//$writer->writeSheetHeader('Sheet1', $header);
 foreach($rows2 as $row2)
	$writer->writeSheetRow('Sheet1', $row2);
foreach($rows as $row)
	$writer->writeSheetRow('Sheet1', $row);

	while ($v = DB_fetch_array($result_num)) {

     $writer->writeSheetRow('Sheet1', array($v['wip_entity_name'],$v['so_header_number'],$v['so_line_number'],
     $v['item_no'],
     $v['item_name'],$v['item_desc'],$v['units'].' ',$v['start_quantity'],$v['quantity_completed'], date('Y-m-d H:i:s',$v['plan_start_date']) 
		  ));
	 }
  
$writer->writeToStdOut();
//$writer->writeToFile('example.xlsx');
//echo $writer->writeToString();
exit(0);
?>
