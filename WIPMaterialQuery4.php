...
if (isset($Updateorder_number) and $Updateorder_number != '') {
    ...
    echo '<tr> <td>工单名称</td> <td>' . $myrow2['wip_entity_name'] . '</td>
        ...
        $sql2 = "SELECT  a.update_date,a.update_by,a.update_type,a.operation_seq_num,a.seq_id,a.quantity_per_assembly,a.quantity_issued, a.date_required,a.required_quantity,
	a.wip_entity_name,e.item_no,e.item_name,e.item_desc,e.units,a.comments,a.creation_date,a.created_by
	 from wip_material_requierment_log a,sf_item_no e
	where a.segment1=e.item_no
        ...
        if (DB_num_rows($result2) == 0) {
            unset($result2);
            prnMsg(_('没有找到用料变更信息！'), 'info');
        } else {
			echo '<p class="page_title_text">
		<img src="' . $RootPath . '/css/' . $Theme . '/images/customer.png" title="' . _('工单用料变更历史') .
 '" alt="" />' . ' ' . _('工单用料变更历史') . '
	</p>';
            ...
            while ($myrow = DB_fetch_array($result2)) {
                ...
                echo '<td>' . $myrow['update_by'] . '</td>
				<td>' . date('Y-m-d H:i:s',$myrow['update_date']) . '</td>
				<td>' . $myrow['update_type'] . '</td>
