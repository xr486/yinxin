...
$rows = array( 
  array('外协采购单号', '状态', '供应商', '备注', '采购日期', '需求日','下单日期','下单人员',),
...
foreach($rows as $row)
	...
	while ($v = DB_fetch_array($result_num)) {
     ...
     $writer->writeSheetRow('Sheet1', array($v['po_num'],$v_status,$v['vendor_name'],$v['note'],
     date('Y-m-d',$v['order_date']),date('Y-m-d',$v['need_date']),date('Y-m-d',$v['creation_date']),$v['created_by'],
 ));
	 }
