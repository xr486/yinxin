...
$rows = array( 
  array('来料报检单号', '行', '供应商编号', '采购单号' ,'行' , '料号', '料号名称', '规格型号', '单位', '采购数量', '来料报检量', '合格量' ,'待入库量','来料报检日期','仓库'),
...
foreach($rows as $row)
	...
	while ($v = DB_fetch_array($result_num)) {
     ...
     $v['quantity'],$v['this_received'],$v['already_inspection_qty'],$v['wait_delivery_quantity'],date('Y-m-d',$v['creation_date']) ,$v['subinventory_code'] ));
	 }
