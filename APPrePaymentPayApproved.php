...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
	...
	from fin_bank_transaction_headers_all pha, vendors c
			...
			and pha.vendor_code=c.vendor_code and transaction_num in (select transaction_num from fin_bank_transaction_lines_all where status = '建立')";
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('预付款冲销发票审核') . '</p>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> 10 )) {
            ...
            unset($v_status);
            echo '  <td><a href="' . $RootPath . '/APPrePaymentPayApproved2.php?New=Yes&Updatepo_num=' . $myrow['transaction_num'] . '">' . $myrow['transaction_num'] . '</td>'; 
			// else if ($myrow['transaction_type']=='AP退款') { 
			// echo '  <td><a href="' . $RootPath . '/APPaymentApproved3.php?New=Yes&Updatepo_num=' . $myrow['transaction_num'] . '">' . $myrow['transaction_num'] . '</td>';  
			// } else { 
			// echo '  <td><a href="' . $RootPath . '/APPaymentApproved4.php?New=Yes&Updatepo_num=' . $myrow['transaction_num'] . '">' . $myrow['transaction_num'] . '</td>';  
			// }
