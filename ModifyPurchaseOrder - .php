...
if (isset($_POST['Submit'])) {
    ...
    if ($_SESSION['num' . $identifier] == 400) {
        ...
        if ($InputError == 1) {
            ...
            $msg = '请输入数量和金额,再重新操作！';
            prnMsg($msg, 'error');
            echo '<br /><div class="centre"><a a href="' . htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '?identifier=' . $identifier . '">' . _('返回继续修改PO单') . '</a></div>';
        } else {
            for ($i = 0; $i < count($_POST["purchase_qty"]); $i++) {
                if ($_POST["purchase_qty"][$i] != '') {
                    DB_Txn_Begin($db);
...
$sql = "select order_id,
               ...
               supplier_name,
               supplier_number
          from sf_orders_all a where purchase_flag = 'O'
...
if (empty($_POST['customer_code']) == 0) {
 ...
 if (Is_Date($_POST['FromDate'])) {
  ...
  $sql .= " and a.purchase_date <=adddate('" . $SQL_ToDate . "',1) ";
  } 
...
//
        ...
        echo '<td><input type="text" class="number"   name="purchase_qty[]" size="8" maxlength="8"  value="' . $myrow['purchase_qty'] . '"  /></td>';
        echo '<td><input type="text" class="number"   name="purchase_price[]" size="8" maxlength="8"  value="' . $myrow['purchase_price'] . '" /></td>';
        echo '<td>' . $myrow['supplier_name'] . '</td>';
        echo '<td>' . $myrow['supplier_number'] . '   </td>
