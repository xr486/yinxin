...
if (isset($_POST['add'])) {
    header('Location: AddPOItemNo.php');
}
if (isset($_POST['Search']) or isset($_POST['Go']) or isset($_POST['Next']) or
    ...
    //查找数据的SQL
    $sql = 'select item_category, a.item_id,a.item_no,a.SafeQty,a.item_desc  from sf_item_no a  where   item_type=1 ';

    //SQL添加条件
    if (isset($_POST['ItemNo']) and $_POST['ItemNo'] != '') {
        $sql = $sql . " and a.item_no like '%" . $_POST['ItemNo'] . "%' ";
    }
	 if (isset($_POST['ItemCategory']) and $_POST['ItemCategory'] != '') {
        $sql = $sql . " and a.item_category like '%" . $_POST['ItemCategory'] . "%' ";
    }
...
?>
<p class="page_title_text"><img src="<?php echo $RootPath; ?>/css/<?php echo $Theme; ?>//images/magnifier.png" title="查询原材料" alt="查询原材料">查询原材料</p>
    ...
    <div>
        ...
        <table cellpadding="3" class="selection">
            <tr>	
				...
				<td>类别：</td>
                <td><input   type="text" name="ItemCategory" value=<?= $_POST['ItemCategory'] ?> >
                </td> 
            </tr> 
            <tr>
        </table>
...
if (isset($_POST['Search']) and isset($result) or isset($_POST['Go']) or isset($_POST['Next']) or
    ...
    $ListPageMax = ceil($ListCount / 15); //$_SESSION['DisplayRecordsMax']
...
?>
    ...
    <?php } ?>
            ...
            <table cellpadding="2" class="selection">
                <tr>
                    <th class="ascending" width = "150" >原材料</th>
                    <th class="ascending" width = "300" >原材料描述</th> 
                    <th class="ascending" width = "100" >类别</th>  
                    <th class="ascending" width = "100" >安全库存</th>    
                </tr>
                <?php
    $k = 0; //row counter to determine background colour
    $RowIndex = 0;
    if (DB_num_rows($result) <> 0) {
        DB_data_seek($result, ($_POST['PageOffset'] - 1) * 15); // $_SESSION['DisplayRecordsMax']
        $i = 0; //counter for input controls
        while (($myrow = DB_fetch_array($result)) and ($RowIndex <> 15)) { //$_SESSION['DisplayRecordsMax']
            if ($k == 1) {
...
?>
                        <td>
                            <a href="<?= $RootPath ?>/UpdateItemNo2.php?ItemID=<?= $myrow['item_id'] ?>"><?= $myrow['item_no'] ?>
                        </td>
                        <td>
            <?= $myrow['item_desc'] ?>
                        </td> 
