...
if (isset($_POST['Submit'])) {
	...
	if (isset($_POST['SelectedMeasureID']) AND $_POST['SelectedMeasureID']!='' AND $InputError !=1) {
		...
		$sql = "SELECT count(*) FROM sf_item_set
		...
		if ( $myrow[0] > 0 ) {
			...
			// relavant
			$sql = "SELECT unitname FROM sf_item_set
				WHERE unitid = '" . $SelectedMeasureID . "'";
			$result = DB_query($sql,$db);
		...
		if ( $myrow[0] > 0 ) {
			...
			$sql = "INSERT INTO sf_item_set (unitname )
...
// PREVENT DELETES IF DEPENDENT RECORDS IN 'sf_item_no'
	...
	if ( DB_num_rows($result) == 0 ) {
		...
		if ($myrow[0]>0) {
			...
			$sql="DELETE FROM sf_item_set WHERE unitname ".LIKE."'" . $OldMeasureName . "'";
			$result = DB_query($sql,$db);
...
/* An unit of measure could be posted when one has been edited and is being updated
  ...
  or deletion of the records*/
	...
	echo '<table class="selection">
			<tr>
				<th bgcolor="#87CEFA" class="ascending">' . _('材料分类') . '</th>
			</tr>';
