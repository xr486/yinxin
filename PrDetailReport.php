...
$sql = "select p.wip_entity_name,a.pr_num,a.status,a.creation_date ,a.created_by,p.chang,p.kuan,p.gao, depart_name,a.need_date,a.remark,p.line,stockid,s.item_name,s.item_desc,p.uom,p.quantity,p.po_num,p.po_line,p.subinventory_code,a.approve_date,a.approve_by,p.price,p.line_amount,(select realname from www_users where userid=a.created_by) realname,p.vendor_code,(SELECT vendor_name FROM vendors v WHERE v.vendor_code=p.vendor_code) vendor_name
from pr_headers_all a, pr_lines_all p,sf_item_no s where p.stockid=s.item_no AND a.pr_num=p.pr_num ";  

$sql .= " order by a.pr_num desc, p.line asc";
//echo $sql;
$result = DB_query($sql,$db);
if(isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
    $sql = "select p.wip_entity_name,a.pr_num,a.status,a.creation_date ,p.chang,p.kuan,p.gao, a.created_by,depart_name,a.need_date,a.remark,p.line,stockid,s.item_name,s.item_desc,p.uom,p.quantity,p.po_num,p.po_line,p.subinventory_code,a.approve_date,a.approve_by,p.price,p.line_amount,(select realname from www_users where userid=a.created_by) realname,p.vendor_code,(SELECT vendor_name FROM vendors v WHERE v.vendor_code=p.vendor_code) vendor_name
 from pr_headers_all a, pr_lines_all p,sf_item_no s where p.stockid=s.item_no AND a.pr_num=p.pr_num ";   
 
 
if(isset($_POST['wip_entity_name']) and $_POST['wip_entity_name'] != ''){ 
        $sql = $sql." and p.wip_entity_name ".LIKE." '%".$_POST['wip_entity_name']."%' "; 
    }
   if(isset($_POST['pr_num_from']) and $_POST['pr_num_from'] != ''){ 
	...
	if(isset($_POST['item_no']) and $_POST['item_no'] != ''){ 
        $sql = $sql." and p.stockid ".LIKE." '%".$_POST['item_no']."%' "; 
    }
	if(isset($_POST['item_name']) and $_POST['item_name'] != ''){ 
        $sql = $sql." and s.item_name ".LIKE." '%".$_POST['item_name']."%' "; 
    }
...
echo '<div class="text-nav-1"><div>' . _('部门') . ':</div> ';
...
echo '<input type="text" name="item_name" value="' . $_POST['item_name'] . '" size="20" maxlength="25" /></div></div>';
...
if (isset($_POST['Search']) or isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
    ...
    echo '<tr> <th class="ascending" width = 100>' . _('请购单号') . '</th>
                    <th  width = 80>' . _('状态') . '</th> 
                    <th class="ascending"width = 120>' . _('使用部门') . '</th>
                    <th  width = 90>' . _('期望到货日期') . '</th>
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ($RowIndex <> $_SESSION['DisplayRecordsMax'] )) {
			...
			//end of page full new headings if
                ...
                echo '</table>
                <a href="' . $RootPath . '/PrDetailReportExcel.php?pr_num_from=' . $_POST['pr_num_from'] .'&wip_entity_name=' . $_POST['wip_entity_name'] . '&depart_name=' . $_POST['depart_name'] . '&item_no=' . $_POST['item_no'] . '&FromDate=' . $_POST['FromDate'] . '&item_name=' . $_POST['item_name'] . '&ToDate=' . $_POST['ToDate'] . '">' . '资料导出Excel表' . '</a>
                </div>';
