...
if (isset($Updateorder_number) and $Updateorder_number != '') {
    ...
    if (isset($_POST['approve'])){
        $sql = "update so_forecast_header set status = 'APPROVED',
    ...
    if (isset($_POST['cancel'])){
        $sql = "update so_forecast_header set status = 'CANCELLED',approve_date=". $creation_date .",approve_remark='". $_POST['approve_remark']  ."',approve_by='" . $_SESSION['UserID'] . "'  where order_number = '".$_POST['order_number']."' ";
        $result = DB_query($sql,$db);
    ...
    if (!isset($_GET['delete'])) {
        ...
        echo'
			...
			<tr >
				...
				<td>' . _('签核意见备注') . ':</td>
				<td colspan="2"> ' . $_POST['approve_remark'] . ' </td>
			</tr>  ';
        echo '</table>';
        echo '<br />';
        $sql2 = "SELECT c.status,line,stockid,a.item_desc,uom,quantity,quantity_cancelled,remark,subinventory_code, status,c.last_update_date,c.last_updated_by,c.need_date
	FROM  so_forecast_line c,
	   sf_item_no a
        ...
        echo '<br /> &nbsp;&nbsp;&nbsp;<input type="submit" name="return" value="关闭当前页面" />
