...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    if (empty($_POST['ToDate']) == 0) {
      ...
      $sql .= " order by a.wip_entity_name desc "; 
//echo $sql;
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    echo '<tr>       <th bgcolor="#87CEFA"    >' . _('工单号') . '</th> 
                    <th bgcolor="#87CEFA"  width = 100>' . _('母件料号') . '</th>
					 <th bgcolor="#87CEFA"   >' . _('母件料号名称') . '</th>
					 <th bgcolor="#87CEFA"  >' . _('母件规格型号') . '</th>
					...
					<th bgcolor="#87CEFA"  width = 70>' . _('欠料量') . '</th> 
					 <th bgcolor="#87CEFA"  >' . _('待检验') . '</th>
					 <th bgcolor="#87CEFA"  >' . _('允收待入库') . '</th>
					 <th bgcolor="#87CEFA"  >' . _('采购未收量') . '</th> 
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> $_SESSION['DisplayRecordsMax'] )) {
		   ...
		   <td>' . $myrow['mitem_no'] . '</td> 
                   ...
                   <td>' . date('Y-m-d', $myrow['plan_start_date']) . '</td>
					...
					<td>' . $wait . '</td> 
					<td>' . $myrow['wait_inspect_quantity'] . '</td> 
					<td>' . $myrow['wait_delivery_quantity'] . '</td> 
					<td>' . $myrow['wait_received'] . '</td> 
    ...
    if (isset($ListPageMax) AND $ListPageMax > 1) {
	 ...
	 echo '<br>
          <div class="centre">
                <a href="' . $RootPath . '/WIPMaterialDetailExcel.php?wip_entity_name=' .$_POST['wip_entity_name'] . '&item_no=' .$_POST['item_no'].'&item_name=' .$_POST['item_name'] .'&item_desc=' .$_POST['item_desc'] .'&FromDate=' .$_POST['FromDate'] .'&ToDate=' .$_POST['ToDate'] . '">
                ' .'资料导出Excel表' . '
                </a>
          </div>';
}
