...
if(isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
    $sql = 'SELECT pr_num, 
STATUS , description,creation_date
FROM pr_headers_all
WHERE 1 =1 ';

    if(isset($_POST['pr_num_from']) and $_POST['pr_num_from'] != ''){
        $sql = $sql." and pr_num >=  " .$_POST['pr_num_from']. " " ;
 
    }
    if(isset($_POST['pr_num_to']) and $_POST['pr_num_to'] != ''){
         $sql = $sql." and pr_num <=  ".$_POST['pr_num_to']." ";
    }
...
echo '<tr><td >' . _('请购单起') . ':</td><td>';
echo '<input type="text" name="pr_num_from" value="' . $_POST['pr_num_from'] . '" size="20" maxlength="25" /></td>';
echo '<td>' . _('请购单止') . ':</td>
	<td>';
echo '<input type="text" name="pr_num_to" value="' . $_POST['pr_num_to'] . '" size="20" maxlength="25" /></td>';
echo '</tr>';
...
--> 
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ($RowIndex <> 15)) {
			...
			echo '  <td><a href="' . $RootPath . '/SearchPR2.php?Updatepr_num=' . $myrow['pr_num'] . '">' . $myrow['pr_num'] . '</td>
				<td>' . $v_status . '</td>
