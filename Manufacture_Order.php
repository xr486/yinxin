...
AND EXISTS (
	...
	WHERE
		a.order_number = c.order_number
   
	AND (
		(ifnull(bumanufacture_flag, 'N') = 'N' and chuanghu_bu_item is not null)
		OR (ifnull(shamanufacture_flag, 'N')= 'N' and chuanghu_sha_item is not null)
	)
)
";
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> 10 )) {
            ...
            echo '  <td><a href="' . $RootPath . '/Manufacture_Order2.php?Updateso_num=' . $myrow['order_number'] . '">' . $myrow['order_number'] . '</td>
				<td>' . $v_status . '</td>
