...
/* $Id: Stocks.php 6371 2013-10-30 09:52:08Z exsonqu $ */
...
include('includes/Customized_Programs.php');
...
if (isset($_POST['NextItem'])){
	$Result = DB_query("SELECT stockid FROM stockmaster WHERE stockid>'" . $StockID . "' and CategoryID='原料' ORDER BY stockid ASC LIMIT 1",$db);
...
if (isset($_POST['PreviousItem'])){
	$Result = DB_query("SELECT stockid FROM stockmaster WHERE stockid<'" . $StockID . "' and CategoryID='原料' ORDER BY stockid DESC LIMIT 1",$db);
...
echo '<div class="centre"><a href="' . $RootPath . '/SelectProductP.php">' . _('返回选择物料') . '</a></div>
	<br />
...
if (isset($_POST['submit'])) {
	...
	if (!is_numeric(filter_number_format($_POST['Zhicheng']))) {
		$InputError = 1;
		prnMsg(_('制程需要输入数字'),'error');
		$Errors[$i] = 'Zhicheng';
		$i++;
	}
			...
			if ($InputError == 0){
				...
				$sql = "UPDATE stockmaster
						SET longdescription='" . $_POST['LongDescription'] . "',
							...
							zhicheng='" . filter_number_format($_POST['Zhicheng']) . "',
			...
			if (DB_num_rows($result)==1){
				...
				$sql = "INSERT INTO stockmaster (stockid,
							...
							VALUES ('".$StockID."',
								...
								'" . filter_number_format($_POST['Zhicheng']) . "',
                                                                '" . $_POST['ProductType'] . "',
                                                                '" . $_POST['ItemType'] . "',
								'" . $_POST['BarCode'] . "',
...
/*If the page was called without $StockID passed to page then assume a new stock item is to be entered show a form with a part Code field other wise the form showing the fields with the existing entries against the part will show for editing with only a hidden StockID field. New is set to flag that the page may have called itself and still be entering a new part, in which case the page needs to know not to go looking up details for an existing part*/
	...
	$_POST['Zhicheng']  = $myrow['zhicheng'];
        $_POST['ProductType']  = $myrow['producttype'];
        $_POST['ItemType']  = $myrow['itemtype'];
...
//<a target="_blank" href="'. $RootPath . '/StockCategories.php">' . _('Add or Modify Stock Categories') . '</a>
...
if (!isset($_POST['Zhicheng']) OR $_POST['Zhicheng']==''){
	$_POST['Zhicheng']=0;
}
...
echo '<input ' . (in_array('EOQ',$Errors) ?  'class="inputerror"' : '' ) .'   type="hidden" class="number" name="EOQ" size="12" maxlength="10" value="' . locale_number_format($_POST['EOQ'],'Variable') . '" />';

echo '<input ' . (in_array('Volume',$Errors) ?  'class="inputerror"' : '' ) .'   type="hidden" class="number" name="Volume" size="12" maxlength="10" value="' . locale_number_format($_POST['Volume'],'Variable') . '" />';

echo '<input ' . (in_array('GrossWeight',$Errors) ?  'class="inputerror"' : '' ) .'   type="hidden" class="number" name="GrossWeight" size="12" maxlength="10" value="' . locale_number_format($_POST['GrossWeight'],'Variable') . '" />';

echo '<input ' . (in_array('NetWeight',$Errors) ?  'class="inputerror"' : '' ) .'   type="hidden" class="number" name="NetWeight" size="12" maxlength="10" value="' . locale_number_format($_POST['NetWeight'],'Variable') . '" />';
echo '<input type="hidden"  name="Zhicheng" size="12" maxlength="10" value="1" />';

echo '<tr><td>' . _('用处') . ':</td>
			<td><input type="text" name="Disposal" size="15" maxlength="20"  value="' . $_POST['Disposal'] . '" /></td>
			</tr>';

echo '<tr><td>' . _('价格') . ':</td>
			<td><input type="text" name="ProductType" size="10" maxlength="15" value="' . $_POST['ProductType'] . '" /></td>
			</tr>';
echo '<tr><td>' . _('存储') . ':</td>
			<td><input type="text" name="ItemType" size="10" maxlength="15" value="' . $_POST['ItemType'] . '" /></td>
			</tr>';
...
echo '<input type="hidden"  name="MBFlag" size="12" maxlength="10" value="A" />';
...
if ($_POST['Discontinued']==0){
	echo '<option selected="selected" value="0">' . _('生效') . '</option>';
} else {
	echo '<option value="0">' . _('生效') . '</option>';
}
if ($_POST['Discontinued']==1){
	echo '<option selected="selected" value="1">' . _('失效') . '</option>';
} else {
	echo '<option value="1">' . _('失效') . '</option>';
}
echo '</select></td>
		</tr>';

echo '<input type="hidden"  name="CategoryID" size="12" maxlength="10" value="原料" />';

echo '<input type="hidden"  name="Controlled" size="12" maxlength="10" value="0" />';

echo '<input type="hidden"  name="Serialised" size="12" maxlength="10" value="0" />';

echo '<input type="hidden"  name="Perishable" size="12" maxlength="10" value="0" />';

echo '<input type="hidden"  name="DecimalPlaces" size="12" maxlength="10" value="2" />';

echo '<input ' . (in_array('BarCode',$Errors) ?  'class="inputerror"' : '' ) .'  type="hidden" name="BarCode" size="22" maxlength="20" value="' . $BarCode . '" />';

echo '<input type="hidden" name="DiscountCategory" size="2" maxlength="2" value="' . $DiscountCategory . '" />';

echo '<input type="hidden"  name="TaxCat" size="12" maxlength="10" value="1" />';

echo '<input type="hidden"  name="Pansize" size="12" maxlength="10" value="0" />';

echo '<input type="hidden"  name="ShrinkFactor" size="12" maxlength="10" value="0" />';
echo '</table>
	<div class="centre">';
...
if ($New==1) {
	echo '<input type="submit" name="submit" value="' . _('插入新料号') . '" />';
