...
and categoryid = '废品' ";
    ...
    if (isset($_POST['Units']) and $_POST['Units']!='') {
        $sql = $sql." and sm.units ".LIKE." '%".$_POST['Units']."%' ";
    }
...
echo '<td>' . _('或') . '</td>';
echo '<td>' . _('部分单位') . ':</td><td><input  type="text" name="Units" value="' . $_POST['Units'] . '" /></td></tr></table>';
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ($RowIndex <> $_SESSION['DisplayRecordsMax'])) {
		    ...
		    <td>' . $myrow['disposalname'] . '</td>
                    <td><a href="' . $RootPath . '/Stocks.php?StockID=' . $myrow['stockid'] . '">修改</td>
                </tr>';
            $i++;
            $RowIndex++;
