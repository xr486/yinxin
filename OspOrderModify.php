...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    if (isset($_POST['vendor']) and $_POST['vendor'] != '') {
        $sql = $sql . " and a.vendor_code " . LIKE . " '%" . $_POST['vendor'] . "%' ";
    }
...
echo '<input type="text"   autocomplete="off"   name="vendor"   value="' . $_POST['vendor'] . '" size="15" maxlength="25" /> ';
echo '</div>';
 
echo '<div class="text-nav-1"><div>' . _('供应商名称') . ':</div>';
echo '<input type="text"   autocomplete="off"   name="vendor_name"   value="' . $_POST['vendor_name'] . '" size="15" maxlength="25" /> ';
echo '</div>';
 
echo '<div class="text-nav-1"><div>' . _('外协采购单号') . ':</div>';
echo '<input type="text"   autocomplete="off"   name="po_num"   value="' . $_POST['po_num'] . '" size="15" maxlength="25" /> ';
echo '</div>';
echo '<div class="text-nav-1"><div>' . _('建单日期从') . ':</div>';
echo '<input type="text"   autocomplete="off"   name="FromDate"  onfocus="WdatePicker()" alt="' . $_SESSION['DefaultDateFormat'] . '" value="' . $_POST['FromDate'] . '" size="15" maxlength="25" /> </div>';
echo '<div class="text-nav-1"><div>' . _('到') . ':</div>';
echo '<input type="text"   autocomplete="off"   name="ToDate"  onfocus="WdatePicker()" alt="' . $_SESSION['DefaultDateFormat'] . '" value="' . $_POST['ToDate'] . '" size="15" maxlength="25" /> </div>';
echo '</div>';
 

/* echo '<tr><td >' . _('料号') . ':</td><td>';
echo '<input type="text"   autocomplete="off"   name="stockid" value="' . $_POST['stockid'] . '" size="20" maxlength="25" /></td>';
