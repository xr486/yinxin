...
if (isset($_POST['deleteRec'])) {
    ...
    if ($CancelDelete == 0) { //ie not cancelled the delete as a result of above tests
        $sql = "DELETE FROM customer_item_relation WHERE item_relation_id='" . $_POST['item_relation_id'] . "'";
        $result = DB_query($sql, $db);
        prnMsg(_('客户料号被成功删除') . ' !', 'success');
        echo '<br /><div class="centre"><a href="' . $RootPath . '/SearchCustomerItem.php">' . _('查询其它客户料号') . '</a></div>';
    ...
    if ($_SESSION['num' . $identifier] == 400) {
        ...
        if ($InputError == 0) {
            ...
            $SQL_ClientSince = FormatDateForSQL($_POST['ClientSince']);
             $v_date =time();     
                DB_Txn_Begin($db);          
                   $sql = "UPDATE customer_item_relation
...
//---------------------------
                ...
                unset($_POST['coyname']); 
                echo '<br />';
                echo '<br /><div class="centre"><a href="' . $RootPath . '/SearchCustomerItem.php">' . _('查询客户对应料号') . '</a></div>';
            
        } else {
            prnMsg(_('验证失败') . '. ' . _('不能更新'), 'error');
        }
    } else {
        header('Location: SearchCustomerItem.php');
    }
} else {
...
if (isset($Updateitem_relation_id) and $Updateitem_relation_id != '') {
    //CreditLimit,
    $sql = "select a.customer_code,a.customer_name,b.customer_item,b.remark,c.item_no,c.item_name,c.item_desc,b.enable_flag,b.item_relation_id,b.creation_date,b.created_by 
	...
	$_POST['customer_item'] = $myrow['customer_item'];
    ...
    if (!isset($_GET['delete'])) {
        ...
        echo '          <tr>
			...
			<td colspan="3">' . $_POST['item_desc'] . '</td>
				...
				<td bgcolor="#87CEFA">' . _('客户简称') . ':</td>
				<td colspan="3">' . $_POST['customer_code'] . '</td>
				 
				</tr>
				<td bgcolor="#87CEFA">' . _('客户全称') . ':</td>
				<td colspan="3">' . $_POST['customer_name'] . '</td>
			</tr>  
				<td bgcolor="#87CEFA">' . _('客户料号') . ':</td>
				<td><input  type="text" name="customer_item"  autofocus="autofocus" value="' . $_POST['customer_item'] . '" size="36" maxlength="40" /></td>
...
echo '</select></td>
        ...
        echo '<br />
			<div class="centre">
				...
				<input type="submit" name="deleteRec" value="' . "删除客户料号" . '" />&nbsp;
