...
if(isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
    $sql = 'select ah.receivenum,ah.receiveamount,ah.taxamount,ah.receivedate,ah.narrative ,ah.customer_code,ve.customer_name,ah.bankchangenum,ah.currency_code,ah.bankaccountname,ah.receive_type,ah.prereceive_used,al.creation_date,al.receiveline,al.shipment_num,al.amount,al.dis_amount
	...
	if(isset($_POST['receive_type']) and $_POST['receive_type'] != ''){
        $sql = $sql." and ah.receive_type = '".$_POST['receive_type']."' ";
    }
    ...
    //echo $SQL_FromDate;
    $sql .= " and  ah.receivedate >= '" . $SQL_FromDate . "' ";
}
if (empty($_POST['ToDate'])==0) {
    $SQL_ToDate = strtotime( $_POST['ToDate']) + 86400;
     //echo $SQL_ToDate;
    $sql .= " and  ah.receivedate <='" . $SQL_ToDate . "' ";
}

if (empty($_POST['FromDate1'])==0) {
    $SQL_FromDate1 = strtotime( $_POST['FromDate1']);
    //echo $SQL_FromDate;
    $sql .= " and  al.creation_date >= '" . $SQL_FromDate1 . "' ";
}
if (empty($_POST['ToDate1'])==0) {
    $SQL_ToDate1 = strtotime( $_POST['ToDate1']) + 86400;
     //echo $SQL_ToDate;
    $sql .= " and  al.creation_date <='" . $SQL_ToDate1 . "' ";
}
...
if (!isset($_POST['FromDate1'])) {
    $_POST['FromDate1'] = date( "Y-m-d", mktime(0,0,0,date("m"),date("d")-30,date("Y")));
}
if (!isset($_POST['ToDate1'])) {
    $_POST['ToDate1'] = Date('Y-m-d');
}

 echo '<tr><td>' . _('冲销日期从') . ':</td>
		<td>';
echo '<input type="text" name="FromDate1"   onfocus="WdatePicker()"  value="' . $_POST['FromDate1'] . '" size="20" maxlength="25" /></td>';
//echo '</tr>';
echo ' <td>' . _('到日期') . ':</td>
		<td>';
echo '<input type="text" name="ToDate1"  onfocus="WdatePicker()"  value="' . $_POST['ToDate1'] . '" size="20" maxlength="25" /></td>';
echo '<td  >' . _('客户名称') . ':</td><td>';
echo '<input type="text" name="customer_name" value="' . $_POST['customer_name'] . '" size="20" maxlength="25" /></td>';
echo '<td>' . _('客户代码') . ':</td> <td>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
    ...
    echo '<tr>
                    ...
                    <th class="ascending">' . _('收款号码') . '</th>
                    <th class="ascending">' . _('收款日期') . '</th>
                    <th class="ascending">' . _('收款金额') . '</th>
					<th class="ascending">' . _('币别') . '</th> 
					<th class="ascending">' . _('银行收款单/转账号码') . '</th>
					<th class="ascending">' . _('银行账户名') . '</th>
					<th class="ascending">' . _('冲销日期') . '</th>
                    <th class="ascending">' . _('收款备注') . '</th> 
					 <th class="ascending">' . _('行') . '</th> 
					 <th class="ascending">' . _('出货单号') . '</th>
					 <th class="ascending">' . _('收款金额') . '</th>
					 <th class="ascending">' . _('优惠金额') . '</th>
            </tr>';
