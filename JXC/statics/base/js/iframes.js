...
function okonlineqq(type) {
	...
	$('#met' + type + 'img').attr('src', metimgurl + type + '/' + type + '_' + $("input[name='" + type + "_type']:checked").val() + hz);
...
$(document).ready(function() {
	...
	$(".skedtype span").click(function(){
		...
		var top = $("h3[sliding='"+sliding+"']").offset().top+88;
...
/*批量推荐、置顶、WAP*/
var listpilang = $("select[name='lisnew'],select[name='liscom'],select[name='listop'],select[name='liswap']");
...
$(document).ready(function() {
	$(".metsliding_box").hide();
	$(".metsliding_box_1").show();
	$(".metsliding").click(function(e){
		var thbox = $(".metsliding_box_"+$(this).attr("sliding"));
		if(thbox.is(":hidden")){
			thbox.show();
		}else{
			thbox.hide();
...
$(function(){
    $(".bnt_tags").click(function() {
		...
		if ($("#" + c)[0].value=="") {
			...
			var url, data;
			url = gettagspath;
			data = "title=" + encodeURIComponent($("#" + c)[0].value);
		    $.ajax({
				type: "post",
				cache: !1,
				url: url,
