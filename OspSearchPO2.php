...
if (isset($Updatepo_num) and $Updatepo_num != '') {
    //CreditLimit,
    $sql = "SELECT pha.po_num, pha.status,order_type, pha.note,pha.payment_term ,pha.order_date,pha.app_remark, pha.creation_date, pha.tax_amount,pha.all_line_amount, pha.created_by, pha.youhui_amount,pha.approve_date, pha.approved_by,vendor_name,pha.po_all_amount,pha.vendor_code,pha.payment_type,pha.tax_name,pha.delivery_coyname,pha.delivery_date
...
AND po_num= '" .$Updatepo_num."'";
    ...
    if (!isset($_GET['delete'])) {
        ...
        if ($_POST['status'] == 'INPROCESS') {
            ...
            $v_status = $_POST['status'];
			  ...
			  if ($_POST['approve_date']>1) {
		      $approve_date = date('Y-m-d H:i:s',$_POST['approve_date']);
			  } 
        ...
        <input type="text"   autocomplete="off"   readonly="readonly" value="' . $_POST['po_num'] . '" /></div>
                  <input  type="hidden" name="po_num"  value="' . $_POST['po_num'] . '" />
			
                <div class="text-nav-1"><div>' . _('状态') . ':</div>
				<input type="text"   autocomplete="off"   readonly="readonly" value="' . $v_status . '" /> </div>
        ...
        echo'
           ...
           if ($_SESSION['price_flag']=='N') {
			echo ' 
				...
				<input type="text"   autocomplete="off"   readonly="readonly" value="' . $_POST['po_all_amount']. '" /> </div>
               ...
               echo ' 
		   <div class="text-nav-1"><div>' . _('签核人员') . ':</div>
				<input type="text"   autocomplete="off"   readonly="readonly" value="' . $_POST['approved_by'] . '" /> </div>
				<div class="text-nav-1"><div>' . _('签核日期') . ':</div>
				<input type="text"   autocomplete="off"   readonly="readonly" value="' .  $approve_date . '" /> </div>
                 <div class="text-nav-1"><div>' . _('签核意见') . ':</div>
				<input type="text"   autocomplete="off"   readonly="readonly" value="' . $_POST['app_remark']   . '" /> </div>
    ...
    <input type="text"   autocomplete="off"   readonly="readonly" value="' . $_POST['order_type']. '" /></div>
        ...
        echo '</table>';
        echo '<br />';
        $sql2 = "SELECT a.line,a.stockid,g.item_name,g.units,a.price,a.quantity,a.po_num,a.line_amount,a.creation_date,ifnull(quantity_received,0) quantity_received,ifnull(quantity_accepted,0) quantity_accepted ,
            ifnull(a.quantity_deliveried,0) quantity_deliveried,ifnull(a.quantity_cancelled,0) quantity_cancelled,ifnull(a.quantity_rejected,0) quantity_rejected,a.created_by,a.need_date,a.operation_code,a.operation_seq_num,a.wip_entity_name
                  from po_lines_all a,wip_jobs_all b,so_lines_all c,sf_item_no g where a.wip_entity_name=b.wip_entity_name and  b.so_header_number=c.order_number and c.stockid=g.item_no and  	b.so_line_number=c.line and  1=1 and    po_num = '" .$Updatepo_num."' order by a.line";
