...
// Keep the project module visible for sessions created before it was added.
...
// 			$default_url = $RootPath . '/sale_ekanban.php';
			$default_url = $RootPath . '/sale_ekanban123.php';
			$default_name = '个人主页';
		}
...
// 顶部样式
$_SESSION['CompanyRecord']['coyname'] = "易道";
		...
		<ul id="left1" class="leftnav_first_ul">
			...
			while ($i < count($ModuleLink)) {
				...
				$arr = array(
					...
					$moduleIcon = $arr[$ModuleList[$i]];
					$moduleIconSrc = $RootPath . '/css/' . $Theme . '/images/' . $moduleIcon . '.png';
					$moduleIconStyle = '';
					if ($ModuleList[$i] === '项目管理') {
						$moduleIconSrc = $RootPath . '/project_management/icons/project.webp';
						$moduleIconStyle = 'filter:invert(1);';
					}
					echo '<li class="litem leftnav_first_li">
                        <a href="##" style="color:#fff;text-decoration:none;display:flex;align-items:center;">
                            <img style="width:15px;height:15px;margin-right:2px;' . $moduleIconStyle . '" src="' . $moduleIconSrc . '" />
					...
					} else if ($_SESSION['Module'] == 'project_management') {
						include 'project_management/main_menu.php';
					} else {
