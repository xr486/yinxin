...
$Title = _('查询研发已处理的报价申请');
$ViewTopic = '查询研发已处理的报价申请';
$BookMark = '查询研发已处理的报价申请';
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('查询研发已处理的报价申请') . '</p>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> 10 )) {
            ...
            echo '   <td><a target="_blank" href="' . $RootPath . '/QuoteToCustomer2.php?Updateorder_number=' . $myrow['order_number'] . '">' . $myrow['order_number'] . '</td>
				<td>' . $myrow['status']. '</td>
