...
FROM
  ...
  www_users a
WHERE
  a.salesman = b.salesmancode
and a.userid = '". $_SESSION['UserID']."' 
                         and effective_date < ".$v_date.
                         " and (disable_date >  ".$v_date .
                        " or disable_date = 0 )";
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('合同：选择客户') . '</p>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ($RowIndex <> $_SESSION['DisplayRecordsMax'])) {
			...
			echo '  <td><a href="' . $RootPath . '/AddOrder.php?New=Yes&customer_code=' . $myrow['customer_code'] . '">' . $myrow['customer_code'] . '</td>
