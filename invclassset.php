...
if (isset($_POST['Submit'])) {
	...
	if (ContainsIllegalCharacters($_POST['MeasureName'])) {
		$InputError = 1;
		prnMsg( _('请输入正确的交易名称') ,'error');
	}
	if (trim($_POST['MeasureName']) == '') {
		$InputError = 1;
		prnMsg( _('交易名称不能为空'), 'error');
	}
	...
	if (isset($_POST['SelectedMeasureID']) AND $_POST['SelectedMeasureID']!='' AND $InputError !=1) {
		...
		$sql = "SELECT count(*) FROM mtl_transaction_type
				WHERE unitid <> '" . $SelectedMeasureID ."'
				AND type_name ".LIKE." '" . $_POST['MeasureName'] . "'";
		$result = DB_query($sql,$db);
		$myrow = DB_fetch_row($result);
		if ( $myrow[0] > 0 ) {
			$InputError = 1;
			prnMsg( _('交易名称有重复，请重新输入'),'error');
		} else {
			...
			// relavant
			$sql = "SELECT type_name,transaction_type FROM mtl_transaction_type
				WHERE unitid = '" . $SelectedMeasureID . "'";
			$result = DB_query($sql,$db);
			if ( DB_num_rows($result) != 0 ) {
				...
				$sql = array();
				$sql[] = "UPDATE mtl_transaction_type
					SET type_name='" . $_POST['MeasureName'] . "',
                    
                    transaction_type='" . $_POST['MeasureName2'] . "'
					WHERE type_name ".LIKE." '".$OldMeasureName."'";
				$sql[] = "UPDATE mtl_transaction_type
					SET type_name='" . $_POST['MeasureName'] . "',
		...
		$sql = "SELECT count(*) FROM mtl_transaction_type
				WHERE type_name " .LIKE. " '".$_POST['MeasureName'] ."'";
		$result = DB_query($sql,$db);
		$myrow = DB_fetch_row($result);
		if ( $myrow[0] > 0 ) {
			$InputError = 1;
			...
			$sql = "INSERT INTO mtl_transaction_type (type_name,transaction_type)
...
// PREVENT DELETES IF DEPENDENT RECORDS IN 'sf_item_no'
	...
	if ( DB_num_rows($result) == 0 ) {
		// This is probably the safest way there is
		prnMsg( _('不能删除该交易类型，因为交易类型已经不存在'),'error');
	} else {
		$myrow = DB_fetch_row($result);
		$OldMeasureName = $myrow[0];
		$sql= "SELECT COUNT(*) FROM inv_transactions_all WHERE transaction_type = '" . $OldMeasureName . "'";
		$result = DB_query($sql,$db);
		$myrow = DB_fetch_row($result);
		if ($myrow[0]>0) {
			prnMsg( _('不能删除这个交易类型，因为有属于该交易类型'),'error');
			echo '<br />' . _('There are') . ' ' . $myrow[0] . ' ' . _('个属于这个交易类型') . '</font>';
		} else {
			$sql="DELETE FROM mtl_transaction_type WHERE type_name ".LIKE."'" . $OldMeasureName . "'";
			$result = DB_query($sql,$db);
			prnMsg( $OldMeasureName . ' ' . _('交易删除成功') . '!','success');
		}
...
/* An unit of measure could be posted when one has been edited and is being updated
  ...
  or deletion of the records*/
	...
	$sql = "SELECT unitid,type_name,
			transaction_type
			FROM mtl_transaction_type
			ORDER BY unitid";
	...
	echo '<table class="selection">
			<tr>
				<th class="ascending">' . _('交易名称') . '</th>
                
                <th class="ascending">' . _('交易类型') . '</th>
                 <th>' . _('编辑') . '</th>
                        <th>' . _('删除') . '</th>
			</tr>';
	...
	while ($myrow = DB_fetch_row($result)) {
		...
		echo '<td><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?SelectedMeasureID=' . $myrow[0] . '&amp;delete=1" onclick="return confirm(\'' . _('是否要删除这个税率类型?') . '\');">' . _('Delete')  . '</a></td>';
		echo '</tr>';
...
if (isset($SelectedMeasureID)) {
	echo '<div class="centre">
			<a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '">' . _('查看交易类型') . '</a>
		</div>';
}
...
if (! isset($_GET['delete'])) {
	...
	if (isset($SelectedMeasureID)) {
		...
		$result = DB_query($sql, $db);
		if ( DB_num_rows($result) == 0 ) {
			prnMsg( _('找不到需要的交易类型，请重试.'),'warn');
			unset($SelectedMeasureID);
		} else {
			$myrow = DB_fetch_array($result);

			$_POST['MeasureID'] = $myrow['unitid'];
			$_POST['MeasureName']  = $myrow['type_name'];
            
            $_POST['MeasureName2']  = $myrow['transaction_type'];
...
echo '<div class="text-nav2">';

	echo '<div class="text-nav-1">
	<div>' . _('交易名称') . ':' . '</div>
		<input required="required" pattern="(?!^ *$)[^+<>-]{1,}" type="text" name="MeasureName" title="'._('Cannot be blank or contains illegal characters').'" placeholder="'._('请输入交易名称').'" size="30" maxlength="30" value="' . $_POST['MeasureName'] . '" /></div>
		';
        ...
        echo '<div class="text-nav-1">
		<div>' . _('交易类型') . ':</div>
		<select required="required" name="MeasureName2">';


	echo '<option value="杂项出库">' . _('杂项出库') . '</option>';
	echo '<option value="杂项入库">' . _('杂项入库') . '</option>';
