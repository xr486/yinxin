...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    $sql = "select ap_invoice_type,invoice_type,
			      pha.invoice_num,pha.invoice_name,
			   pha.invoice_amount,pha.payment_amount,
			       pha.tax_amount,
                  c.vendor_code,
                    pha.narrative,
				pha.currency_code,
 ...
 $sql .= " order by pha.invoice_date  desc ";
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('供应商发票查询') . '</p>';
...
echo '</table><div class="centre"><input type="submit" name="Search" value="查找"> 
 <a href="' . $RootPath . '/APSearchInvoiceExcel.php?invoice_num=' .$_POST['invoice_num'] .'&invoice_name'  .$_POST['invoice_name'] .
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    echo '<tr>    <th class="ascending" width = 150>' . _('发票单号') . '</th>
                    <th class="ascending" width = 150>' . _('发票号码') . '</th>
                    <th   width = 70>' . _('状态') . '</th>
					<th   width = 70>' . _('发票类型') . '</th>
                    <th class="ascending"width = 100>' . _('供应商代码') . '</th>
                    <th class="ascending"width = 250>' . _('供应商名称') . '</th>
					<th   width = 70>' . _('类型') . '</th>
                    <th class="ascending"width = 90>' . _('发票金额') . '</th>  
                    <th class="ascending"width = 90>' . _('已付款金额') . '</th>    
                    <th class="ascending"width = 90>' . _('税别') . '</th>     
                    <th class="ascending"width = 90>' . _('税额') . '</th>                          
                    <th class="ascending"width = 150>' . _('备注') . '</th>
                    <th class="ascending"width = 90>' . _('发票日') . '</th>
                    <th class="ascending"width = 160>' . _('建立时间') . '</th>
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> 50 )) {
            ...
            unset($v_status);
			if ($myrow['invoice_type']=='费用发票') {
			 echo '  <td><a  target="_blank" href="' . $RootPath . '/APSearchInvoice3.php?Updatepo_num=' . $myrow['invoice_name'] .'&vendor_code=' . $myrow['vendor_code'] .'">' . $myrow['invoice_name'] . '</td>';
			}   else {
			echo '  <td><a  target="_blank" href="' . $RootPath . '/APSearchInvoice2.php?Updatepo_num=' . $myrow['invoice_name'] .'&vendor_code=' . $myrow['vendor_code'] .'">' . $myrow['invoice_name'] . '</td>';
