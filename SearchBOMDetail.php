...
echo '<p class="page_title_text">
		<img src="' . $RootPath . '/css/' . $Theme . '/images/customer.png" title="' . _('当前BOM明细') .
 '" alt="" />' . ' ' . _('当前BOM明细') . '
	</p>';
...
<div class="text-nav">
	...
	bom_routings_all c 
		...
		from bom_substitutes_all bsa where b.component_sequence_id=bsa.component_sequence_id ) sub_count,b.sunhao_rate
FROM bom_lines_all b,sf_item_no a
        ...
        if (DB_num_rows($result2) == 0) {
            unset($result2);
            prnMsg(_('没有找到当前BOM明细，请重新登录查询！'), 'info');
        } else {
...
echo '<div>
        <a href="' . $RootPath . '/SearchBOMDetailExcel.php?item_no=' .$searchitem_no .'&item_name=' .$item_name .' ">' .'资料导出Excel表' . '</a>
    </div>';
		...
		while ($myrow=DB_fetch_array($result4)) {
			...
			$Level1 = str_repeat('-&nbsp;',$Level-1).$Level;
			  ...
			  echo '  <td >' . $Level1 . '</td>
			   ...
			   DisplayBOMItems( $myrow['component_item'],$Level+1,$db);
