...
if(isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
    $sql = 'select ah.payment_num,ah.payment_amount,ah.tax_amount,
	                ah.payment_date,ah.narrative,ah.vendor_code,ve.vendor_name,
					ah.bankchangenum,ah.currency_code,ah.bankaccountname,
					ah.payment_type,ah.prepayment_used,ah.creation_date 
            from    ap_payment_headers_all ah,vendors ve
            where  ve.vendor_code=ah.vendor_code
			  and  ah.payment_type="预付款"  ';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ($RowIndex <> $_SESSION['DisplayRecordsMax'])) {
			...
			$paymentdate= date('Y-m-d',$myrow['payment_date']); 
