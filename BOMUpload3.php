...
if (isset($Updatepo_num) and $Updatepo_num != '') {
    ...
    if (!isset($_GET['delete'])) {
        ...
        echo '<br />';
        $sql2 =" select a.stockid,replace(replace(replace(a.item_name,':',','),'：',','),'；',';')  new_desc,replace(replace(replace(b.item_name,':',','),'：',','),'；',';')  old_desc
	   from bom_upload a,sf_item_no b  where  
      a.stockid=b.item_no and  replace(replace(a.item_name,':',','),'：',',') <>replace(replace(b.item_name,':',','),'：',',') 
	   and a.created_by = '" . $_SESSION['UserID'] . "' ";
        ...
        if (DB_num_rows($result2) == 0) {
            ...
            $tableheader = '<tr>
                                        ...
                                        <th width="130">' . '料号' . '</th>
                                        <th  width="230" >' . '上传规格型号' . '</th>
                                         <th  width="230" >' . '系统规格型号' . '</th>
...
//
                echo ' 
		      <td>' . $myrow['stockid'] . '</td> 
					  <td>' . $myrow['new_desc'] . '</td>
					  <td>' . $myrow['old_desc'] . '</td>
