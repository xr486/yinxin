...
//    echo $sqlinsertrcvline;
    ...
    if($_POST['accept'.$i]>0){
    $sqltrancsation="insert into po_rcv_transactions(receipt_num,receipt_line,qc_remark,po_num,po_line,stockid,transaction_type,transaction_date,transaction_quantity,creation_date,created_by,last_update_date,last_updated_by,lot_num) ";
    $sqltrancsation.="values( '".$_POST['receipt_num'.$i]."','".$_POST['receipt_line'.$i]."','".$_POST['qc_remark'.$i]."','".$_POST['po_num'.$i]."','".$_POST['po_line'.$i]."','".$_POST['stockid'.$i]."','ACCEPT','".$time."','".$_POST['accept'.$i]."','".$time."','".$_SESSION['UserID']."','".$time."','".$_SESSION['UserID']."','".$_POST['lot_num'.$i]."')";
//    echo $sqltrancsation;
    $result_trancsation = DB_query($sqltrancsation, $db); 
    }
    if($_POST['reject'.$i]>0){
     $sqltrancsation="insert into po_rcv_transactions(receipt_num,receipt_line,qc_remark,po_num,po_line,stockid,transaction_type,transaction_date,transaction_quantity,creation_date,created_by,last_update_date,last_updated_by,lot_num) ";
    $sqltrancsation.="values( '".$_POST['receipt_num'.$i]."','".$_POST['receipt_line'.$i]."','".$_POST['qc_remark'.$i]."','".$_POST['po_num'.$i]."','".$_POST['po_line'.$i]."','".$_POST['stockid'.$i]."','REJECT','".$time."','".$_POST['reject'.$i]."','".$time."','".$_SESSION['UserID']."','".$time."','".$_SESSION['UserID']."','".$_POST['lot_num'.$i]."')";
//    echo $sqltrancsation;
    $result_trancsation = DB_query($sqltrancsation, $db);
       ...
       unset($result_trancsation);
        ...
        if ($InputError == 1) {
            ...
            $msg = '检测成功！';
            prnMsg($msg, 'success');
             echo '<br /><div class="centre"><a href="' . $RootPath . '/InPOCheck.php">' . _('采购单进货检测') . '</a></div>';
