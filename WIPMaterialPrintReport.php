...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    $sql = " select   b.creation_date,b.created_by,b.request_name,c.item_no,c.item_desc,c.item_name,
   b.operation_seq_num,c.units,a.wip_entity_name,(b.need_quantity - b.issue_quantity) wait_quantity, b.need_quantity,b.issue_quantity
						from wip_jobs_all a,wip_material_request_lines b,sf_item_no c 
                        where b.wip_entity_name =a.wip_entity_name and b.item_no=c.item_no   ";
    ...
    if (empty($_POST['ToDate']) == 0) {
	 ...
	 $sql .= " order by b.request_name,a.wip_entity_name "; 
...
echo '<div class="text-nav-1"><div>' . _('申请日期起') . ':</div>';
...
echo '<div class="text-nav-1"><div>' . _('申请日期止') . ':</div>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    echo '<tr>    <th bgcolor="#87CEFA"    >' . _('申请日期') . '</th> 
	   <th bgcolor="#87CEFA"    >' . _('工单号') . '</th> 
                    <th bgcolor="#87CEFA"   >' . _('工序') . '</th>
                    <th bgcolor="#87CEFA"  width = 100>' . _('料号') . '</th>
					 <th bgcolor="#87CEFA"   >' . _('料号名称') . '</th>
					 <th bgcolor="#87CEFA"  >' . _('规格型号') . '</th>
					 <th bgcolor="#87CEFA"  >' . _('单位') . '</th>
                    <th bgcolor="#87CEFA"  width = 70>' . _('申请数量') . '</th> 
                    <th bgcolor="#87CEFA" width = 90>' . _('已领料数量') . '</th>
					<th bgcolor="#87CEFA"  width = 70>' . _('待领数量') . '</th>  
            </tr>';
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> $_SESSION['DisplayRecordsMax'] )) {
		   ...
		   <td>'. $myrow['wip_entity_name'] . '</td>  
					...
					<td>' . $myrow['units'] . '</td>   
					<td>' . $myrow['need_quantity'] . '</td> 
					<td>' . $myrow['issue_quantity'] . '</td> 
