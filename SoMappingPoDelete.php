...
if (isset($_GET['mapping_id'])) {
       $sql = "delete from so_po_mapping   where  mapping_id= '" . $_GET['mapping_id'] . "' ";
	   $result = DB_query($sql,$db);

	   $sql2 = "update  po_lines_all
	   set so_issue_qty= so_issue_qty - '" . $_GET['assign_qty'] . "'
	   where  po_num= '" . $_GET['po_num'] . "'
        and  line= '" . $_GET['po_line'] . "' ";
	   $result = DB_query($sql2,$db);

         DB_Txn_Commit($db);
		prnMsg('业务订单与采购单匹配关系删除成功',success);
}

  	 	 	 	 	  
if(isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
    $sql = "select a.so_num,a.so_line,a.creation_date ,a.created_by,a.po_num,a.po_line,a.assign_qty,b.customer_code,d.vendor_code,c.	item_no,c.item_name,c.item_desc,c.units,a.mapping_id
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ($RowIndex <> 15)) {
			...
			echo '  <td><a href="' . $RootPath . '/SearchSO4.php?Updateorder_number=' . $myrow['so_num'] . '" target="_blank" >' . $myrow['so_num'] . '</td>	
				...
				<td>' . $myrow['units'] . '</td>  <td><a href="' . $RootPath . '/SoMappingPoDelete.php?mapping_id=' . $myrow['mapping_id'] . '&po_num=' .$myrow['po_num'] .'&po_line=' .$myrow['po_line'] .'&assign_qty=' .$myrow['assign_qty'] .'"  >删除</td>	  
				';       	 
			echo '
			</tr>';
			$i++;
