...
//  ";
    $sql = " SELECT delivery_num,status,delivery_date,b.customer_code,  delivery_amount,a.creation_date,narrative,a.created_by,b.customer_name  FROM so_delivery_headers_all a, customers b  WHERE delivery_amount>0 and 1 = 1 AND a.customer_code = b.customer_code  ";
 ;
// echo $sql;
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    echo '<tr>
                    ...
                    <th class="ascending"width = 180>' . _('建立日期') . '</th>
                     <th class="ascending"width = 80>' . _('打印') . '</th            </tr>';
...
// var_dump($myrow);
            echo '  <td><a href="' . $RootPath . '/DeliveryReport2.php?Updatedelivery_num=' . $myrow['delivery_num'] . '">' . $myrow['delivery_num'] . '</td>
                <td>' . $v_status . '</td>
                ...
                <td>' . date('Y-m-d H:i:s', $myrow['creation_date']) . '</td>
                 <td> <a href="' . $RootPath . '/PrintDeliveryReport.php?Updatedelivery_num=' . $myrow['delivery_num'] . '" target="_blank">'. _('打印'). '</td>
                                 ';


          $totalamount = $myrow['delivery_amount']+$totalamount;
            echo '
            </tr>';
            $i++;
            $RowIndex++;
