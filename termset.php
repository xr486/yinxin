<?php
/* $Id: term_set.php 6495 2013-12-11 23:05:30Z rchacon $*/
...
if (isset($_POST['Submit'])) {
	...
	if (isset($_POST['SelectedMeasureID']) AND $_POST['SelectedMeasureID']!='' AND $InputError !=1) {
		...
		$sql = "SELECT count(*) FROM term_set
				WHERE termid <> '" . $SelectedMeasureID ."'
				AND term_name ".LIKE." ' " . $_POST['MeasureName'] . "'";
		$result = DB_query($sql,$db);
		$myrow = DB_fetch_row($result);
		if ( $myrow[0] > 0 ) {
			$InputError = 1;
			prnMsg( _('客户条件有重复，请重新输入'),'error');
		} else {
			...
			// relavant
			$sql = "SELECT term_name FROM term_set
				WHERE termid = '" . $SelectedMeasureID . "'";
			$result = DB_query($sql,$db);
			if ( DB_num_rows($result) != 0 ) {
				...
				$sql = array();
				$sql[] = "UPDATE term_set
					SET term_name='" . $_POST['MeasureName'] . "'
					WHERE term_name ".LIKE." '".$OldMeasureName."'";
				$sql[] = "UPDATE sf_item_no
					SET units='" . $_POST['MeasureName'] . "'
					WHERE units ".LIKE." '" . $OldMeasureName . "'";
			} else {
				$InputError = 1;
				prnMsg( _('客户条件不存在.'),'error');
			}
		}
		$msg = _('客户条件已经更新');
	} elseif ($InputError !=1) {
		...
		$sql = "SELECT count(*) FROM term_set
				WHERE term_name " .LIKE. "  '".$_POST['MeasureName'] ."'";
		$result = DB_query($sql,$db);
		$myrow = DB_fetch_row($result);
		if ( $myrow[0] > 0 ) {
			$InputError = 1;
			...
			$sql = "INSERT INTO term_set (term_name )
...
// PREVENT DELETES IF DEPENDENT RECORDS IN 'sf_item_no'
	...
	if ( DB_num_rows($result) == 0 ) {
		// This is probably the safest way there is
		prnMsg( _('不能删除该客户条件，因为客户条件已经不存在'),'warn');
	} else {
		$myrow = DB_fetch_row($result);
		$OldMeasureName = $myrow[0];
		$sql= "SELECT COUNT(*) FROM sf_item_no WHERE item_category1 ".LIKE." '" . $OldMeasureName . "'";
		$result = DB_query($sql,$db);
		$myrow = DB_fetch_row($result);
		if ($myrow[0]>0) {
			prnMsg( _('不能删除这个客户条件，因为有客户属于该客户条件'),'warn');
			echo '<br />' . _('There are') . ' ' . $myrow[0] . ' ' . _('个客户属于这个客户条件') . '</font>';
		} else {
			$sql="DELETE FROM term_set WHERE term_name ".LIKE."'" . $OldMeasureName . "'";
			$result = DB_query($sql,$db);
			prnMsg( $OldMeasureName . ' ' . _('客户条件删除成功') . '!','success');
		}
...
/* An unit of measure could be posted when one has been edited and is being updated
  ...
  or deletion of the records*/
	...
	echo '<table class="selection">
			<tr>
				<th class="ascending">' . _('客户条件类型') . '</th>
			</tr>';
...
if (isset($SelectedMeasureID)) {
	echo '<div class="centre">
			<a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '">' . _('查看客户条件类型') . '</a>
		</div>';
}
...
if (! isset($_GET['delete'])) {
	...
	if (isset($SelectedMeasureID)) {
		...
		$result = DB_query($sql, $db);
		if ( DB_num_rows($result) == 0 ) {
			prnMsg( _('找不到需要的客户条件类型，请重试.'),'warn');
			unset($SelectedMeasureID);
		} else {
			$myrow = DB_fetch_array($result);

			$_POST['MeasureID'] = $myrow['termid'];
			$_POST['MeasureName']  = $myrow['term_name'];
