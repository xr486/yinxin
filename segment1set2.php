...
// PREVENT DELETES IF DEPENDENT RECORDS IN 'sf_item_no'
	...
	if ( DB_num_rows($result) == 0 ) {
		...
		if ($myrow[0]>0) {
			...
			echo '<br />' . _('There are') . ' ' . $myrow[0] . ' ' . _('个成本属于这个类型') . '</font>';
		} else {
			$sql="DELETE FROM sf_item_category WHERE unitname ".LIKE."'" . $OldMeasureName . "'";
			$result = DB_query($sql,$db);
			prnMsg( $OldMeasureName . ' ' . _('成品类型删除成功') . '!','success');
		}
...
/* An unit of measure could be posted when one has been edited and is being updated
  ...
  or deletion of the records*/
	...
	echo '<table class="selection">
			<tr>
				<th bgcolor="#87CEFA" class="ascending">' . _('成品分类') . '</th>
			</tr>';
...
if (isset($SelectedMeasureID)) {
	echo '<div class="centre">
			<a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '">' . _('查看成品分类') . '</a>
		</div>';
}
