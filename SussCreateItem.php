<?php 
 ...
 require_once 'upload2.class.php'; //多文件上传
                    ...
                    for ($i = 0; $i < $length; $i++) {
                        if ($decoded_dest[$i]['name'] == $key) {
                            if (!$decoded_dest[$i]['dest']) {
                                ...
                                echo '<meta http-equiv="refresh" content="0; url=' . $RootPath .'/SussCreateItem.php?OrderNum=' . $_POST['OrderNum'] . '" />';
                            } else {
                                // echo $key . ': ' . $decoded_dest[$i]['dest'] . '<br>';
                                $sql = "insert into sf_item_no_file (file_name,item_no,file_patch,creation_date,created_by) values ('".$value."','".$_POST['OrderNum']."','".$decoded_dest[$i]['dest']."','".$time."','".$_SESSION['UserID']."')";
...
<table class="selection">
    <?php for($j=1;$j<=6;$j++){?>
        <div class="text-nav2" <?php echo $j>6&&$_POST['file_name'.$j]==''?'style="display:none"':''?>>
