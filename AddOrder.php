...
//            if ($myrow['flag'] != 0){
//                prnMsg( _('该采购单编号已存在，请重新输入采购单编号'), 'error');
//		$InputError=1;
//            }
//        }
//        $_SESSION['Contract']->Narrative = $_POST['Narrative'];
//	if ($_POST['SalesMan']=='') {
//		prnMsg( _('你必须选择一个采购员'), 'error');
//		$InputError=1;
//	}
...
//	}
	if ($InputError==0) {
		$_SESSION['Contract'.$identifier]->po_num=$_POST['po_num'];
...
//                }
			    ...
			    $_SESSION['Contract'.$identifier]->Subinventory_code=$_POST['Subinventory_code'];
...
if (isset($_POST['Edit'])) {
	$_SESSION['Contract'.$identifier]->LineItems[$_POST['LineNumber']]->Quantity=$_POST['Quantity'];
        $_SESSION['Contract'.$identifier]->LineItems[$_POST['LineNumber']]->POPrice=$_POST['POPrice'];
        $_SESSION['Contract'.$identifier]->LineItems[$_POST['LineNumber']]->amount=$_POST['POPrice']*$_POST['Quantity'];
}
...
//                 echo $value;
		if (filter_number_format($_POST['Quantity'.$Index])>0) {
            ...
            $POPrice = filter_number_format($_POST['po_price'.$Index]);
//                        $UOM=$_POST['UOM'.$Index];
//                        if ($POPrice<=0 or $POPrice==''){
//                            $flag = 1;
//                        } else{
                            $amount = $POPrice * $NewItem_array[$StockID];
//                            echo $amount;
                            $_SESSION['Contract'.$identifier]->AddLine($StockID, $ItemDescription, $NewItem_array[$StockID],$POPrice,$_POST['Units'.$StockID], $amount);    
//                        }
                }
	}
}
...
//    echo $rownum;
        ...
        $_SESSION['Contract'.$identifier]->po_num=$OrderNum;
	DB_Txn_Begin($db);
	$InputError=0;
//	if ($_SESSION['Contract'.$identifier]->compact_code=='') {
//		prnMsg( _('你必须输入一个采购单编号'), 'error');
//                echo '<br /><div class="centre"><a a href="'. htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '?identifier='.$identifier.'">' . _('返回采购单') . '</a></div>';
//		$InputError=1;
//	}    
...
//		$RequestNo = GetNextTransNo(38, $db);
		...
		foreach ($_SESSION['Contract'.$identifier]->LineItems as $LineItems) {
			$LineSQL="INSERT INTO po_lines_all (po_num,
													line,
													stockid,
													item_desc,
													price,
                                                    ...
                                                    quantity,
													 ...
													 subinventory_code,
                                                      ...
                                                      last_updated_by)
												VALUES(
                                                     '" . $_SESSION['Contract'.$identifier]->po_num . "',
                                                    ...
                                                    '".$LineItems->UOM."', 
                                                    '".$LineItems->note."',
													'".$_SESSION['Contract'.$identifier]->Subinventory_code."',
                                                    'INPROCESS',
													$create_date,
                                                    $create_date,
                                                    '".$_SESSION['UserID']."', 
													'".$_SESSION['UserID']."')";
			$ErrMsg =_('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR  SEEK ASSISTANCE') . ': ' . _('采购单Line不能存入数据库，原因是：');
			$DbgMsg = _('The following SQL to insert the request header record was used');
			$Result = DB_query($LineSQL,$db,$ErrMsg,$DbgMsg,true);
                        $sumamount=$sumamount+$LineItems->amount;
                        
		}
                $HeaderSQL="INSERT INTO po_headers_all ( po_num,
                                                         vendor, 
                                                         status,
                                                         amount,
                                                         note,
		...
		$ErrMsg =_('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' . _('采购单不能存入数据库，原因是：');//'" . FormatDateForSQL($_SESSION['Contract'.$identifier]->DispatchDate) . "',
		$DbgMsg = _('The following SQL to insert the request header record was used');
...
//            echo '<br /><div class="centre"><a href="'.$RootPath.'/AddPurchaseOrder.php">' . _('建立新的采购单') . '</a></div>';
	}
...
echo '	<div class="centre">
		<a href="'.$RootPath.'/AddPurchaseOrder.php">返回选择厂商</a>
	</div>';
	...
	echo '<tr>
		...
		<tr>
			...
			<td><input type="text" class="number" name="Quantity" value="' . $_SESSION['Contract'.$identifier]->LineItems[$_GET['Edit']]->Quantity. '" /></td>
                ...
                <tr>
			<td>' . _('单价') . '</td>
			<td><input type="text" class="float" name="POPrice" value="' . $_SESSION['Contract'.$identifier]->LineItems[$_GET['Edit']]->POPrice. '" /></td>
		</tr>';
...
echo '<table class="selection">';
echo '<tr>
		<th colspan="2" width=550 ><h4>' . _('采购单总体信息') . '</h4></th>
	</tr>';
echo 	'<tr>
	
                <td><input type="hidden" name="po_num" maxlength="20" size="15" value="' . $_SESSION['Contract'.$identifier]->po_num . '" /></td>
	</tr>';        
echo	'<tr>
		<td>' . _('采购员') . ':</td>'
...
echo '<td><select name="Subinventory_code">';
...
echo '<div class="centre">
		<input type="hidden" name="flag" value = "2" />
		<input type="submit" name="Update" value="' . _('保存采购单头信息并添加行') . '" />
	</div>
    </div>
	</form>';
}
if (!isset($_SESSION['Contract'.$identifier]->po_num) or $_SESSION['Flag'.$identifier] == 1) {
	include('includes/footer.inc');
	exit;
}
...
if (isset($SearchResult)) {
	$j = 1;
	echo '<br />
		<div class="page_help_text">' . _('通过输入需求数量来选择物料，当完成后点击添加至采购单。') . '</div>
		<br />
	...
	if ($ListCount > 0) {
		...
		<tr>
			<td>
				...
				<input tabindex="'.($j+9).'" type="submit" value="'._('添加至采购单').'" /></td>';
		if ($Offset >= 0){
        echo '
                            <td>
...
//		}
		...
		echo '<td>' . $myrow['item_no'] . '</td>
				...
				<td><input type="text" name="po_price'.$i.'" value="' . $myrow['po_price'] . '" /> </td> 
	...
	echo '<tr>
			...
			<td style="text-align:center" colspan="6"><input type="hidden" name="order_items" value="1" />
				<input tabindex="'.($j+8).'" type="submit" value="'._('添加至采购单').'" /></td>';
				if ($Offset >= 0){
        echo '
                            <td>
                                    <input type="hidden" name="NextList" value="'.($Offset+1).'" />
...
echo '<p class="page_title_text">' . _('采购单行维护'). '</p>';
// echo '<br /><div class="centre"><h4>' . _('采购单Line维护') . '</h4></div>';
echo '  <table class="selection">
	<tr>
		<th>' .  _('Line Number') . '</th>
...
foreach ($_SESSION['Contract'.$identifier]->LineItems as $LineItems) {
	...
	echo '<td>' . $LineItems->LineNumber . '</td>
			...
			<td>' . $LineItems->UOM . '</td>
                        <td class="number">' . locale_number_format($LineItems->POPrice) . '</td>
...
echo '	<div class="centre">
		...
		<input type="submit" name="SubmitFlag" value="' . _('返回采购单头信息') . '" />
	</div>';
echo '</form>';
