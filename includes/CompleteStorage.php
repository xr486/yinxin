...
echo '<td>' . _('From') . ':</td>
   ...
   // $SQL_ToDate = FormatDateForSQL($_POST['ToDate']);
    $sql = "select order_id,
              ...
              start_work_date,
              end_work_date
          from sf_orders_all a where delivery_qty is  null
          and complete_flag = 'Y'
...
//
		...
		<td><font color="red">' . $myrow['order_number'] . '</font></td>';
            ...
            <td><input type="text" class="number"   name="delivery_qty' . $myrow['order_id'] . '" size="25" maxlength="25"  value="' . $myrow['delivery_qty'] . '" /></td>
        </tr>';
        $RowCounter++;
        If ($RowCounter == 500) {
            $RowCounter = 1;
