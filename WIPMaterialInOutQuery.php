...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    echo '<div class="text-nav-table">
                    ...
                    <th bgcolor="#87CEFA"  width = 70>' . _('产品料号') . '</th>
                    <th bgcolor="#87CEFA"  width = 70>' . _('产品描述') . '</th>
					...
					<th bgcolor="#87CEFA"  width = 50>' . _('工序') . '</th>     
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> $_SESSION['DisplayRecordsMax'] )) {
            ...
            <td>' . $myrow['status'] . '</td> 
            <td>' . $myrow['master_item'] . '</td> 
            <td>' . $myrow['master_name'] . '</td> 
            ...
            <td>' . $myrow['quantity'] . '</td> 
                   ...
                   if($myrow['transaction_type'] == '工单领料'){

                       echo '<td><a href="' . $RootPath . '/PrintWIPIssue.php?Updatedelivery_num='.$myrow['trans_num'].'" target="_blank"  >打印</a></td>';
                   }else{

                       echo '<td><a href="' . $RootPath . '/PrintWIPReturn.php?Updatedelivery_num='.$myrow['trans_num'].'" target="_blank"  >打印</a></td>';
                   }
        ...
        echo '</table></div>';
		 echo '<br>
          <div class="centre">
                <a href="' . $RootPath . '/WIPMaterialInOutQueryExcel.php?wip_entity_name=' .$_POST['wip_entity_name'] . '&item_no=' .$_POST['item_no'].
				'&item_name=' .$_POST['item_name'] .'&item_desc=' .$_POST['item_desc'] .'&trans_num=' .$_POST['trans_num'] .
				'&loccode_from=' .$_POST['loccode_from'] .'&FromDate=' .$_POST['FromDate'] .'&ToDate=' .$_POST['ToDate'] . '">
                ' .'资料导出Excel表' . '
                </a>
          </div>';   
