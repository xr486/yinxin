...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    $sql = 'select order_id,
	       ...
	       delivery_qty,
               ...
               start_work_date,
               end_work_date
          from sf_orders_all a where delivery_qty is not null';
...
echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('完工入库查询') . '</p>';
...
if (isset($_POST['Search']) and isset($result) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
    ...
    echo '<tr>
	                                ...
	                                <th width =100>' . '订单号' . '</th>
                                        <th width =90 >' . '客户编码' . '</th>
                                         <th  width = 80>' . '订单量' . '</th> 
                                         <th width =80 >' . '入库量' . '</th>
                                         <th  width =160>' . '开工日期' . '</th>
                                         <th  width =160>' . '完工日期' . '</th>
            </tr>';
    ...
    if (DB_num_rows($result) <> 0) {
        ...
        while (($myrow = DB_fetch_array($result)) AND ( $RowIndex <> 10)) {//$_SESSION['DisplayRecordsMax']
            ...
            echo '<td >' . $myrow['delivery_qty'] . '   </td>
                  <td>' . date('Y-m-d H:i:s',$myrow['start_work_date']) . '   </td>
                <td>' . date('Y-m-d H:i:s',$myrow['end_work_date']) . '   </td>
			</tr>';
            $i++;
            $RowIndex++;
