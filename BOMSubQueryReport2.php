...
<div class="text-nav-1"><div>分类:</div ><input type="text" readonly="readonly" value="' . $myrow['item_category1'] . '" /></div>
        ...
        $sql2 = "SELECT b.component_sequence_id,b.assembly_item_no,b.operation_seq_num,b.component_quantity,b.sunhao_rate,b.weizhi,a.item_no,b.effectivity_date,
...
FROM bom_lines_all b,sf_item_no a
        ...
        if (DB_num_rows($result2) == 0) {
            ...
            while ($myrow = DB_fetch_array($result2)) {
			 ...
			 $sql5 = "SELECT c.operation_seq_num,a.creation_date,item_no,item_desc,item_name,b.units,a.substitute_item_quantity,
	 ...
	 FROM bom_substitutes_all a,sf_item_no b,bom_lines_all c              
        ...
        $result5 = DB_query($sql5, $db);
			   while ($myrow5 = DB_fetch_array($result5)) {
                 echo '
		              <td > </td> 
                      <td>' . $myrow5['operation_seq_num'] . '</td>
...
echo '<div>
        <a href="' . $RootPath . '/BOMSubQueryReportExcel.php?item_no=' .$searchitem_no .' ">' .'资料导出Excel表' . '</a>
    </div>';
