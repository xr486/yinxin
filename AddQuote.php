...
//            if ($myrow['flag'] != 0){
//                prnMsg( _('该报价单编号已存在，请重新输入报价单编号'), 'error');
//		$InputError=1;
//            }
//        }
...
//		prnMsg( _('你必须输入一个报价单编号'), 'error');
//                echo '<br /><div class="centre"><a a href="'. htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '?identifier='.$identifier.'">' . _('返回报价单') . '</a></div>';
//		$InputError=1;
//	}    
...
//		$RequestNo = GetNextTransNo(38, $db);
		...
		foreach ($_SESSION['Contract'.$identifier]->LineItems as $LineItems) {
			...
			$ErrMsg =_('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR  SEEK ASSISTANCE') . ': ' . _('报价单Line不能存入数据库，原因是：');
                ...
                $HeaderSQL="INSERT INTO so_headers_all ( so_num, 
						...
						VALUES(
											...
											$create_date,
                                                                                        $create_date,
											'" . $_SESSION['UserID'] . "','" . $_SESSION['UserID'] . "')";
		$ErrMsg =_('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' . _('报价单不能存入数据库，原因是：');//'" . FormatDateForSQL($_SESSION['Contract'.$identifier]->DispatchDate) . "',
		$DbgMsg = _('The following SQL to insert the request header record was used');
...
//            echo '<meta http-equiv="refresh" content="1; url=' . $RootPath . '/index.php" />';
			echo '<br />';
            prnMsg( _('报价单建立成功！报价单号为：'.$OrderNum), 'success');
//            echo '<br /><div class="centre"><a href="'.$RootPath.'/AddPurchaseOrder.php">' . _('建立新的报价单') . '</a></div>';
	}
...
if (isset($_GET['Edit'])) {
	...
	echo '<tr>
			<th colspan="2"><h4>' . _('修改报价单行资料') . '</h4></th>
		</tr>';
	echo '<tr>
			<td>' . _('行') . '</td>
...
echo '<table class="selection">';
echo '<tr>
		<th colspan="2" width=550 ><h4>' . _('报价单总体信息') . '</h4></th>
	</tr>';
echo 	'<tr>
...
echo '<div class="centre">
		<input type="hidden" name="flag" value = "2" />
		<input type="submit" name="Update" value="' . _('更新报价单头信息并添加行') . '" />
	</div>
    </div>
	</form>';
}
...
echo '
	...
	<div class="centre">
		<input type="hidden" name="flag" value = "2" />
		<input type="submit" name="Search" value="' . _('查询选择料号') . '" />
	</div>
	<br />
	</div>
	</form>';
...
if (isset($SearchResult)) {
	$j = 1;
	echo '<br />
		<div class="page_help_text">' . _('通过输入需求数量来选择物料，当完成后点击添加至报价单。') . '</div>
		<br />
	...
	if ($ListCount > 0) {
		...
		<tr>
			<td>
				...
				<input tabindex="'.($j+9).'" type="submit" value="'._('添加至报价单').'" /></td>';
		if ($Offset >= 0){
        echo '
                            <td>
...
//		echo '<input type="hidden" name="DecimalPlaces'.$i.'" value="' . $myrow['decimalplaces'] . '" />';
	...
	echo '<tr>
			...
			<td style="text-align:center" colspan="6"><input type="hidden" name="order_items" value="1" />
				<input tabindex="'.($j+8).'" type="submit" value="'._('添加至报价单').'" /></td>';
				if ($Offset >= 0){
        echo '
                            <td>
                                    <input type="hidden" name="NextList" value="'.($Offset+1).'" />
...
echo '<p class="page_title_text">' . _('报价单行维护'). '</p>';
// echo '<br /><div class="centre"><h4>' . _('报价单Line维护') . '</h4></div>';
echo '  <table class="selection">
	<tr>
		<th>' .  _('行') . '</th>
		<th  >' .  _('料号') . '</th>
...
echo '	<div class="centre">
		...
		<input type="submit" name="SubmitFlag" value="' . _('返回报价单头') . '" />
	</div>';
echo '</form>';
